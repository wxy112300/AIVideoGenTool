...
- History 新增极简批量管理：按当前筛选结果批量选择、复制文件、编辑标签和删除；视频复制最高分辨率版本，图片复制最后版本，标签编辑复用详情页交互并保持重复合并。
- History Tag 筛选支持“包含 / 排除”模式：可排除带有 NSFW 等 Tag 的内容；Tag 列表限高独立滚动，避免标签数量多时撑开筛选面板。
...
- 修复提示词版本把初始空白误记为第一版，以及“影视细节扩写”可能缩写、遗漏原始要求或缺少时间线的问题；详细预设增加覆盖率与长度验收，后端 JSON/Markdown 包装会在写入版本前清理。
- 修复 History 详情“继续创作”沿用旧草稿提示词版本的问题：打开历史任务时只恢复该任务提示词，从图片或视频输出继续时建立空白的新提示词分支，旧草稿不再被追加或误显示为当前版本。
- 修复 H3 Extend 提示词增强因界面没有 Slot 1 图片而退化为 T2VA 的问题：FL2VA 边界续写与 Continuum V3.8 按真实边界首帧输出 I2VA 对齐声明；Motion Context 保持 R2VA，以 `<Video 1>` 作为续写源，并只把临时末帧用于视觉核对，不污染执行参考编号。

## 0.61.8 — 2026-09-13

- 加固 H3 Prompt 增强的全局原文忠实度：十个预设现在都以用户原始意图、人物与动作归属、顺序、镜头、时间、对白、声音和结果为权威，只允许增加兼容的可观察细节，不得缩写、替换、调换或省略用户要求；“影视细节扩写”继续额外要求逐项充分展开。
- 修复参考图多人关系可能被错误重写的问题：增强器先确认用户命名角色与可见人物及其动作的对应关系，图像歧义时保留用户原称呼和动作归属；FL2VA 使用稳定视觉线索维持首尾帧身份、服装、动作和相对尺度，R2V 不再为未知人物强行分配年龄或其他无依据属性。
...
- 新增可选 MiniMax H3 VR180 SBS v2 LoRA：接入 `vr180sbs` 触发词、21:9/768p 与固定强度 1.0；输出仍需外部拉伸至 2:1 并写入立体球面元数据，I2V 与真实立体一致性未宣称验证。
- 修复视频创建页切换画面比例后，清晰度选项中的实际输出尺寸未同步更新的问题；比例变化现在会刷新分辨率选项，同时保留当前短边清晰度档位。

## 0.59.3 — 2026-09-05

- 修复图片任务入队会同步触发全量环境扫描并因探针超时而失败的问题；图片入队现在与视频一致，仅使用最近一次已完成的扫描结果，冷缓存时把模型、节点和 `/object_info` 校验延后到任务执行阶段。

## 0.59.2 — 2026-09-05

- H3 创作页新增 21:9 比例选项，按原生 32 像素网格生成；该比例仅在 H3 路径显示，切换到其他视频模型时自动回退为“原始比例”。

- 修复历史视频详情播放器延迟挂载后，左下角上一项/下一项按钮偶尔点击无响应的问题；导航改为根节点事件委托，并补充动态插入控件的回归测试。
...
- 完成架构健康 Gate 的静态边界、兼容性和当前 renderer 合成 smoke 审查；自动化验证通过。真实 Electron DevTools 性能前后对比、完整打包应用 smoke，以及窄屏图片/视频详情的现行结构与历史 harness 期望对齐，仍保留为后续验证项。

## 0.56.1 — 2026-08-31

- 完成 Queue、Create、History、Settings 的 renderer 窄依赖迁移与独立启动组合层收口；保持现有 IPC、持久化、队列、媒体和桌面工作流兼容。
- 完成 History 500 条数据量的视口媒体调度、图片/视频并发限制、批量 masonry 布局和滚动锚点恢复；自动化与合成基准通过。真实 Electron DevTools 性能对比和 viewport smoke 仍待环境补测，不把合成结果当作运行时性能承诺。
...
- 详细扩写改用自由时序：不再强制 3/4/5/6 个等距时间段或固定中间时间点；按准备、移动、接触、反应、减速和停稳等真实事件分配时间，仅在真实剪切、状态变化或必要的镜头/关键帧对齐时使用时间点，同时保留 H3 必需的首尾帧约束。规则同步覆盖 ComfyUI、LM Studio 和 H3 Prompt Writer 路径。

## 0.54.0 — 2026-08-30

- 创建页和历史详情页补充 H3 输入素材分辨率：FL2VA 参考图、R2V Slot 参考图以及历史输入素材均显示实际尺寸。
- 创建页在加入队列操作旁显示 H3 Token 数量估算；历史详情在“版本与生成参数”中显示本次提交实际使用的 Token 数量，旧记录没有该数据时保持兼容。
- 优化 H3 视频生成和续写入队：不再在按钮热路径中强制执行全量环境扫描，优先复用缓存；冷启动时把依赖检查延后到任务准备阶段。输入图片归档、哈希校验和失败阻断保持不变。
- 减少入队校验期间对完整 AppState 的重复深拷贝，降低大历史记录对入队响应的额外影响。

## 0.53.0 — 2026-08-29

- 性能与加速页新增 MiniMax H3 最终视频 VAE 后端选择：按环境扫描提供“自动”、FP16 基线和实验性 INT8 ConvRot 选项；“自动”优先 INT8，缺少所选文件时回退到另一份已安装 VAE，FP16 与 INT8 均缺失时禁用设置并阻止 H3 入队。设置保存后在下一条尚未开始的 H3 任务领取时生效，正在计算的任务不变；八条内置 H3 工作流统一使用该选择，音频 VAE 仍固定为 FP32，其他模型不受影响。

## 0.52.1 — 2026-08-29

- 队列卡片新增折叠操作菜单、提升到队首和随机 Seed，收纳低频操作并保持卡片主操作区简洁。
- 队列新增单条水平批次停止线：支持拖动调整位置、禁止放到第一个任务之前，等待任务可跨越停止线移动，任务与停止线位置会同步更新。
- 修复移除停止线、继续队列、队列结束及任务跨线拖动时的状态同步问题，避免队列被意外启动或暂停状态丢失。

## 0.52.0 — 2026-08-29

- 提示词增强新增 H3 角色尺度语义保护：识别 `tiny`、`miniature`、`shrink`、`giant` 及中文、比例和批注表达，将角色解释为相对于完整环境的统一世界尺度变化，保持参考角色的身份、年龄、脸部成熟度、头身比、四肢比例、姿态和年龄相符的行为，避免误变成玩具、塑料人偶或小孩。
- 尺度保护覆盖 T2VA、I2VA、FL2VA、L2VA、R2V，以及原生 Qwen、Qwen-VL、多模态、LM Studio/llama-server 和 H3 Prompt Writer 路径；模型遗漏关键尺度约束时，输出归一化会补回对应的 H3 描述字段。
- 提示词批注可在正文、括号或方括号要求中参与语义处理，但不会写入最终 H3 提示词正文；显式的 chibi、玩偶或夸张比例请求仍按用户意图保留。
...
- Spectrum MiniMax H3 推荐版本从 `v0.2.22` 更新为 `v0.2.23`：完成 active SA-Solver PECE 与 RefDelta 多后端互操作，并将 active-PECE 默认策略设为 `balanced`；现有 H3 工作流、参数和既有互操作不变。
...
- 设置页“节点与工作流”改为“节点与依赖”并移至第二项；内置核心与工作流继续由应用内部管理，不出现在可安装依赖列表。

## 0.50.0 — 2026-08-27

- 设置页新增队列运行时隔离策略，可选不重启、仅 LoRA 边界、仅模型变化或每个任务前重启；策略只在实际领取任务后、提交 workflow 前控制本地 ComfyUI 进程重启，H3 API 显存释放保持独立。默认使用 LoRA 边界隔离，远程端点保持 connection-only。
- 修正暂停、自然清空、取消和自动恢复的运行时所有权：暂停或清空不再为等待任务重启；本地任务取消会立即中止任务、卸载模型并关闭 ComfyUI，成功停止后不再执行 API 清理或恢复性重启；自动重试不会覆盖用户已暂停的队列。

## 0.49.4 — 2026-08-27

- 修复带视频 LoRA 的 H3 任务可能影响后续连续队列显存峰值的问题：任务结束后增加 LoRA 感知的两阶段释放，并以绝对空闲显存低水位替代相对下降量判断；普通无 LoRA H3 队列仍保持单阶段释放，仅在释放无法收敛时重启 ComfyUI。

## 0.49.3 — 2026-08-27

- 节点扫描现在把 MiniMax H3 Prompt Writer 专用运行接口的 HTTP 失败标记为可修复故障；每张节点卡片改用统一 38px 高的单一分体操作按钮，主按钮按状态显示安装、更新、修复或重装，卸载收纳在右侧菜单并将节点移入可恢复备份后重启复检。
- 修复节点分体按钮受全局 `details` 样式影响而上下错位的问题；主按钮与下拉按钮在标准及 1.25 倍缩放下保持同高、顶部对齐，菜单会在点击空白、其他操作或打开另一菜单时自动收起。

## 0.49.2 — 2026-08-26

- 修复 MiniMax H3 Prompt Writer `0.4.1` 兼容补丁丢失换行、导致 `gguf_backend.py` 语法错误且节点运行接口返回 HTTP 404 的问题。
- Prompt Writer 安全替换现在会在移动旧目录前，使用所选 ComfyUI Python 校验所有应用补丁文件的语法；验证失败时保留当前节点目录并报告原始错误。
- 新增本地 ComfyUI 运行时 harness，直接复用应用保存的设置和服务层接口执行 Prompt Writer 探针、依赖扫描、ComfyUI 重启及节点修复；默认操作只读并支持 JSON 输出，自动化诊断不再依赖桌面点击。

## 0.49.1 — 2026-08-26

- 修复 MiniMax H3 Prompt Writer 等节点在本地分支与上游分叉时因 `git pull --ff-only` 直接失败的问题；更新器现在会识别不可快进错误，完整备份旧目录，再克隆并校验干净副本，不会丢失本地提交或兼容补丁。
- 补充节点分叉更新的回归测试，并同步 H3 Desktop 运行时提示的可访问性断言。

## 0.49.0 — 2026-08-26

- 将 Windows/NVIDIA H3 ready 基线统一为 `torch 2.10.0+cu130`、`torchvision 0.25.0+cu130` 和 `torchaudio 2.10.0+cu130`；一键修复会把 2.9 环境升级到该组合，并要求匹配的 SageAttention `_fused` 原生扩展能够加载，避免 Desktop 只升级 Torch 后留下 ABI 不兼容 DLL。
- 将 Windows/NVIDIA H3 最低版本统一为 Torch 2.10/cu130，并以 `torch 2.10.0`、`torchvision 0.25.0`、`torchaudio 2.10.0` 作为稳定修复组合；三件套一致的更高 Torch 2.x 环境不再被降级，SageAttention 仍按 Comfy 官方精确 wheel、`_fused` 导入和 CUDA 自检独立判定（当前 Windows cu130 wheel 发布到 Torch 2.11）。
- 提示词增强日志在提交和完成事件中记录输入类型、用户选择的预设、实际生效的预设，以及无提示词自动起稿的种子/变体 ID；完成日志保留本次运行耗时，同时不写入提示词正文、预设正文或素材路径。
- 修复 Qwen3.6/Qwen3.8 的「影视细节扩写」在长时长下生成超过 `VisionLLMNode` 2048-token 输入上限的 workflow；共享 H3 预算保持不变，仅在 MultiModal Prompt Nodes 适配边界按运行节点契约限幅。

## 0.48.1 — 2026-08-26

- 统一队列 LoRA 标签与其他任务元数据标签的视觉样式，保留 LoRA 名称和强度信息。
- 修复 H3 R2V 队列卡片不显示输入参考素材缩略图的问题；图片参考沿用队列图片读取，视频参考支持按 R2V 槽位加载。
- 新增 H3「影视细节扩写」提示词预设：结合原始提示词与参考素材，在保持用户意图、对白/文字原语言和 H3 字段顺序的前提下，补充因果动作、身体反应、镜头动机、材质/环境反馈、灯光、音效和时间线细节；该模式拥有独立的本地输出余量和更宽的编辑器软参考范围。
- 修复 Gemma 4 26B-A4B Prompt Writer 使用通用多模态 chat template 后无法识别停止边界的问题，改用原生 Gemma 4 handler；恢复按请求预算自动选择 16K/24K 上下文，并清理无效的 4096-token 与 repeat-penalty 补偿补丁。
- Prompt Writer 安装器改为复用统一兼容适配器识别应用补丁，避免 `catalog.py` 上下文修复被误判为未知本地改动并触发重复备份或重装。
- 聚焦安装器测试 26 项与 TypeScript 类型检查通过；Gemma 4 26B-A4B 使用 2413 字符 creative brief、Auto 16K 和 2048-token 输出预算完成真实生成。

## 0.48.0 — 2026-08-26

- 为历史视频详情页播放器增加无标题右键菜单：复制文件、打开所在目录、使用系统播放器打开、查看视频信息、全屏/退出全屏；打开系统播放器前会自动暂停页面视频。
- 查看视频信息改为固定尺寸 Overlay，并修复全屏播放器内的文字布局和信息显示；保留播放器自动隐藏和关闭后焦点恢复。
- MiniMax H3 Prompt Writer 推荐版本更新至 `0.4.1`，兼容新版及嵌套路径的 GGUF 后端，并为非音乐模式补回 `2_048` 输出预算。
- `npm.cmd run verify` 通过（108 个测试文件、846 个测试、production build、20 组对比度检查）。

## 0.47.0 — 2026-08-26

- 重做历史视频详情页播放器和全屏控制栏：整合上一个/下一个、当前序号、播放/暂停、时间、进度、音量、收藏和全屏控制，并在全屏状态下保持自动隐藏。
- 增加按视频时长百分比跳转的左右方向键，统一为每次前进或后退 10%；音量控制改为接近原生播放器的悬停展开交互。
- 恢复播放器设置菜单中的下载、播放速度和画中画功能，修复菜单图标、运行时本地化 key 和控制按钮布局问题；序号显示压缩为 `current / total`。
- 弱化底部控制栏遮挡，补充分辨率、帧率和模型等视频参数信息，并保持收藏状态与详情侧栏同步。
- 按 Create、Queue、History、Settings 页面边界拆分 Renderer 构建 chunk；主入口降至约 434 kB，消除 500 kB chunk 构建警告。
- `npm.cmd run verify` 通过（107 个测试文件、842 个测试、production build、20 组对比度检查）。

## 0.46.1 — 2026-08-25

- 修复图片模式提示词增强按钮在输入提示词后仍保持禁用的问题；输入非空时局部同步按钮状态，不触发整页重渲染或影响输入焦点。
- `npm.cmd run verify` 通过。

## 0.46.0 — 2026-08-25

- 新增 OmniGen2 图片工作流：无参考图走文生图，有一到两张参考图时走参考图编辑；复用创建页的裁剪、标注、Mask、队列和历史能力，并补充模型目录、节点依赖与 Prompt 增强接入。
- 图片模型能力限制统一由创建窗口根据当前模型处理；单参考模型隐藏多余的 Slot 入口，生成数量上限统一为 6，设置页移除模型张数和运行策略说明。
- 继续拆分图片 Workflow、队列和历史渲染模块，修复比例/分辨率、提示词提示、模型切换和图片编辑控件的交互；`npm.cmd run verify` 通过。
...
- 创建页支持 Z-Image 无参考图文生图；有参考图时，Z-Image 使用单图 img2img，带 Mask 时使用原生 VAE Inpaint，Z-Image-Turbo 使用官方 Canny + Fun ControlNet 参考控制并支持 Mask。
- Z-Image 模型复用现有图片工作区的 Prompt、通用裁剪、标注画布和 Mask 工具；提示词增强新增视角变换、细节补充、材质/光照/透视和标注清理约束。
- 补充 Z-Image 工作流、队列、环境扫描、创建页预检和 Prompt 选择器测试；`npm.cmd run verify` 通过。

## 0.43.7 — 2026-08-25

- 放宽 H3 提示词扩写：350–500 词现在只是简单 R2V 任务的起始参考范围；对白密集、多镜头、复杂参考关系和更长时长可以自然生成更长 Prompt。
- 为 Native Qwen、Qwen3.6/Qwen3.8 多模态、Qwen-VL LoRA 和 LM Studio 扩写路径增加按模式与时长计算的输出余量，避免 FL2VA 等普通 H3 模式过早截断。
- 创建页右下角字数提示改为动态参考范围，不再将超过参考范围的 Prompt 标记为超限。

## 0.43.6 — 2026-08-25

- 修复队列启动时释放提示词模型后，提示词增强按钮仍显示模型驻留状态的问题；所有释放入口现在统一同步 `unloading` → `unloaded` 状态，释放异常时也会正确归位。

## 0.43.5 — 2026-08-25

- 吸收社区 H3 Prompt 编译器的内容锁思路，但不引入额外的独立 Node 运行时：对白、歌唱、旁白和画面文字现在分别建立输入账本，按邻近语义提示区分台词与招牌/字幕文字。
- 多语言对白保留原始语言、字符、空格和标点；旁白要求画面人物闭嘴，普通对白要求口型同步，歌唱保持独立的演唱语义；模型遗漏或翻译内容时由输出归一化阶段恢复。
- 将内容锁定与输出恢复接入原生 TextGenerate、多模态 Qwen3.6/Qwen3.8、H3 Prompt Writer、LM Studio、llama-server 和 Qwen-VL LoRA 路径，并补充分类与恢复测试。

## 0.43.4 — 2026-08-24

- 撤回纯文字 MiniMax H3 27B Prompt Rewriter 节点路径及其依赖，保留 Qwen3-VL 8B + H3 Prompt Rewriter LoRA、Qwen 多模态和 Gemma 提示词模型。
- 提示词模型入口继续按能力排序；已保存的 27B 纯文字 Rewriter 默认选择会自动迁移到保留的 Qwen3.6 27B 多模态模型。

## 0.43.3 — 2026-08-24

- 精简设置页 LoRA 卡片：复用统一的模型组件行，按标题、目录与文件名展示，保留状态图标、缺失提示、安装信息和精简使用说明表格。
- H3 提示词增强链路增加编译器持有的对白锁：跨 ComfyUI、Prompt Writer、llama-server、LM Studio 和 Qwen VL 路径保留用户原始语言、字符与标点，并在输出检查中提示遗漏、重复或新增对白。
- 精简提示词扩写模型入口：移除 Gemma E4B Q3、Gemma 12B Q4 和标准 Gemma 26B-A4B Q4，保留 Uncensored 26B 档；旧设置会自动迁移到对应的保留模型。
