...
- H3 视频 LoRA 任务改用本地 ComfyUI 进程隔离：提交前重启以保证 LoRA 从干净模型状态挂载，完成后在进入非 LoRA 任务或队列结束前重启；连续 LoRA 仅保留任务间一次重启，普通无 LoRA 队列仍使用原有释放路径。远程 ComfyUI 保持 connection-only，不执行进程操作。
