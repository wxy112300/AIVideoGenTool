...
let promptRuntimeState = { service: { phase: "stopped", ownership: "none", endpoint: "", message: "提示词运行时未运行", updatedAt: new Date().toISOString(), operationId: 0 }, model: { phase: "unloaded", modelId: null }, operation: { phase: "idle", operationId: null, origin: null, promptId: null, startedAt: null, retainModel: false, terminalStatus: null, terminalReason: null, finishedAt: null } };
...
const api = {
  getState: async () => clone(currentState), getComfyRuntimeState: async () => clone(runtimeState), getPromptRuntimeState: async () => clone(promptRuntimeState), getAppVersion: async () => ${JSON.stringify(packageJson.version)}, setSettingsDirty: async () => {}, respondWindowClose: async () => {},
