...
当前开发版本：**0.42.13**。本版本将队列上下移动按钮改为可视化拖拽排序，拖动时实时显示预计队列编号，并保留键盘排序备用操作；同时修复图片处理、图生视频和视频续写的 Prompt 增强运行期间删除版本后被异步状态回传恢复的问题。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
