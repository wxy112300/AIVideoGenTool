...
当前开发版本：**0.42.14**。本版本将确认、设置辅助和窗口操作弹窗移到独立 overlay 层，减少背景页面重绘并保持焦点；同时修复队列和历史删除后的滚动位置恢复。队列继续支持可视化拖拽排序、实时预计编号和键盘备用操作。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
