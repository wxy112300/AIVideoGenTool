...
当前开发版本：**0.51.1**。设置页将可安装的第三方 ComfyUI 节点与运行依赖集中到第二项“节点与依赖”；内置核心和工作流 JSON 仍由应用内部管理，ComfyUI 核心版本在“系统与路径”中检查。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
