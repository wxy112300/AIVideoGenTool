...
当前开发版本：**0.61.7**。本 patch release 将 H3 LoRA 自动触发词放入官方结构的首个 `[Shot 1]`，避免破坏参考声明与字段结构，同时保留已有 trigger 和旧自由文本 Prompt 的兼容行为；版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
