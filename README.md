...
当前开发版本：**0.39.0**。本版本完成 P17 Settings 分类与保存/扫描动作层级：窄屏使用单行可滚动分类栏，补齐键盘/ARIA 语义，并将环境扫描动作归入本机环境；同时保留既有保存、扫描、队列与持久化逻辑。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
