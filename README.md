...
当前开发版本：**0.49.3**。本版本让节点扫描能够识别 MiniMax H3 Prompt Writer 的运行接口故障并提供修复入口，同时加入状态驱动的节点安装、更新、修复、重装和可恢复卸载操作。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
