...
当前版本为 **`0.8.11`**。本版本完成了本地图片/视频工作台的一轮完整闭环：图片处理模式接入 Qwen-Image-Edit-2511，支持 Picture 1 与参考图、批量候选、固定 Seed、20/40 步及 Lightning 4 步档和目标分辨率；图片任务可在 ComfyUI 未启动时先加入队列，执行时自动启动并验证节点。图片输出固定为 PNG，并支持嵌套、Unicode 和自定义 ComfyUI `output` 目录。
