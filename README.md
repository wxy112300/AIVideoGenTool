...
当前开发版本：**0.18.3**。本阶段完成 Electron 队列模块化：入队与素材归档、任务执行、取消/恢复/自动重试、历史落盘和 worker 生命周期均已从 `main.ts` 分离，并保持现有 IPC 与持久化数据契约。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
