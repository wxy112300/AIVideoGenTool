...
当前开发版本：**0.31.5**。本版本继续修复当前 renderer 图片处理页的窄窗体验：素材区文字不再单字掉行，添加 Slot 按钮与素材卡标签不会竖向收缩或互相覆盖，底部提交条为输出控件保留安全间距；同时保留 0.31.3 的通知反馈与恢复路径。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
