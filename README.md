...
当前开发版本：**0.49.4**。本版本为带视频 LoRA 的 H3 连续队列增加两阶段运行时释放与绝对空闲显存校验，普通无 LoRA H3 队列仍保持单阶段释放。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
