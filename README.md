...
当前版本为 **`0.9.0`**。本版本为 FLUX.2 Klein 4B 增加了 20 步快速质量和 50 步高质量两个档位；同时隔离了 Qwen 图片任务与视频任务的 ComfyUI 运行配置：Qwen 的 CPU VAE/激进卸载参数不再污染 MiniMax H3 等视频工作流，队列在不同模型之间切换时会核对实际运行配置并安全重启 ComfyUI，避免 H3 FP16 VAE 在 CPU FP32 路径中发生 dtype 冲突。
