...
当前开发版本：**0.41.11**。本版本补齐 Qwen3.8 GGUF 的 vision 投影文件发现与视觉 handler 兼容，在工作流提交前验证运行节点实际接受的模型/mmproj，并修复充足显存下仍被强制使用 CPU 的启动延迟。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
