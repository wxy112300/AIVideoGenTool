...
当前开发版本：**0.29.5**。所有本地提示词后端在显式启动后都会保持模型驻留，连续扩写可复用已加载权重；手动退出、开始队列或关闭应用时统一释放。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
