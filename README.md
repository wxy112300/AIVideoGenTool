...
当前开发版本：**0.41.6**。本版本固定 Settings 页头“重新扫描全部”在右侧动作区的位置，保存/放弃更改只在左侧提交区出现，不再因保存组出现而移动扫描入口；窄屏按扫描在上、提交在下排列。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
