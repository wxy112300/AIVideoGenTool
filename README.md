...
当前开发版本：**0.42.7**。本版本修复视频续写草稿从历史或已保存状态返回 Create 时的工作流能力误判，并修正 Qwen3.8 Prompt 增强的推理前言泄漏与自动语言判断。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
