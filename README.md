...
当前开发版本：**0.43.7**。本版本放宽 H3 提示词扩写的长度策略，按模式与时长给出动态参考范围和输出预算；复杂对白、多镜头和长时长 Prompt 不再被 500 词建议过早压缩。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
