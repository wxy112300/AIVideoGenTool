...
当前开发版本：**0.18.9**。本阶段将 ComfyUI 安装发现迁入独立模块，统一处理显式目录、常见磁盘位置、Desktop 1/2 注册表、Portable/Manual 布局、多实例去重与选中版本。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
