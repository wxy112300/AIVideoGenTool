...
当前开发版本：**0.32.2**。本版本修正当前 renderer Queue 的性能总览位置，并恢复 Spectrum 默认策略：兼容节点已安装且用户没有历史选择时自动开启，手动选择会被记忆；队列快照、状态机和恢复语义保持不变。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
