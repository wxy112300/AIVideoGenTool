...
当前开发版本：**0.18.7**。本阶段完成官方工作流依赖服务拆分：目标路径、离线扫描、代理下载、临时文件校验和安全覆盖由独立模块负责，失败时保留原工作流。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
