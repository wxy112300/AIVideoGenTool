...
当前开发版本：**0.61.6**。本 patch release 重构 H3 Prompt 增强控制与参考模式语义，修复自动 LoRA 触发词重复及其抢占参考声明开头的问题；版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
