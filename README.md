...
当前开发版本：**0.18.4**。本阶段完善全局顶部通知：区分信息、警告、错误、任务完成和队列完成，跨页面保留、延长可读时间并同步写入应用日志；通知更新与页面渲染解耦，不会打断媒体播放或抢走输入焦点。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
