...
当前版本为 **`0.8.12`**。本版本隔离了 Qwen 图片任务与视频任务的 ComfyUI 运行配置：Qwen 的 CPU VAE/激进卸载参数不再污染 MiniMax H3 等视频工作流，队列在两类任务之间切换时会核对实际运行配置并安全重启 ComfyUI，避免 H3 FP16 VAE 在 CPU FP32 路径中发生 dtype 冲突。
