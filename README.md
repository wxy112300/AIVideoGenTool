...
当前开发版本：**0.29.3**。本版本修复 Windows 分发版中 ComfyUI 日志包装器接管标准输出后关闭共享底层句柄，导致采样进度条以 `I/O operation on closed file` 失败的问题。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
