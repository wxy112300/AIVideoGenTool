...
当前开发版本：**0.41.7**。本版本修正启动环境扫描期间队列页过早显示 ComfyUI 错误的问题：队列尚未运行时显示初始化状态，真实的队列生命周期错误仍保留。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
