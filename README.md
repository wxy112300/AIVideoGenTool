...
当前开发版本：**0.18.8**。本阶段开始拆分 ComfyUI 实例管理：H3/Prompt 核心节点要求、最低版本和语义版本判定已迁入独立兼容性模块，环境服务只负责组合 API 与本地源码检测结果。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
