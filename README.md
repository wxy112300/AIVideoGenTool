...
当前开发版本：**0.43.6**。本版本修复队列启动释放提示词模型后，提示词增强按钮状态未同步更新的问题；提示词模型现在会正确发布卸载状态，并继续按能力排序提示词模型入口。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
