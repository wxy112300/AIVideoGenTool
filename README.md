...
当前开发版本：**0.20.0**。本阶段稳定了队列生命周期、ComfyUI 状态与 TAE 实时预览，统一了提示词扩写运行时和 `llama-cpp-python` 修复流程，并补充了多语言设置、依赖安装反馈和模型/节点运行时校验。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
