...
当前开发版本：**0.18.10**。本阶段继续拆分 ComfyUI 运行管理：VRAM/Qwen 启动参数集中到运行策略模块，本机端点校验、后台进程启动和服务就绪等待迁入通用进程辅助模块。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
