...
当前开发版本：**0.34.0**。本版本完成 P12 History 键盘语义：卡片支持 Enter/Space，视频/图片 Tab 支持单一 Tab stop 与 Arrow/Home/End，More/Shift+F10 菜单支持键盘导航和返回焦点；保留 P11 的工具条单行与相册列轨稳定性。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
