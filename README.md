...
当前开发版本：**0.42.6**。本版本增加 ComfyUI 新版视频 API 与 PyAV 的启动兼容检测：发现官方核心仍存在已知导入缺口时，只自动修复所选 ComfyUI Python 环境中的 PyAV，并在启动前复检。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
