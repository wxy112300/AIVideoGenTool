...
当前开发版本：**0.42.0**。本版本将图生视频、视频续写和图片处理拆分为独立持久化创作空间，并让视频续写提示词增强分析裁剪区间的最后一帧；后台增强任务会持续归属于发起页面。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
