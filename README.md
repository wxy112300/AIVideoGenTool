...
当前开发版本：**0.56.5**。本 patch 优化 History 大列表的缓存、分帧媒体绑定和离屏布局成本，保持深层卡片首次交互立即可用；同时修复队列 runtime 原型方法在组合时丢失接收者、导致 H3 任务无法执行的问题。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
