...
当前开发版本：**0.25.0**。本阶段为历史作品加入收藏、1–5 分评分和组合筛选，筛选上下文会同步到详情页导航；同时补充 ComfyUI 运行日志桥接和旧历史元数据迁移。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
