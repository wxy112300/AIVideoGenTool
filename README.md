...
当前开发版本：**0.18.11**。本阶段将 ComfyUI 启动编排迁入独立服务，统一处理 Desktop 外壳启动、源码/Portable Python 启动、核心与数据目录分离以及模型、输出、数据库和运行策略参数。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
