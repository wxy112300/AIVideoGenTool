...
当前开发版本：**0.18.2**。本阶段完成项目入口文档与安装反馈校准，并开始把队列快照、纯状态变换和 mutation IPC 从 Electron `main.ts` 拆成独立模块。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
