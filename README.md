...
当前开发版本：**0.36.0**。本版本新增 ComfyUI 数据库智能修复：按锁占用、依赖缺失、路径不可用、迁移不兼容和 SQLite 损坏分类处理，修复前备份并保留恢复路径；同时保留 P13 图片媒体状态、P12 History 键盘语义和 P11 的紧凑相册列轨。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
