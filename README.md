...
当前开发版本：**0.42.4**。本版本修复 MultiModal Prompt Nodes 当前上游 Qwen3.8 识别结构被安装器误判的问题，并在每次环境扫描后记录版本快照、缺失依赖与运行时异常，方便远程定位其他电脑上的环境故障。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
