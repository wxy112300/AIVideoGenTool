...
当前开发版本：**0.18.5**。本阶段开始拆分 ComfyUI 依赖管理：节点与官方工作流声明集中到独立 catalog，节点的离线文件扫描、在线 API 验证和兼容性检查迁入专用服务；ComfyUI 未启动时仍可管理已安装依赖。版本变化见 [CHANGELOG.md](CHANGELOG.md)。项目仍在 `0.x` 阶段，优先支持 Windows、NVIDIA GPU 和本地 ComfyUI。
