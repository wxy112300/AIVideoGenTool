...
当前版本为 **`0.10.3`**。箭头工具在按下并拖动时实时显示箭身和箭头，不再先显示矩形占位；文字标记移除大块黑色背景，改用透明背景和细描边，减少对原图的遮挡。Electron 视觉回归测试新增拖动中箭头的像素检查。

上一版本为全屏视觉标记画布增加了以鼠标位置为中心的滚轮缩放，以及选择工具下直接拖动画面平移；中键拖动和 `Space + 左键拖动` 可在其他工具下临时平移。

上一版本修复了 Fabric 7 默认使用中心原点、背景图却被定位到画布 `(0,0)`，导致只能看到图片右下四分之一的问题；背景图和区域标记现在明确使用左上原点。

上一版本在图片处理工作区新增全屏视觉标记画布：可以用画笔、高亮、矩形、椭圆、箭头和文字指出具体修改位置，并为每处标记补充说明。原图保持不变，应用保存可继续编辑的画布工程和供 Qwen 读取的标注预览；入队时标注图仍占用原 Picture 槽位，并自动要求模型在结果中移除框线、箭头、编号和文字。

上一版本将顶部悬浮提示从整页渲染中解耦；通知出现和消失只更新提示层，不再中断视频播放、重置页面滚动或抢走 Prompt 输入框焦点。

上一版本收紧了 ComfyUI 数据库修复提示：只在服务不可连接、最近一次启动日志包含尚未恢复的数据库初始化错误时出现；正常服务、历史日志及错误后已成功启动的情况不再误报。

上一版本在设置页明确区分了 ComfyUI 的安装入口、核心目录与数据 / 节点目录；Comfy Desktop 将核心代码和用户数据分开存放时，模型、节点、输入输出与更新目标不再混淆。扫描、安装与启动逻辑保持不变。

上一版本完善了项目开发 Harness：根 `AGENTS.md` 明确需求优先级、核心产品不变量、修改影响面和分级验证规则，并新增架构、UX 及 ComfyUI 模型工作流契约，减少跨页面、跨模型和持久化路径上的回归。仓库同时提供统一的 `npm.cmd run verify` 验证入口和原型构建命令。

上一版本为 FLUX.2 Klein 4B 增加了 20 步快速质量和 50 步高质量两个档位；同时隔离了 Qwen 图片任务与视频任务的 ComfyUI 运行配置：Qwen 的 CPU VAE/激进卸载参数不再污染 MiniMax H3 等视频工作流，队列在不同模型之间切换时会核对实际运行配置并安全重启 ComfyUI，避免 H3 FP16 VAE 在 CPU FP32 路径中发生 dtype 冲突。

图片历史现在独立为图片工作区，保留项目的全部编辑版本，支持继续编辑、开始图生视频、版本缩略图浏览、上一/下一版本切换和大图查看。入队按钮会即时显示提交状态并阻止重复点击，生成历史会保存运行耗时和性能统计；视频工作流继续支持 MiniMax H3、R2V 多参考和 Spectrum 加速。
