...
function firstPassResolutionFor(draft: Draft): GenerationQueueTask["resolution"] {
...
export function imageTaskFromDraft(
  ...
  const targetResolution = normalizeImageTargetResolution(
    isH3Image ? "source" : draft.targetResolution,
    basePicture?.width ?? 0,
...
export function upscaleTaskFromRequest(
  ...
  if (hasKonohamaruFields) {
    ...
    return {
      ...
      createdAt: now,
      updatedAt: now,
      outputFilename: uniqueKonohamaruUpscaleFilename(
