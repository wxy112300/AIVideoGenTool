...
function isInsideQuotedText(source, start) {
    const closingStack = [];
    for (let index = 0; index < start; index += 1) {
        const character = source[index] ?? "";
        if (character === "\"" && source[index - 1] !== "\\") {
            if (closingStack.at(-1) === "\"")
                closingStack.pop();
            else
                closingStack.push("\"");
            continue;
...
function findDelimitedEnd(source, start) {
    let depth = 0;
...
function stripAnnotationSeparator(value) {
    return value
        .replace(/^\s*[:：]\s*/u, "")
        .replace(/^\s*[-–—]\s*/u, "")
        .trim();
}
function annotationTextFromBody(body) {
    ...
    if (isSingleChineseLabel && remainder && !startsWithSeparator && !chineseInstructionStartPattern.test(remainder.trim()))
        return undefined;
    // English labels must have a boundary after the label; otherwise words such
    // as “noteworthy” would be parsed as an annotation.
    if (isEnglishLabel && remainder && !startsWithSeparator)
        return undefined;
    if (!remainder.trim())
        return undefined;
    ...
    if (!note)
        return undefined;
    if (!isChineseLabel && !startsWithSeparator)
        return undefined;
    return note;
}
function anchorFor(source, start, previousRanges) {
    let before = "";
    let cursor = 0;
    ...
    for (let index = 0; index < before.length; index += 1) {
        if (clauseBoundaryPattern.test(before[index] ?? ""))
            boundary = index + 1;
    }
    const clause = before.slice(boundary).replace(/\s+/gu, " ").trim();
    if (!clause)
        return "the beginning of the draft";
    return clause.slice(-180);
}
function needsSeparator(before, after) {
    if (!before || !after || /\s/u.test(before) || /\s/u.test(after))
        return false;
    if (wordLikePattern.test(before) && openingDelimiters.has(after))
        return true;
    if (closingDelimiters.has(before) && wordLikePattern.test(after))
        return true;
    if (!wordLikePattern.test(before) || !wordLikePattern.test(after))
        return false;
    return !cjkPattern.test(before) && !cjkPattern.test(after);
}
export function parsePromptAnnotations(sourcePrompt) {
    const source = sourcePrompt ?? "";
    const annotations = [];
    const ranges = [];
    for (let index = 0; index < source.length; index += 1) {
        if (!openingDelimiters.has(source[index]))
            continue;
        if (isInsideQuotedText(source, index))
            continue;
    ...
    if (!ranges.length)
        return { prompt: source, annotations };
    let prompt = "";
    let cursor = 0;
    for (const range of ranges) {
        ...
        if (punctuationAfterPattern.test(after ?? ""))
            left = left.replace(/\s+$/u, "");
        prompt += /\s/u.test(before ?? "") && /\s/u.test(after ?? "")
            ? left.replace(/\s$/u, "")
            : left;
        if (needsSeparator(before, after))
            prompt += " ";
        cursor = range.end;
    }
...
export function stripPromptAnnotations(sourcePrompt) {
    return parsePromptAnnotations(sourcePrompt).prompt;
}
export function promptAnnotationInstruction(parsed) {
