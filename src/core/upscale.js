...
import { KONOHAMARU_MODEL_ID, buildKonohamaruUpscaleWorkflow, isKonohamaruMode, konohamaruModeSpec } from "./konohamaru-dlss5.js";
...
export function estimateUpscaleResources(input) {
    const profile = upscaleEstimateProfiles[input.modelId];
    const safeSourceShortEdge = Math.max(1, Math.min(Math.max(1, input.sourceWidth), Math.max(1, input.sourceHeight)));
    const targetShortEdge = Math.min(Math.max(1, input.targetWidth), Math.max(1, input.targetHeight));
    ...
    const frameCount = Math.max(1, Math.ceil(Math.max(0, input.duration) * Math.max(1, input.fps)));
    const internalScale = input.modelId === "flashvsr" &&
    ...
    const internalScaleTime = input.modelId === "flashvsr" && internalScale === 4
        ? 1.18
        : 1;
    const vramMinGb = roundHalf(profile.baseVramMinGb +
...
function seedVr2AdaptiveMemoryBudgetBytes(tileMode, resources) {
    if (!resources || resources.systemMemoryAvailableBytes <= 0) {
        return seedVr2SegmentMemoryBudgetBytes(tileMode);
    }
    const reserveBytes = Math.max(8 * 1024 ** 3, resources.systemMemoryTotalBytes * 0.15);
    const availableAfterReserve = Math.max(1 * 1024 ** 3, resources.systemMemoryAvailableBytes - reserveBytes);
    ...
    return Math.max(1 * 1024 ** 3, availableAfterReserve * modeShare / 2.75);
}
function seedVr2VramFrameLimit(targetPixels, resources) {
    ...
    const latentFrames = Math.max(1, Math.floor(Math.max(0.55, availableGib - reservedGib) / (0.55 * megapixels)));
...
function seedVr2CompatibleFrameCount(value) {
    const bounded = Math.max(5, Math.floor(value));
    return Math.max(5, Math.floor((bounded - 1) / 4) * 4 + 1);
}
export function nativeSeedVr2SegmentPlan(task, resources, preferredFramesPerSegment) {
    ...
    const [targetWidth, targetHeight] = upscaleDimensions(task.sourceWidth, task.sourceHeight, task.targetHeight);
    ...
    const memoryFrameLimit = seedVr2CompatibleFrameCount(Math.floor(preprocessingBudgetBytes / Math.max(1, bytesPerFloatRgbFrame)));
    const vramFrameLimit = seedVr2VramFrameLimit(targetWidth * targetHeight, resources);
    const framesPerSegment = seedVr2CompatibleFrameCount(preferredFramesPerSegment ?? memoryFrameLimit);
...
export function createUpscaleFilename(sourceFilename, targetHeight) {
    ...
    const metadata = stem.match(/^(.*?)-(?:\d+p|4K)-(\d+(?:\.\d+)?s)-(\d{8}-\d{6})(?:-v\d+)?$/i);
    const base = metadata
        ? `${metadata[1]}-${suffix}-${metadata[2]}-${metadata[3]}`
        : `${stem}-${suffix}`;
    return `${base}-v01.mp4`;
}
export function uniqueUpscaleFilename(sourceFilename, targetHeight, existingNames) {
...
function konohamaruModeLabel(mode) {
    if (!isKonohamaruMode(mode))
        throw new Error("Konohamaru filename mode is invalid");
...
export function createKonohamaruUpscaleFilename(sourceFilename, mode, frameOutputFps = "off") {
...
export function uniqueKonohamaruUpscaleFilename(sourceFilename, mode, existingNames, frameOutputFps = "off") {
...
function loadVideoNode(sourceVideo, useExternalBatching) {
    return {
...
function exactScaleNode(image, task) {
    const [targetWidth, targetHeight] = upscaleDimensions(task.sourceWidth, task.sourceHeight, task.targetHeight);
    return {
...
function combineVideoNode(task, useExternalBatching) {
    return {
...
function renderNativeSeedVr2Workflow(task, sourceVideo, segment) {
    const [targetWidth, targetHeight] = upscaleDimensions(task.sourceWidth, task.sourceHeight, task.targetHeight);
    return {
        "1": {
...
export function renderUpscaleWorkflow(task, sourceVideo, models, objectInfo, nativeSeedVr2Segment) {
    ...
    if (missingSeedVr2Nodes.length) {
        const nodeLabel = task.modelId === "seedvr2-native-int8"
            ? "原生 SeedVR2 节点"
            : "SeedVR2 节点模块";
        throw new Error(`${nodeLabel}版本过旧或尚未加载，缺少：${missingSeedVr2Nodes.join(", ")}。` +
