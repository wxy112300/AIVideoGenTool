...
const qwenImage21PromptPresetDefaults: Record<ImagePromptPreset, string> = {
...
export function createDefaultQwenImage21PromptPresets(): Record<ImagePromptPreset, string> {
  return { ...qwenImage21PromptPresetDefaults };
}

export function normalizeQwenImage21PromptPresets(value: unknown): Record<ImagePromptPreset, string> {
  const defaults = createDefaultQwenImage21PromptPresets();
  const source = value && typeof value === "object"
    ? value as Partial<Record<ImagePromptPreset, unknown>>
    : {};
  return {
...
export function qwenImage21PromptContract(
  ...
  return [
    ...
    outputMode === "writer"
      ? "Produce exactly one concise, direct English paragraph inside the detailed_description field for the image model."
...
export function qwenImage21PromptUserContent(request: EnhanceRequest): string {
  ...
  return [
    hasReferences
      ? "Rewrite the user's image-edit instruction according to the Qwen Image 2.1 edit contract."
      : "Rewrite the user's text-to-image instruction according to the Qwen Image 2.1 T2I contract. No reference image is supplied.",
    ...(referenceContext ? [`Reference map:\n${referenceContext}`] : []),
    hasReferences
      ? "Use <image1>, <image2>, and so on for supplied images in input order."
      : "Do not add <image1>, <image2>, or any other reference-image token.",
    "User's original instruction:",
    request.prompt.trim(),
    hasReferences
      ? "Output only the final English image-edit prompt. Preserve proper nouns and visible text in their original wording."
      : "Output only the final English image-generation prompt. Preserve proper nouns and visible text in their original wording."
  ].join("\n\n");
}

export function normalizeQwenImage21PromptOutput(value: string): string {
  const fenced = value
