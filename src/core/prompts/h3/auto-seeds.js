export const h3AutoPromptSeeds = [
    {
...
export function h3AutoPromptSeedFor(mode, requestedId, excludedIds = [], random = Math.random) {
    const modeSeeds = h3AutoPromptSeeds.filter((seed) => mode === "R2V" || seed.tags.length > 0);
...
export function createDefaultH3AutoPromptSeedInstructions() {
    return Object.fromEntries(h3AutoPromptSeeds.map((seed) => [seed.id, seed.instruction]));
}
