export const defaultH3PromptPresets = {
    "official-storyboard": [
    ...
    "detailed-cinematic": [
        "Create a high-detail, production-ready H3 prompt from the user's original idea and supplied reference media. First understand and preserve the intended subjects, relationships, setting, action, style, dialogue, visible text, timing, and restrictions; do not replace the idea with a different story.",
        "Use reference images as visual evidence for identity, pose, composition, wardrobe, materials, lighting, and spatial anchors that are actually visible. For I2VA, develop forward from the first frame; for FL2VA, connect the two exact endpoints; for L2VA, converge on the final frame; for R2V, keep every supplied Picture, Video, Audio, and Subject role exact.",
        "Build a complete causal timeline: opening state, intention and preparation, primary action, contact or force, weight and momentum, reaction and secondary motion, motivated camera response, material or environmental response, and final state. For every necessary beat, specify one dominant action, the camera behavior, the visible change, the synchronized audible event, and the state that ends the beat. Prefer one continuous shot unless the user or the reference structure requires a cut.",
        "When the original prompt leaves operational gaps, add only reasonable scene-grounded details that make the requested result executable: ordinary micro-actions, gaze and expression, body mechanics, hand-object contact, cloth or hair movement, inertia, focus or exposure changes, motivated lighting changes, ambience, and synchronized sound. Do not invent new characters, locations, important props, plot events, visible text, dialogue, music, or major appearance changes, and never override an explicit constraint.",
        "Make camera choices motivated and concrete: establish framing or viewpoint, the primary movement's direction, speed or amplitude, focus behavior, and its relationship to the action. Use one primary camera move per beat; do not stack unrelated pans, zooms, or orbits merely to sound cinematic.",
        "Use the official H3 fields and exact field order for the selected mode. Keep explanatory prose in English when required by the H3 contract, but preserve dialogue, lyrics, voiceover words, and visible text exactly in the user's original language. There is no fixed 350-500 or 500-word target for this preset: expand only while each sentence controls a visible, audible, temporal, or continuity-relevant result, and remove repetition before shortening a necessary detail. Return only the final H3 prompt."
    ].join("\n"),
    "reference-faithful": [
