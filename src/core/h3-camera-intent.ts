...
function cameraFallbackSentence(intent: H3CameraIntent): string {
  const motionLabels: Record<H3CameraMotionKind, string> = {
    arc: "Arc Shot",
    pan: "pan",
    tilt: "tilt",
    push: "push-in",
  ...
  if (/(?:inside|indoors|interior|within|里面|内部|内侧|室内)/iu.test(source)) spatial.push("beginning inside the established space");
  if (/(?:outside|outdoors|exterior|outward|外面|外部|外侧|向外)/iu.test(source)) spatial.push("oriented toward the exterior");
  if (/(?:through|穿过|透过)/iu.test(source)) spatial.push("passing through the visible opening");
  if (/(?:behind|后方|后面)/iu.test(source)) spatial.push("maintaining the requested rear relation");
  if (/(?:above|上方)/iu.test(source)) spatial.push("maintaining the requested position above the target");
  if (/(?:below|under|下方)/iu.test(source)) spatial.push("maintaining the requested position below the target");
  if (/(?:\bleft\b|左侧)/iu.test(source)) spatial.push("maintaining the requested left-side relation");
  if (/(?:\bright\b|右侧)/iu.test(source)) spatial.push("maintaining the requested right-side relation");
  if (/(?:\bclockwise\b|顺时针)/iu.test(source)) spatial.push("moving clockwise");
  if (/(?:\b(?:counterclockwise|anticlockwise)\b|逆时针)/iu.test(source)) spatial.push("moving counterclockwise");
