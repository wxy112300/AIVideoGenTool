import { h3Fl2vaImageDiffusionModel, h3Fl2vaTurbo8Lora, h3ImageTextEncoder, h3ImageVideoVae, h3Ref2vaImageDiffusionModel, h3Ref2vaTurbo8Lora, minimaxH3ImageI2ICapability, minimaxH3ReferenceEditCapability } from "./capabilities.js";
import { minimaxH3ImageFl2vaNodeTypes, minimaxH3ImageI2IRequiredNodeTypes, minimaxH3ImageRef2vaNodeTypes, minimaxH3ImageStudioSharedNodeTypes, minimaxH3ImageTurboNodeTypes, minimaxH3ReferenceEditRequiredNodeTypes } from "./node-requirements.js";
import { imageReferenceInputPath, orderedPictures, pictureReferencePattern } from "./shared.js";
const H3_IMAGE_QUALITY_PROFILE = "recommended | 5 frames";
export const H3_IMAGE_RESOLUTION_PROFILE = "native-detail-0.98mp";
export const H3_IMAGE_RESOLUTION_PROFILE_LABEL = "native detail | 0.98 MP";
const H3_IMAGE_RESOLUTION_AREA = 0.98 * 1024 * 1024;
const H3_IMAGE_DIMENSION_ALIGNMENT = 32;
const H3_IMAGE_SOURCE_FIT_VALUES = ["crop_center", "contain_pad", "stretch"];
const H3_IMAGE_REFERENCE_DETAIL_VALUES = ["match_generation_area", "max_identity_2048"];
...
function isH3ImageModelId(modelId) {
    return modelId in h3ImageCapabilities;
}
function h3ImageOptionsFor(modelId, options) {
    return {
...
function defaultDiffusionModelFor(modelId) {
    return modelId === "minimax-h3-image-i2i" ? h3Fl2vaImageDiffusionModel : h3Ref2vaImageDiffusionModel;
}
function h3ImageRecipeContractFor(modelId, qualityProfile) {
...
function h3ImageRecipeIdentityFor(modelId, qualityProfile) {
    const contract = h3ImageRecipeContractFor(modelId, qualityProfile);
    if (!contract || !isH3ImageModelId(modelId))
        return undefined;
...
export function h3ImageRecipeFor(modelId, qualityProfile, diffusionModelFilename) {
...
export function validateH3ImageRecipeSnapshot(modelId, qualityProfile, recipe, taskDiffusionModelFilename) {
    const contract = h3ImageRecipeContractFor(modelId, qualityProfile);
    if (!contract)
        return [`H3 图片质量档 ${qualityProfile} 与路线 ${modelId} 不匹配。`];
    if (!recipe)
        return ["H3 图片队列任务缺少冻结 recipe；只有旧任务才允许走 legacy fallback。"];
    const errors = [];
...
export function h3ImageOutputDimensions(sourceWidth, sourceHeight) {
    if (!(Number.isFinite(sourceWidth) && Number.isFinite(sourceHeight) && sourceWidth > 0 && sourceHeight > 0))
        return [0, 0];
    const ratio = sourceWidth / sourceHeight;
    const align = (value) => Math.max(H3_IMAGE_DIMENSION_ALIGNMENT, Math.round(value / H3_IMAGE_DIMENSION_ALIGNMENT) * H3_IMAGE_DIMENSION_ALIGNMENT);
    return [
        align(Math.sqrt(H3_IMAGE_RESOLUTION_AREA * ratio)),
        align(Math.sqrt(H3_IMAGE_RESOLUTION_AREA / ratio))
    ];
}
function roleLabel(picture) {
    switch (picture.role) {
...
function h3ReferenceMapping(pictures, runtimePictureNumberMap) {
...
function hasImageGuidanceData(picture) {
    return Boolean(picture.mask?.regionCount ||
        picture.markup?.objectCount ||
        picture.mask?.maskPath.trim() ||
        picture.markup?.renderedPath.trim());
}
function compileH3Prompt(modelId, prompt, pictures) {
    ...
    if (modelId === "minimax-h3-image-i2i") {
        if (ordered.length !== 1)
            errors.push("H3 FL2VA 源图 I2I 必须恰好包含一张 Picture 1。");
        if (ordered[0]?.pictureNumber !== 1)
            errors.push("H3 FL2VA 源图必须是 Picture 1。");
    }
    else {
        if (ordered.length < 1 || ordered.length > 9)
            errors.push("H3 REF2VA 参考编辑支持 1–9 张 Picture。");
        if (ordered[0]?.pictureNumber !== 1)
            errors.push("H3 REF2VA 必须使用 Picture 1 作为基准图。");
    ...
    const runtimePictureNumberFor = new Map(runtimePictureNumberMap.map((mapping) => [mapping.visiblePictureNumber, mapping.runtimePictureNumber]));
    const referencedPictureNumbers = new Set();
    const runtimeReferencedPictureNumbers = new Set();
 ...
 */
export function applyH3ImageVramCleanup(workflow, objectInfo) {
    if (!Object.prototype.hasOwnProperty.call(objectInfo, "VRAM_Debug"))
        return false;
    ...
    if (!Array.isArray(samples) || typeof samples[0] !== "string")
        return false;
    const upstream = workflow[samples[0]];
    if (upstream?.class_type === "VRAM_Debug" && upstream.inputs.unload_all_models === true)
        return false;
    const baseId = "vram_cleanup";
    let cleanupId = baseId;
    let suffix = 2;
    while (workflow[cleanupId])
        cleanupId = `${baseId}_${suffix++}`;
    workflow[cleanupId] = {
...
function buildH3ImageWorkflow(modelId, task, run) {
    ...
    const recipeErrors = validateH3ImageRecipeSnapshot(modelId, task.qualityProfile, hasFrozenRecipe ? task.h3ImageRecipe : undefined, task.diffusionModelFilename);
    if (hasFrozenRecipe && recipeErrors.length)
        throw new Error(`H3 图片冻结 recipe 校验失败：${recipeErrors.join(" ")}`);
    if (!recipe)
        throw new Error(`H3 图片质量档 ${task.qualityProfile} 未登记，请重新选择 Base 或该路线的 Turbo 质量档。`);
    const modelFilename = recipe.diffusionModelFilename?.trim();
    if (!modelFilename)
        throw new Error("H3 图片 recipe 缺少冻结 diffusion checkpoint 文件名。 ");
    const workflow = {
        model: {
    ...
    const modelForSampling = recipe.adapter === "base"
        ? ["model", 0]
        : ["turbo", 0];
...
function unresolvedImagePlaceholders(workflow) {
    return Object.values(workflow).flatMap((node) => Object.values(node.inputs).filter((value) => typeof value === "string" && /^\{\{IMAGE_\d+\}\}$/u.test(value))).map(String);
}
function validateH3ImageWorkflow(modelId, workflow, qualityProfile = "base-quality-20", allowImagePlaceholders = false, frozenRecipe) {
    ...
    const prepare = Object.values(workflow).find((node) => node.class_type === (modelId === "minimax-h3-image-i2i" ? "H3ImageToImagePrepare" : "H3ReferenceEditPrepare"));
    if (!prepare)
        errors.push("H3 图片工作流缺少路线专属 Prepare 节点。");
    if (prepare?.inputs.quality_profile !== H3_IMAGE_QUALITY_PROFILE) {
        errors.push("H3 图片首版必须固定为 recommended | 5 frames。 ");
    }
    if (prepare && !H3_IMAGE_SOURCE_FIT_VALUES.includes(String(prepare.inputs.source_fit))) {
    ...
    if (modelId === "minimax-h3-reference-edit" && prepare &&
        !H3_IMAGE_REFERENCE_DETAIL_VALUES.includes(String(prepare.inputs.reference_detail))) {
    ...
    if (selector.length !== 1)
        errors.push("H3 图片工作流必须包含一个 H3ImageFrameSelector。");
    if (saves.length !== 1)
        errors.push("H3 图片工作流必须包含一个 SaveImage 输出。");
    ...
    const expectedRuntimePictureNumbers = Array.from({ length: loadImages.length }, (_, index) => index + 1);
    ...
    if (modelId === "minimax-h3-reference-edit" && prepare) {
        ...
        const expectedReferenceSockets = expectedRuntimePictureNumbers.slice(1).map((runtimePictureNumber) => `reference_image_${runtimePictureNumber}`);
...
function schemaInputGroups(value) {
...
function enumValues(value) {
    ...
    if (Array.isArray(value[0])) {
        const values = value[0].filter((item) => typeof item === "string");
        return values.length ? values : undefined;
    }
    const values = value.filter((item) => typeof item === "string");
    return values.length === value.length ? values : undefined;
}
const h3RequiredSockets = {
...
function validateH3RuntimeSchema(workflow, objectInfo) {
    const errors = [];
...
export function buildMinimaxH3ImageI2IWorkflow(task, run) {
    return buildH3ImageWorkflow("minimax-h3-image-i2i", task, run);
}
export function buildMinimaxH3ReferenceEditWorkflow(task, run) {
    return buildH3ImageWorkflow("minimax-h3-reference-edit", task, run);
}
export function validateMinimaxH3ImageI2IWorkflow(workflow, qualityProfile = "base-quality-20", allowImagePlaceholders = false, frozenRecipe) {
    return validateH3ImageWorkflow("minimax-h3-image-i2i", workflow, qualityProfile, allowImagePlaceholders, frozenRecipe);
}
export function validateMinimaxH3ReferenceEditWorkflow(workflow, qualityProfile = "base-quality-20", allowImagePlaceholders = false, frozenRecipe) {
    return validateH3ImageWorkflow("minimax-h3-reference-edit", workflow, qualityProfile, allowImagePlaceholders, frozenRecipe);
}
export function validateMinimaxH3ImageRuntimeSchema(workflow, objectInfo) {
