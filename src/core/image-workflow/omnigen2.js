import { omnigen2Capability, omnigen2DiffusionModel, omnigen2TextEncoder, omnigen2Vae } from "./capabilities.js";
import { exactImageDimension, orderedPictures, pictureReferencePattern, unresolvedImagePlaceholders } from "./shared.js";
const omnigen2DefaultNegativePrompt = "blurry, low quality, distorted, ugly, bad anatomy, deformed, poorly drawn";
...
function omnigen2PictureWithMarkupGuide(picture, pictureNumber) {
    return {
        ...picture,
...
/** OmniGen2 accepts up to two visual inputs. */
export function compileOmniGen2Prompt(prompt, pictures) {
    ...
    for (const picture of ordered) {
        ...
        const pictureHasMarkup = Boolean(picture.markup?.objectCount && picture.markup.renderedPath.trim());
        if (pictureHasMask) {
            hasMask = true;
            if (compiledNumber !== 1) {
        ...
        if (pictureHasMarkup)
            hasMarkup = true;
        compiledPictures.push(pictureHasMarkup && !pictureHasMask
    ...
    if (hasMarkup || hasMask)
        promptParts.push(omnigen2MarkupContract);
    const marked = ordered.filter((picture) => picture.markup?.objectCount);
    if (marked.length) {
        promptParts.push(marked
...
export function validateOmniGen2Workflow(workflow, _qualityProfile = "native", allowImagePlaceholders = false) {
    ...
    const scaledInputNodes = Object.values(workflow).filter((node) => node.class_type === "ImageScaleToTotalPixels");
    const encodedReferenceNodes = Object.values(workflow).filter((node) => node.class_type === "VAEEncode");
    const referenceLatentNodes = Object.values(workflow).filter((node) => node.class_type === "ReferenceLatent");
    ...
    if (maskNodes.length > 1)
        errors.push("OmniGen2 工作流最多包含 1 个 LoadImageMask 节点。");
    if (compositeNodes.length > 1)
        errors.push("OmniGen2 工作流最多包含 1 个 ImageCompositeMasked 节点。");
...
function omnigen2QualityProfile(requestedId) {
    return omnigen2Capability.qualityProfiles.find((profile) => profile.id === requestedId) ?? omnigen2Capability.qualityProfiles[0];
}
function omnigen2OutputDimensions(task, picture) {
    return [
...
export function buildOmniGen2Workflow(task, run) {
    ...
    const workflow = {
        ...
        clip: {
            ...
            inputs: { clip_name: omnigen2TextEncoder, type: "omnigen2", device: "default" }
        },
        vae: {
        ...
        scheduler: {
            ...
            inputs: { model: ["model", 0], scheduler: "simple", steps: quality.steps, denoise: 1 }
        },
        guider: {
        ...
        exactSize: {
            ...
            inputs: { image: ["decoded", 0], upscale_method: "lanczos", width: outputWidth, height: outputHeight, crop: "disabled" }
        },
        save: {
            class_type: "SaveImage",
            inputs: { images: ["exactSize", 0], filename_prefix: omnigen2OutputPrefix(task, run) }
        }
    };
    if (compiled.pictures.length) {
        ...
        compiled.pictures.forEach((_reference, index) => {
            ...
            workflow[inputKey] = { class_type: "LoadImage", inputs: { image: `{{IMAGE_${index}}}` } };
            workflow[scaledKey] = {
                class_type: "ImageScaleToTotalPixels",
                inputs: { image: [inputKey, 0], upscale_method: "area", megapixels: 1, resolution_steps: 1 }
            };
            workflow[encodedKey] = {
        ...
        workflow.imageSize = { class_type: "GetImageSize", inputs: { image: ["scaledImage1", 0] } };
    ...
    if (hasMask) {
        workflow.mask = { class_type: "LoadImageMask", inputs: { image: "{{MASK_0}}", channel: "red" } };
        workflow.sourceImage = {
            class_type: "ImageScale",
            inputs: { image: ["input1", 0], upscale_method: "lanczos", width: outputWidth, height: outputHeight, crop: "disabled" }
        };
        workflow.composite = {
            class_type: "ImageCompositeMasked",
            inputs: { destination: ["sourceImage", 0], source: ["exactSize", 0], x: 0, y: 0, resize_source: true, mask: ["mask", 0] }
        };
        workflow.save.inputs.images = ["composite", 0];
    }
