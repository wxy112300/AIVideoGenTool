...
export function compileQwenImage21Prompt(prompt, pictures) {
    const compiled = compileImagePromptWithLimit(prompt, pictures, qwenImage21Capability.maxPictures, "Qwen Image 2.1", true);
    return {
        ...compiled,
        prompt: officialQwenImage21Prompt(compiled.prompt)
    };
}
function qwenImage21LatentDimension(value, fallback, multiple = 8) {
...
function qwenImage21ReferenceResolution(width, height) {
    if (!(width > 0 && height > 0))
        return 0;
...
const officialQwenImage21WorkflowOptions = {
...
export function validateQwenImage21Workflow(workflow, qualityProfile = "preview-25", allowImagePlaceholders = false) {
    return validateQwenImage21WorkflowForOptions(workflow, qualityProfile, allowImagePlaceholders, officialQwenImage21WorkflowOptions);
}
...
function dynamicComboNestedInputSpec(value, selectedOption, inputName) {
    if (!Array.isArray(value) || typeof value[1] !== "object" || value[1] === null || Array.isArray(value[1])) {
        return undefined;
    }
    const options = value[1].options;
    if (!Array.isArray(options))
        return undefined;
    const option = options.find((candidate) => candidate && typeof candidate === "object" && !Array.isArray(candidate) && String(candidate.key) === String(selectedOption));
...
function runtimeInputSpec(groups, inputName, selectedParentValue) {
...
const qwenImage21RequiredSockets = {
...
const qwenImage21GgufRequiredSockets = {
...
function runtimeEnumValues(value) {
    ...
    const directValues = value.filter((item) => typeof item === "string");
    if (directValues.length === value.length)
        return directValues.length ? directValues : undefined;
    if (!value[1] || typeof value[1] !== "object" || Array.isArray(value[1]))
        return undefined;
    const options = value[1].options;
    if (!Array.isArray(options))
        return undefined;
    const directOptions = options.filter((option) => typeof option === "string");
    if (directOptions.length === options.length && directOptions.length > 0)
        return directOptions;
    const keys = options
        .filter((option) => option !== null && typeof option === "object" && !Array.isArray(option) && typeof option.key === "string")
        .map((option) => option.key);
    return keys.length === options.length && keys.length ? keys : undefined;
}
function isDynamicComboSpec(value) {
    return Array.isArray(value) && value[0] === "COMFY_DYNAMICCOMBO_V3";
}
function runtimeSupportsInput(groups, inputName, nodeInputs) {
    ...
    if (baseSpec === undefined)
        return false;
    if (runtimeInputSpec(groups, inputName, nodeInputs[baseName]) !== undefined)
        return true;
    return !isDynamicComboSpec(baseSpec);
}
function validateQwenImage21RuntimeSchemaForOptions(workflow, objectInfo, options) {
    const errors = [];
...
export function validateQwenImage21RuntimeSchema(workflow, objectInfo) {
    return validateQwenImage21RuntimeSchemaForOptions(workflow, objectInfo, officialQwenImage21WorkflowOptions);
}
export function validateQwenImage21GgufRuntimeSchema(workflow, objectInfo) {
...
function buildQwenImage21WorkflowForOptions(task, run, options) {
    ...
    const referenceInputs = Object.fromEntries(compiled.pictures.map((picture, index) => [
    ...
    const customSize = hasPictures && ((task.aspectRatio ?? "source") !== "source" || (task.targetResolution ?? "source") !== "source");
    const outputPrefix = [
    ...
    const workflow = {
        ...
        vae: { class_type: "VAELoader", inputs: { vae_name: qwenImage21Vae } },
        positive: { class_type: "TextEncodeQwenImage21", inputs: positiveInputs },
        empty: {
            class_type: "EmptyLatentImage",
            inputs: { width: qwenImage21LatentDimension(outputWidth, hasPictures ? sourceWidth : textOnlyWidth, hasPictures ? 32 : 8), height: qwenImage21LatentDimension(outputHeight, hasPictures ? sourceHeight : textOnlyHeight, hasPictures ? 32 : 8), batch_size: 1 }
        },
        ...(hasPictures ? {
            sizeSwitch: {
                class_type: "ComfySwitchNode",
                inputs: { on_false: ["positive", 2], on_true: ["empty", 0], switch: customSize }
            },
            cache: {
...
export function buildQwenImage21Workflow(task, run) {
