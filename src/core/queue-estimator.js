...
function sampleSeconds(performanceDuration, startedAt, completedAt) {
    return validSeconds(performanceDuration) ?? durationBetween(startedAt, completedAt);
}
function loraSignature(loras) {
    return (loras ?? [])
        .map((lora) => {
...
function numericFeature(features, key, fallback) {
    const value = features[key];
    return typeof value === "number" && Number.isFinite(value) && value > 0 ? value : fallback;
}
function interpolationCost(value) {
    if (value === "rife4x")
        return 1.3;
    if (value === "rife2x")
        return 1.15;
...
function videoWorkload(features) {
    ...
    return Math.max(0.001, (resolution / 480) ** 2
...
function imageWorkload(features) {
...
function upscaleWorkload(features) {
...
function scaledSampleSeconds(sample, task) {
...
function videoSample(asset, version, seconds) {
    const taskType = version.kind === "upscale"
        ? "upscale"
...
function imageSample(version, seconds) {
...
function videoHistorySamples(history) {
    const samples = [];
    for (const asset of history) {
        for (const version of asset.versions) {
            const seconds = sampleSeconds(version.performanceStats?.durationSeconds, version.startedAt ?? asset.startedAt, version.createdAt || asset.updatedAt || asset.createdAt);
            if (seconds != null)
                samples.push(videoSample(asset, version, seconds));
        }
    }
    return samples;
}
function imageHistorySamples(history) {
    const samples = [];
    for (const project of history) {
        for (const version of project.versions) {
            ...
            if (version.kind === "source")
                continue;
            const seconds = sampleSeconds(version.performanceStats?.durationSeconds, version.startedAt, version.createdAt);
            if (seconds != null)
                samples.push(imageSample(version, seconds));
        }
    }
    return samples;
}
function numericDistance(left, right) {
...
function featureDistance(task, sample) {
    if (task.taskType !== sample.taskType)
    ...
    if (task.modelId !== sample.modelId) {
        ...
        if (taskModel.family !== sample.modelFamily)
            return Number.POSITIVE_INFINITY;
    }
    const features = sample.features;
    let distance = task.modelId === sample.modelId ? 0 : 3;
    if (taskModel.variant && sample.modelVariant && taskModel.variant !== sample.modelVariant)
        distance += 1;
    const compareNumber = (key, value) => {
        distance += numericDistance(value, typeof features[key] === "number" ? features[key] : undefined);
    };
    const compareText = (key, value) => {
        const historical = features[key];
        if (value && typeof historical === "string" && value !== historical)
            distance += 1.5;
    };
    const compareBoolean = (key, value) => {
        const historical = features[key];
        if (value != null && typeof historical === "boolean" && value !== historical)
            distance += 1;
    };
    if (task.taskType === "image-generation") {
...
function nearestSamples(task, history) {
    const samples = [
...
export function estimateQueueTaskSeconds(task, history) {
...
export function estimateQueueTaskRemainingSeconds(task, history, now = Date.now()) {
...
export function estimateQueueRemainingSeconds(tasks, history, now = Date.now()) {
    ...
    const knownEstimates = estimates.filter((value) => value != null);
    ...
    if (!knownEstimates.length)
        return estimates.length ? null : 0;
