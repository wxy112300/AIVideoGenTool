...
import { h3SharedLatentSaveModeFor, h3SaveJointAvForLatentSaveMode } from "./h3-latent-save.js";
import { normalizeVideoDraft, videoModelSupportsDraftInput } from "./video-draft-normalization.js";
import { h3ContinuumWorkflowPathForInput, h3ContinuumManagedWorkflowPathForInput, h3WorkflowPathForInput, isMiniMaxH3ContinuumModel, isMiniMaxH3Fl2vaModel, isMiniMaxH3Model, isMiniMaxH3R2vModel } from "./workflow.js";
const defaultClock = {
    now: () => new Date(),
...
export function promptOf(draft) {
    ...
    return (promptVersions[activePromptVersion]?.text ??
        promptVersions.at(-1)?.text ??
        "").trim();
}
export function outputNames(state) {
    return [
...
export function imageTaskFromDraft(draft, diffusionModelFilename, outputTarget, clock = defaultClock) {
...
export function extensionTaskFromDraft(draft, state, clock = defaultClock, options = {}) {
    ...
    const h3ExecutionPolicy = isMiniMaxH3Model(draft.modelId)
        ...
        : undefined;
    const h3LatentSaveMode = h3SharedLatentSaveModeFor(draft, isMiniMaxH3R2vModel(draft.modelId), isMiniMaxH3ContinuumModel(draft.modelId) && draft.h3ContinuumMode === "managed");
    return {
        id: clock.id(),
        taskType: "extension",
        status: "waiting",
        createdAt: now,
        updatedAt: now,
        outputFilename: createOutputFilename(draft.modelId, resolution, draft.duration, outputNames(state)),
        prompt: promptOf(draft),
        promptVersion: activePromptIndexForDraft(draft) + 1,
...
export function upscaleTaskFromRequest(request, state, clock = defaultClock) {
    ...
    if (hasAetherScaleFields) {
        ...
        return {
            ...
            createdAt: now,
            updatedAt: now,
            outputFilename: uniqueAetherScaleUpscaleFilename(request.sourceFilename, target.aetherScale.mode, outputNames(state)),
    ...
    if (hasDlss5Fields) {
        ...
        if (target.provider !== "dlss5")
            throw new Error("DLSS5 target normalization failed");
        return {
            id: clock.id(),
            taskType: "upscale",
            status: "waiting",
            createdAt: now,
            updatedAt: now,
            outputFilename: uniqueDlss5UpscaleFilename(request.sourceFilename, target.scale, outputNames(state)),
    ...
    const [targetWidth, targetOutputHeight] = h3Native
        ? h3NativeUpscaleDimensions(request.sourceWidth, request.sourceHeight, targetHeight)
    ...
    return {
        ...
        createdAt: now,
        updatedAt: now,
        outputFilename: uniqueUpscaleFilename(request.sourceFilename, targetHeight, outputNames(state)),
