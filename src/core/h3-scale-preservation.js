...
export function extractH3MicroFpvIntent(sourcePrompt, supplementalText = "") {
...
function microScaleOutputAlreadyLocked(promptText) {
...
function microScaleOutputLock(intent, promptText) {
    if (/[\p{Script=Han}]/u.test(promptText)) {
...
export function ensureH3ScalePreservationInOutput(promptText, mode, sourcePrompt, supplementalText = "") {
    ...
    if (!shouldAddScaleLock && !shouldAddMicroLock)
        return promptText;
    const locks = [
