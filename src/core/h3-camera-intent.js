...
function cameraAngleKindsFor(clause) {
...
function cameraTargetsFor(clause) {
    ...
    if (followTarget && !/^(?:shot|movement|path)\b/iu.test(followTarget))
...
function isViewpointCameraClause(clause, hasMicroFpv = false) {
    ...
    if (cameraOperatorPattern.test(clause) && !nonOperatorCameraMotionPattern.test(clause))
        return false;
    if (shotMarkerOnly)
        return false;
    ...
    if (!hasMotion && !hasShotVocabulary)
        return false;
    if (hasPhysicalOnlyContext && !hasMotion)
        return false;
    return true;
}
export function extractH3CameraIntent(promptText, supplementalText = "") {
    ...
    for (const clause of clauses) {
        ...
        for (const kind of clauseAngles)
            angleKinds.add(kind);
        if (clauseAngles.length)
...
function cameraIntentInstructionLines(intent) {
    const lines = [
...
function outputHasSpatialEvidence(source, output) {
    const groups = [
        {
...
function outputHasCameraTarget(output, target) {
    ...
    if (!normalizedTarget)
        return true;
    if (output.toLocaleLowerCase().includes(normalizedTarget))
        return true;
...
export function auditH3CameraIntent(sourcePrompt, generatedPrompt) {
