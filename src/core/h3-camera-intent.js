...
export function extractH3CameraIntent(promptText) {
    ...
    const requiresViewpoint = hasViewpointCamera && sourceClauses.some((clause) => viewpointPattern.test(clause) || lookAtViewCameraPattern.test(clause) || approachesViewCameraPattern.test(clause) || spatialSignalPattern.test(clause));
