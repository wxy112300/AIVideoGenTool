import { AETHERSCALE_NODE_REVISION, AETHERSCALE_RUNTIME_BUNDLE_ID } from "./catalog/dependencies/aetherscale.js";
...
export const AETHERSCALE_MODEL_ID = "aetherscale-dlss5";
export const AETHERSCALE_WORKFLOW_PATH = "builtin:upscale/aetherscale-dlss5";
export const AETHERSCALE_MOTION_PROFILE = "torch-lk-compact-v1";
export const AETHERSCALE_DEFAULT_MODE = "performance_2x";
export const AETHERSCALE_DEFAULT_STYLE_PROFILE = "faithful";
...
export const AETHERSCALE_MODE_SPECS = [
...
function modeSpec(mode) {
    ...
    if (!spec)
        throw new Error("AetherScale mode is invalid");
    return spec;
}
export function isAetherScaleMode(value) {
    return AETHERSCALE_CARRIER_MODE_VALUES.includes(value);
}
export function isAetherScaleStyleProfile(value) {
    return value === "faithful" || value === "enhanced";
}
export function aetherScaleModeSpec(mode) {
    return modeSpec(mode);
}
export function aetherScaleOutputGeometry(sourceWidth, sourceHeight, mode) {
...
export function defaultAetherScaleOptions(targetWidth, targetHeight, mode = AETHERSCALE_DEFAULT_MODE, styleProfile = AETHERSCALE_DEFAULT_STYLE_PROFILE) {
    const spec = modeSpec(mode);
    return {
...
export function normalizeAetherScaleOptions(value) {
    if (!isRecord(value))
        throw new Error("AetherScale options must be an object");
...
export function normalizeAetherScaleTarget(input) {
    if (!isRecord(input))
        throw new Error("AetherScale target must be an object");
...
export function isAetherScaleUpscaleOptions(value) {
    try {
...
function schemaInput(node, name) {
...
function checkSchemaInput(nodeType, node, names, missingInputs) {
    if (!node)
        return;
    for (const name of names) {
        if (schemaInput(node, name) === undefined)
            missingInputs.push(`${nodeType}.${name}`);
    }
}
export function validateAetherScaleObjectInfoSchema(objectInfo) {
    const missingNodes = [];
    const missingInputs = [];
    const invalidInputs = [];
    ...
    if (!motion)
        missingNodes.push("AetherScaleMotionAnalysis");
    if (!neural)
        missingNodes.push("AetherScaleNeuralRendering");
    ...
    if (motion) {
        ...
        if (!enumValues(schemaInput(motion, "engine"))?.includes("torch_lk"))
            invalidInputs.push("AetherScaleMotionAnalysis.engine");
        if (!enumValues(schemaInput(motion, "quality"))?.includes("balanced"))
            invalidInputs.push("AetherScaleMotionAnalysis.quality");
        if (!Array.isArray(motion.output) || motion.output[0] !== "AETHERSCALE_MOTION")
            outputMismatch.push("AetherScaleMotionAnalysis.output");
    }
    if (neural) {
        const images = schemaInput(neural, "images");
        const motionInput = schemaInput(neural, "motion");
        if (!Array.isArray(images) || images[0] !== "IMAGE")
            invalidInputs.push("AetherScaleNeuralRendering.images");
        if (!Array.isArray(motionInput) || motionInput[0] !== "AETHERSCALE_MOTION")
            invalidInputs.push("AetherScaleNeuralRendering.motion");
        if (!enumValues(schemaInput(neural, "backend"))?.includes("carrier"))
            invalidInputs.push("AetherScaleNeuralRendering.backend");
        if (!enumValues(schemaInput(neural, "upscale_mode"))?.includes("performance_2x"))
            invalidInputs.push("AetherScaleNeuralRendering.upscale_mode");
        if (!Array.isArray(neural.output) || neural.output[0] !== "IMAGE" || neural.output[1] !== "STRING")
            outputMismatch.push("AetherScaleNeuralRendering.output");
    }
...
export function assertAetherScaleObjectInfoSchema(objectInfo) {
    const result = validateAetherScaleObjectInfoSchema(objectInfo);
    if (!result.valid)
        throw new Error(`AetherScale object_info schema mismatch: ${result.errors.join("; ")}`);
}
...
export function validateAetherScaleWorkflow(workflow, objectInfo) {
    const missingNodes = [];
    const invalidEdges = [];
    const invalidInputs = [];
    const outputMismatch = [];
    ...
    if (!graph)
        ...
        for (const nodeType of AETHERSCALE_REQUIRED_WORKFLOW_NODE_TYPES) {
            if (!Object.values(graph).some((node) => isRecord(node) && node.class_type === nodeType)) {
                if (!missingNodes.includes(nodeType))
        ...
        for (const [nodeId, node] of Object.entries(graph)) {
            ...
            if (typeof node.class_type === "string" && AETHERSCALE_FORBIDDEN_WORKFLOW_NODE_TYPES.includes(node.class_type)) {
        ...
        if (motion?.class_type === "AetherScaleMotionAnalysis" && isRecord(motion.inputs)) {
            if (!sameConnection(inputValue(motion, "images"), "1", 0))
        ...
        if (neural?.class_type === "AetherScaleNeuralRendering" && isRecord(neural.inputs)) {
            if (!sameConnection(inputValue(neural, "images"), "1", 0))
                invalidInputs.push("3.images must use 1:0");
            if (!sameConnection(inputValue(neural, "motion"), "2", 0))
                invalidInputs.push("3.motion must use 2:0");
            ...
            if (!isAetherScaleMode(inputValue(neural, "upscale_mode")))
                invalidInputs.push("3.upscale_mode is invalid");
        ...
        if (combine?.class_type === "VHS_VideoCombine" && isRecord(combine.inputs)) {
            if (!sameConnection(inputValue(combine, "images"), "3", 0))
                invalidInputs.push("4.images must use 3:0");
            if (!sameConnection(inputValue(combine, "audio"), "1", 2))
                invalidInputs.push("4.audio must use 1:2");
            const frameRate = inputValue(combine, "frame_rate");
            if (typeof frameRate !== "number" || !Number.isFinite(frameRate) || frameRate <= 0)
                invalidInputs.push("4.frame_rate must be positive");
            if (!isSafeRelativeComfyPath(inputValue(combine, "filename_prefix")))
                invalidInputs.push("4.filename_prefix must be a safe relative output prefix");
    ...
    if (objectInfo !== undefined) {
        ...
        for (const value of schema.missingNodes)
            if (!missingNodes.includes(value))
                missingNodes.push(value);
...
export function buildAetherScaleUpscaleWorkflow(task, sourceVideo, objectInfo) {
    if (task.modelId !== AETHERSCALE_MODEL_ID)
        throw new Error("AetherScale workflow requires modelId aetherscale-dlss5");
    ...
    if (!task.aetherScale)
        throw new Error("AetherScale workflow is missing its immutable options snapshot");
    ...
    if (!validation.valid)
        throw new Error(`AetherScale workflow validation failed: ${validation.errors.join("; ")}`);
    return graph;
}
