...
const legacyChineseImagePromptPresets: Record<ImagePromptPreset, string> = {
  faithful: "只整理和细化用户明确提出的图片编辑意图，不得新增或推断人物、物品、背景、文字、Logo、水印、动作、风格、材质、光照或构图。保持用户明确的主体、编辑范围和限制不变。",
  "detail-enhance": "在不改变用户指定主体、构图和编辑范围的前提下，补充有助于执行的区域、参考作用、空间关系、材质、光照、透视、边缘融合和需要保持不变的内容。不要添加无关主体、故事或视觉元素。"
};
...
export function normalizeQwenImagePromptPresets(value: unknown): Record<ImagePromptPreset, string> {
  const defaults = createDefaultQwenImagePromptPresets();
  const source = value && typeof value === "object"
    ? value as Partial<Record<ImagePromptPreset, unknown>>
    : {};
  return {
    faithful: typeof source.faithful === "string" && source.faithful.trim() && source.faithful !== legacyChineseImagePromptPresets.faithful
      ? source.faithful.trim()
      : defaults.faithful,
    "detail-enhance": typeof source["detail-enhance"] === "string" && source["detail-enhance"].trim() && source["detail-enhance"] !== legacyChineseImagePromptPresets["detail-enhance"]
      ? source["detail-enhance"].trim()
      : defaults["detail-enhance"]
  };
}

export function qwenImageEditPromptContract(
...
export function qwenImageEditPromptUserContent(request: EnhanceRequest): string {
...
export function normalizeQwenImageEditPromptOutput(value: string): string {
  const fenced = value
