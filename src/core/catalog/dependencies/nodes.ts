...
type NodeVersionDefaults = Pick<
  ...
  "versionMode" | "releaseSource" | "recommendedVersion" | "latestVersion"
>;

/**
 * User-facing package versions.  Installation pins and compatibility evidence
 * stay in the definitions below, but they are not version labels.  A rolling
 * entry has no upstream semantic release; a pinned entry is intentionally
 * identified by the app's immutable install target rather than exposing a
 * commit hash in Settings.
 */
const nodeVersionDefaults: Readonly<Record<string, NodeVersionDefaults>> = {
...
for (const definition of customNodeDefinitions) {
  ...
  if (defaults) Object.assign(definition, defaults);
}
