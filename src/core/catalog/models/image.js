import { birefnetRequiredNodeTypes, flux2Klein4bRequiredNodeTypes, hidreamO1RequiredNodeTypes, lamaInpaintRequiredNodeTypes, minimaxH3ImageI2IRequiredNodeTypes, minimaxH3ReferenceEditRequiredNodeTypes, omnigen2RequiredNodeTypes, qwenImage21GgufRequiredNodeTypes, qwenImage21RequiredNodeTypes, qwenImageEdit2511RequiredNodeTypes, qwenImageEdit2511CropStitchRequiredNodeTypes, zImageRequiredNodeTypes, zImageTurboRequiredNodeTypes } from "../../image-workflow/node-requirements.js";
...
export const imageModelEntries = [
    ...
    entry({
        ...
        capabilities: { maxReferenceImages: 10 },
        scan: { managedBy: "comfyui", vram: "Q8_0 GGUF · Qwen3-VL 8B INT8 · RTX 4090 24GB 推荐 · 文本编码器可卸载", integrated: true, requiredCustomNodeIds: ["comfyui-gguf"], runtimeNodeTypes: qwenImage21GgufRequiredNodeTypes, components: [
                component("Qwen Image 2.1 Uncensored GGUF Q8_0 扩散模型", "unet/qwen-image-2.1-Q8_0.gguf", /(?:^|[\\/])unet[\\/]qwen-image-2\.1-Q8_0\.gguf$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/qwen-image-2.1-Q8_0.gguf?download=true", "unet", "qwen-image-2.1-Q8_0.gguf", "4090 默认使用 Q8_0；模型卡标注约 7.59GB，Qwen3-VL INT8 可卸载到系统内存。")),
                component("Qwen3-VL 8B 文本编码器", "text_encoders/qwen3vl_8b_{bf16|int8_convrot}.safetensors", /text_encoders[\\/]qwen3vl_8b_(?:bf16|int8_convrot)\.safetensors$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/text_encoders/qwen3vl_8b_int8_convrot.safetensors?download=true", "text_encoders", "qwen3vl_8b_int8_convrot.safetensors", "与官方 Qwen Image 2.1 相同的 Qwen3-VL 8B 文本编码器；4090 优先使用 INT8 ConvRot。")),
                component("Qwen Image 2.1 VAE", "vae/qwen_image_2.1_vae_bf16.safetensors", /vae[\\/]qwen_image_2\.1_vae_bf16\.safetensors$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/vae/qwen_image_2.1_vae_bf16.safetensors?download=true", "vae", "qwen_image_2.1_vae_bf16.safetensors"))
            ] }
    }, { name: "Qwen Image 2.1 · Uncensored GGUF Q8_0", badge: "GGUF · Q8_0 默认 · T2I + 最多 10 图", description: "Qwen Image 2.1 的高精度 GGUF 路径；无参考图走 T2I，有参考图复用官方多图编辑与 Paint 引导，需要维护版 ComfyUI-GGUF。" }, { name: "Qwen Image 2.1 · Uncensored GGUF Q8_0", badge: "GGUF · Q8_0 default · T2I + up to 10 images", description: "Higher-precision GGUF path for Qwen Image 2.1. Text-only prompts use T2I; references reuse the official multi-image edit and Paint guidance path through the maintained ComfyUI-GGUF package." }, { name: "Qwen Image 2.1 · Uncensored GGUF Q8_0", badge: "GGUF · Q8_0 預設 · T2I + 最多 10 圖", description: "Qwen Image 2.1 的高精度 GGUF 路徑；無參考圖走 T2I，有參考圖復用官方多圖編輯與 Paint 引導，需要維護版 ComfyUI-GGUF。" }),
    entry({
        id: "qwen-image-2-1-uncensored-gguf-q6", family: "qwen-image", category: "image", adapterId: "qwen-image-2-1-uncensored-gguf-q6", promptPackId: "qwen-image-2-1", order: 994, inputModes: ["image"],
        capabilities: { maxReferenceImages: 10 },
        scan: { managedBy: "comfyui", vram: "Q6_K GGUF · Qwen3-VL 8B INT8 · RTX 4090 24GB 推荐 · 文本编码器可卸载", integrated: true, requiredCustomNodeIds: ["comfyui-gguf"], runtimeNodeTypes: qwenImage21GgufRequiredNodeTypes, components: [
                component("Qwen Image 2.1 Uncensored GGUF Q6_K 扩散模型", "unet/qwen-image-2.1-Q6_K.gguf", /(?:^|[\\/])unet[\\/]qwen-image-2\.1-Q6_K\.gguf$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/qwen-image-2.1-Q6_K.gguf?download=true", "unet", "qwen-image-2.1-Q6_K.gguf", "4090 备用质量/显存档；模型卡标注约 5.88GB，Qwen3-VL INT8 可卸载到系统内存。")),
                component("Qwen3-VL 8B 文本编码器", "text_encoders/qwen3vl_8b_{bf16|int8_convrot}.safetensors", /text_encoders[\\/]qwen3vl_8b_(?:bf16|int8_convrot)\.safetensors$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/text_encoders/qwen3vl_8b_int8_convrot.safetensors?download=true", "text_encoders", "qwen3vl_8b_int8_convrot.safetensors", "与官方 Qwen Image 2.1 相同的 Qwen3-VL 8B 文本编码器；4090 优先使用 INT8 ConvRot。")),
                component("Qwen Image 2.1 VAE", "vae/qwen_image_2.1_vae_bf16.safetensors", /vae[\\/]qwen_image_2\.1_vae_bf16\.safetensors$/i, guide("abenzerps / Qwen-Image-2.1-GGUF", "https://huggingface.co/abenzerps/Qwen-Image-2.1-GGUF/resolve/main/vae/qwen_image_2.1_vae_bf16.safetensors?download=true", "vae", "qwen_image_2.1_vae_bf16.safetensors"))
            ] }
    }, { name: "Qwen Image 2.1 · Uncensored GGUF Q6_K", badge: "GGUF · Q6_K · T2I + 最多 10 图", description: "Qwen Image 2.1 的 Q6_K GGUF 备用路径；质量和显存占用低于 Q8_0，但明显高于 Q4，适合 4090 需要余量时使用。" }, { name: "Qwen Image 2.1 · Uncensored GGUF Q6_K", badge: "GGUF · Q6_K · T2I + up to 10 images", description: "Q6_K fallback GGUF path for Qwen Image 2.1. It trades some quality and memory for headroom while staying above the retired Q4 route." }, { name: "Qwen Image 2.1 · Uncensored GGUF Q6_K", badge: "GGUF · Q6_K · T2I + 最多 10 圖", description: "Qwen Image 2.1 的 Q6_K GGUF 備用路徑；品質和顯存占用低於 Q8_0，但明顯高於 Q4，適合 4090 需要餘量時使用。" }),
    entry({
