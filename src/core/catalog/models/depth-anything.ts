...
export const DEPTH_ANYTHING_V2_SMALL_REPOSITORY =
  "depth-anything/Depth-Anything-V2-Small-hf" as const;
export const DEPTH_ANYTHING_V2_SMALL_REVISION =
  "5426e4f0f36572d16453bbda7a8389317b1bef99" as const;
export const DEPTH_ANYTHING_V2_SMALL_MODEL_SUBDIRECTORY =
  "depthanything/Depth-Anything-V2-Small-hf" as const;
export const DEPTH_ANYTHING_V2_SMALL_MODEL_FILENAME = "model.safetensors" as const;

function escaped(value: string): string {
  return value.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&");
}

function depthModelPattern(): RegExp {
  const separator = "[/\\\\]";
  const pathPattern = escaped(DEPTH_ANYTHING_V2_SMALL_MODEL_SUBDIRECTORY)
    .replaceAll("/", separator);
  return new RegExp(
    `${pathPattern}${separator}${escaped(DEPTH_ANYTHING_V2_SMALL_MODEL_FILENAME)}$`,
    "i"
  );
}

function depthInstallGuide(): CatalogInstallGuide {
  return {
...
function depthComponent(): CatalogModelComponent {
  return component(
...
export const depthAnythingV2: CatalogModelEntry = entry({
...
export const depthAnythingModelEntries: CatalogModelEntry[] = [depthAnythingV2];
