...
const managedPromptTraditional = {
    "lightx2v/minimax-h3-prompt-rewriter-8b": {
        name: "MiniMax H3 Prompt Rewriter LoRA · Qwen3-VL 8B",
        badge: "Qwen3-VL 8B · PEFT LoRA · ComfyUI",
...
const managedPromptEntries = managedPromptModelDefinitions.map((model) => {
    ...
    if (model.backend === "comfyui-qwenvl-lora") {
        ...
        return entry({
            ...
            scan: {
                ...
                components: [
                    ...baseFiles.map(([filename, label]) => component(label, `${baseDirectory}/${filename}`, new RegExp(`${basePattern}/${filename.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")}$`, "i"), guide("Qwen · Qwen3-VL-8B-Instruct", `${baseUrl}/${filename}?download=true`, baseDirectory, filename, baseNotes))),
                    ...[1, 2, 3, 4].map((shard) => {
    ...
    const makeGuide = (filename) => guide(sourceLabel, `${baseUrl}/${filename}?download=true`, model.targetDirectory, filename, notes);
    return entry({
        id: model.id,
...
export const promptModelEntries = [...nativePromptEntries, ...managedPromptEntries];
