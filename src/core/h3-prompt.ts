...
export function h3ShotPolicyForPrompt(promptText: string): H3ShotPolicy {
  ...
  if (!prompt) return "default-single";
...
export interface H3PromptControlInput {
  rawPrompt: string;
  mode: H3PromptMode;
...
export interface H3PromptControlPlan {
  mode: H3PromptMode;
  preset: H3PromptPreset;
  sourcePrompt: string;
  ...
  hasExactRotation: boolean;
  hasScale: boolean;
  hasMicroScale: boolean;
  hasAction: boolean;
  ...
  modules: readonly H3PromptModuleId[];
}
...
function h3PromptControlPlanFor(input: H3PromptControlInput): H3PromptControlPlan {
  ...
  const modules: H3PromptModuleId[] = ["intent-lock"];
  if (hasReference) modules.push("reference-delta");
  if (cameraIntent.hasViewpointCamera) modules.push("camera-route");
  if (cameraIntent.hasPhysicalCamera) modules.push("camera-disambiguation");
  if (cameraIntent.rotationDegrees.length) modules.push("exact-rotation");
  if (scaleIntent.detected && scaleIntent.humanSubject) modules.push("micro-scale");
  if (microFpvIntent.detected) modules.push("metaphor-disambiguation");
  if (hasAction || hasReference) modules.push("action-mechanics");
  if (hasAction && hasHumanSubject) modules.push("human-motion-integrity");
  if (hasInteraction) modules.push("subject-reaction");
  if (hasExpression) modules.push("expression-detail");
  if (dialogueLocks.length) modules.push("speech-gate");
  if (hasSound) modules.push("sound-causality");
  if (h3ShotPolicyForPrompt(input.rawPrompt) !== "allow-multiple") {
    modules.push("shot-continuity");
  }
  if (input.mode === "FL2VA" || input.mode === "L2VA") modules.push("endpoint-transition");
  return {
    mode: input.mode,
...
export function buildH3PromptControlPlan(input: H3PromptControlInput): H3PromptControlPlan {
 ...
 */
export function h3PromptControlInstruction(input: H3PromptControlInput): string {
...
export interface H3PromptControlAudit {
  passed: boolean;
  modules: readonly H3PromptModuleId[];
  missing: string[];
}
...
export function auditH3PromptControlOutput(
  plan: H3PromptControlPlan,
  generatedPrompt: string
): H3PromptControlAudit {
  ...
  if (plan.hasCamera) {
    ...
    if (!cameraAudit.passed) missing.push("camera-control");
  }
  ...
  if (plan.shotPolicy !== "allow-multiple" && (
    /\[Shot\s+[2-9]\]/iu.test(generatedPrompt) ||
...
export function h3DetailedExpansionMinimumWords(
  mode: H3PromptMode,
  durationSeconds = 5,
  sourcePrompt = ""
): number {
...
function h3TextFromJson(value: unknown, mode: H3PromptMode): string | undefined {
  if (typeof value === "string") return value.trim() || undefined;
  if (Array.isArray(value)) {
    const candidates = value.map((item) => h3TextFromJson(item, mode)).filter(Boolean) as string[];
  ...
  for (const candidateValue of Object.values(record)) {
    ...
    if (candidate && /(?:integrated_multimodal_description|detailed_description)\s*:/iu.test(candidate)) return candidate;
  }
  return undefined;
}

export function unwrapH3ModelOutput(promptText: string, mode: H3PromptMode): string {
  ...
  if (fenced?.[1]) output = fenced[1].trim();
  ...
  for (let pass = 0; pass < 3; pass += 1) {
    ...
    try {
      ...
      if (!extracted || extracted === output) break;
      output = extracted;
    } catch {
      break;
    }
  ...
  return output
    ...
    .replace(/^\s*```\s*$/gmu, "")
    .replace(
      new RegExp(`^[ \\t]*(?:#{1,6}[ \\t]+)?(?:\\*\\*|__)?(${fieldAlternation})(?::(?:\\*\\*|__)|(?:\\*\\*|__)?[ \\t]*:)[ \\t]*`, "gimu"),
...
function collapseUnexpectedH3Shots(
  ...
  const cameraCutReplacements = normalizedTimeline
    ...
    .replace(/\b(?:a|an)\s+new\s+(?:camera\s+)?angle\s+(?:reveals?|shows?|frames?)\b/giu, "the ongoing camera movement gradually reveals")
    .replace(/\b(?:the\s+)?view\s+(?:suddenly\s+)?(?:shifts?|changes?)\s+to\b/giu, "the camera physically continues its existing trajectory toward")
    .replace(/\b(?:the\s+)?camera\s+is\s+(?:now|suddenly)\s+(?:positioned|placed|located)\b/giu, "the camera physically arrives along its existing trajectory")
    .replace(/\bfrom\s+another\s+(?:camera\s+)?angle\b/giu, "as the camera continues along its existing trajectory")
    .replace(/\b(?:a|an)\s+new\s+(?:camera\s+)?angle\b/giu, "the next angle reached along the same physical trajectory")
    .replace(/(?:新机位|新角度|另一个角度)/gu, "摄影机沿既有轨迹连续运动后形成的角度")
    .replace(/(?:画面突然切换|视角突然切换|镜头突然来到)/gu, "摄影机沿既有轨迹连续运动至");
  if (!collapsed && cameraCutReplacements === timeline) return promptText;
...
function repairH3PromptControlViolations(
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    ...
    if (audit.passed) return repaired;
    let next = repaired;
...
export function assertDetailedCinematicExpansion(
  promptText: string,
  mode: H3PromptMode,
  durationSeconds: number,
  sourcePrompt: string
  ...
  const normalizeToken = (token: string): string => {
    ...
    if (!/^[a-z]+$/u.test(lower) || lower.length < 5) return lower;
  ...
  const outputTokens = new Set(
    [...new Intl.Segmenter(undefined, { granularity: "word" }).segment(timeline)]
  ...
  const sourceTokens = [...new Intl.Segmenter(undefined, { granularity: "word" }).segment(cleanSource)]
    ...
    .filter((token) => {
      if (/\p{Script=Han}/u.test(token)) return outputHasCjk && token.length >= 2;
      const normalized = normalizeToken(token);
      if (/\d/u.test(normalized)) return true;
      if (sourceHasCjk !== outputHasCjk) return false;
