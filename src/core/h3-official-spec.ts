...
export function h3OfficialPromptBaseline(mode: H3PromptMode): string {
  const modeRules = mode === "R2V"
    ? [
        ...
        "R2V detail rule: write detailed_description in playback order, establish the overall style before [Shot 1], insert reference labels where their content actually appears, preserve the shared H3 shot/dialogue/audio rules, and target roughly 350-500 English words for a typical 5-second generation prompt."
      ]
    : mode === "FL2VA"
      ? [
  ...
  return [
    ...
    "Reference economy rule: inspect every supplied image, video, and audio reference, but use it as a compact visual/audio anchor rather than a script to transcribe. Mention identity, composition, objects, lighting, motion character, or voice qualities only when they affect continuity, the requested action, or the atmosphere. Do not turn the prompt into an inventory of visible clothing, materials, background objects, or lighting that the H3 encoder already receives. Never invent unsupported characters, props, locations, plot, camera moves, dialogue, or text.",
    ...
    "Visual economy rule: H3 receives the reference media directly. Use a compact opening anchor and, when needed, a compact final-state anchor; spend the remaining words on what changes over time. Do not repeat a static wardrobe, material, background, or lighting inventory unless it affects the action, continuity, or atmosphere.",
    ...
    "Detail budget rule: produce a production-ready prompt rather than a caption, but prefer compactness over completeness. Scale the number of action beats and the word budget to the actual effective duration; give most of the space to motion, micro-reactions, camera, atmosphere, and sound. Use the longer 350-500 word range for R2V's full-reference analysis or when a longer clip genuinely needs more beats. Do not repeat the same fact in different sections or finish a long-duration action as if it were a 5-second clip.",
    ...modeRules,
    ...nonR2vExclusions,
