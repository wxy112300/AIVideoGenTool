...
import { h3ScalePreservationContract } from "./h3-scale-preservation.js";
 ...
 */
export function h3CommunityPromptWriterContract(
  mode: H3PromptMode,
  ...
  const referenceRules = mode === "R2V"
    ? [
        ...
        preset === "detailed-cinematic"
          ? "R2V detailed-expansion floor: for a simple approximately five-second reference-generation task, normally develop detailed_description to at least the 350-500 grounded-English-word starting range. Scale upward for longer duration, several required actions, dialogue, interactions, or complex reference roles. If the source brief is already more detailed, preserve every requirement and expand from that baseline. Each added sentence must ground causal action, reaction, camera, visible change, continuity, or sound; state static reference facts once and never pad."
  ...
  const groundingRule = preset === "detailed-cinematic"
    ? "Detailed-expansion factual boundary: this preset must produce the most fully developed timeline, never a concise rewrite. Preserve every fact, user-written action, and action order from the user's brief and supplied references. For every user action, add at least two applicable kinds of scene-grounded execution detail such as anticipation, gaze/expression, body mechanics, contact/force, weight/momentum, affected-subject or object response, motivated camera behavior, settling, or causally synchronized sound. Never collapse multiple user actions into a generic summary, and never make the final timeline less developed than the source brief. If output space is tight, remove repeated reference inventory and assistant-added filler before any user-required action, camera instruction, reaction, dialogue, or constraint. Do not introduce new characters, locations, important props, plot events, visible text, dialogue, music, or major appearance changes, and never override an explicit constraint."
  ...
  return [
    ...
    groundingRule,
    "Continuous-shot default: use multiple shots only when the user's intent or a referenced temporal/camera structure requires them; otherwise keep exactly one unbroken [Shot 1]. Do not put an editorial cut, edit, dissolve, wipe, montage, scene change, or shot reset inside it; use continuous physical camera movement for reframing.",
...
export function h3SmallModelPromptContract(
  mode: H3PromptMode,
  preset: H3PromptPreset = "official-storyboard"
): string {
  ...
  const detailedExpansionRules = preset === "detailed-cinematic"
    ? [
        "Detailed cinematic expansion rule: after extracting the user's intent and inspecting the supplied media, silently checklist every concrete user-specified subject, action, action order, camera/viewpoint, route, timing, dialogue, sound, and prohibition. Render every checklist item as an explicit observable fact or event in the final H3 fields in the same order, then expand the timeline through opening state, trigger or preparation, primary action, physical response, reaction or secondary motion, camera response, and final state. Use only the phases the action needs, let each phase have natural duration, and identify its visible change, primary camera behavior, synchronized audio event only when caused or requested, and resulting state. Never collapse a chain of user actions into a generic summary.",
        "Detailed timing rule: preserve the real H3 first/last-frame anchors for the selected mode, but do not force intermediate events into fixed fractions or equally spaced timestamps. Use a time or [Shot N] only for a genuine cut, state transition, camera/keyframe alignment, or other meaningful temporal anchor; a continuous camera adjustment stays inside the same shot.",
        mode === "R2V"
          ? "Detailed expansion coverage rule: for a simple approximately five-second request, normally develop detailed_description to at least the 350-500 grounded-English-word starting range. Scale upward for longer duration, multiple required actions, dialogue, interactions, or complex references. This is a coverage floor and planning range, not a padding target or hard maximum; if the user's source is already detailed, preserve all of it and expand from that baseline."
          : "Detailed expansion coverage rule: for a simple approximately five-second request, normally develop integrated_multimodal_description to roughly 180-320 grounded English words. Scale upward for longer duration, multiple required actions, dialogue, or interactions. This is a coverage floor and planning range, not a padding target or hard maximum; if the user's source is already detailed, preserve all of it and expand from that baseline.",
        "Detailed expansion allocation rule: spend the added length on user-requested action, interactions, reactions, camera path, timing, continuity, dialogue, and causally synchronized sound; state reference appearance once and remove repeated inventories, generic adjectives, decorative atmosphere, and filler before removing a necessary user action, camera instruction, reaction, dialogue, or constraint."
      ]
    : [];
  return [
    ...
    h3ScalePreservationContract(),
    "Micro-FPV/metaphor execution rule: when the user says ant-size, ant's view, insect-eye, or similar shorthand for a human/scene, translate it into an invisible low-clearance viewpoint and reference-derived world scale, not a literal animal, drone, or physical camera. If an actual animal or device is explicitly the subject, preserve that explicit subject.",
    "Exact orbit rule: copy every user-specified camera rotation angle or fraction exactly. A 180-degree orbit is one half orbit/semicircle that stops at its endpoint; do not expand a partial orbit into a full circle, add an extra revolution, or substitute a subject rotation.",
    "Route rule: when a camera or scaled subject travels from A to B, describe the continuous route through meaningful landmarks, surfaces, obstacles, turns, openings, and arrival state. Keep the camera route and subject route spatially linked, and never replace either with teleportation or a sequence of decorative cuts.",
    ...
    "Single-shot rule: when the user says one shot, one take, continuous shot, no cuts, 一镜到底, 单镜头, or 不剪辑, output exactly one [Shot 1] as one unbroken take. Do not add later shot markers, editorial cuts, edits, transitions, montage, scene changes, shot resets, or hidden cuts; keep all camera and action changes inside that shot.",
...
export function h3OfficialPromptBaseline(mode: H3PromptMode): string {
  ...
  return [
    ...
    h3ScalePreservationContract(),
    "Micro-FPV/metaphor execution rule: translate ant-size, ant's view, insect-eye, or similar human-scale shorthand into an invisible low-clearance viewpoint and reference-derived world scale, not a literal animal, drone, or physical camera, unless the user explicitly requests that subject.",
    "Exact orbit rule: preserve every user-specified camera rotation angle or fraction exactly; 180 degrees means one half orbit/semicircle ending at the opposite side, never an expanded full circle or extra revolution.",
    "Route rule: when the camera or a scaled subject travels from A to B, keep a continuous route through visible landmarks, surfaces, obstacles, turns, openings, and the arrival state; do not teleport or turn route phases into unexplained cuts.",
