...
export function h3CommunityPromptWriterContract(
  ...
  const referenceRules = mode === "R2V"
    ? [
        ...
        preset === "detailed-cinematic"
          ? "R2V detailed-expansion floor: follow the request-specific minimum and approximately doubled target supplied by the detailed cinematic gate. Scale with the effective duration, source length, required actions, dialogue, interactions, and reference roles instead of falling back to a generic five-second range. Preserve every requirement and expand from that baseline. Each added sentence must ground causal action, reaction, camera, visible change, continuity, or sound; state static reference facts once and never pad."
  ...
  return [
    ...
    groundingRule,
    "Continuous-shot default: unless the user explicitly requests multiple views, keep one continuous [Shot 1] with no editorial cut and obtain all reframing through physical camera or focus movement inside the take.",
...
export function h3SmallModelPromptContract(
  ...
  const detailedExpansionRules = preset === "detailed-cinematic"
    ? [
        ...
        mode === "R2V"
          ? "Detailed expansion coverage rule: obey the request-specific minimum and approximately doubled writing target supplied by the detailed cinematic gate. Scale from the actual duration, source length, required actions, dialogue, interactions, and reference complexity; never substitute a generic five-second range."
          : "Detailed expansion coverage rule: obey the request-specific minimum and approximately doubled writing target supplied by the detailed cinematic gate. Scale from the actual duration, source length, required actions, dialogue, and interactions; never substitute a generic five-second range.",
  ...
  return [
    ...
    "Dual-language rule: the target output language controls explanatory H3 prose only. Keep every official H3 field name exactly in its required English ASCII form. Never translate, transliterate, paraphrase, censor, or normalize dialogue, lyrics, voiceover words, or visible text; each keeps the language and punctuation supplied by the user.",
    ...
    "Single-shot rule: an explicit one-shot request locks the whole timeline to one continuous [Shot 1] with no editorial cut; keep all action and camera changes inside that take.",
