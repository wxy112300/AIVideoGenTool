...
import { promptContainsVideoLoraTrigger, videoLoraDefinition } from "./video-loras.js";
const baseSections = [
...
function hasDialogue(prompt) {
    return /<d>\s*\[[^\]]+\]\s*.+?<\/d>/su.test(prompt);
}
function hasSpeakerId(prompt) {
    return /\(S\d+(?:\s*,\s*S\d+)*\)/u.test(prompt);
}
function checkShotTimestamps(prompt) {
    const shotNumbers = [...prompt.matchAll(/\[Shot\s+(\d+)\]/giu)]
        .map((match) => Number(match[1]))
        .filter((value) => Number.isInteger(value));
    if (!shotNumbers.some((value) => value >= 2))
        return true;
...
function firstShotHasTimestamp(prompt) {
    ...
    if (!firstShotMarker || firstShotMarker.index === undefined)
        return false;
...
function sectionsInOrder(prompt, sections) {
    let previous = -1;
...
function timestampsOutsideDuration(prompt, durationSeconds) {
    if (!Number.isFinite(durationSeconds) || (durationSeconds ?? 0) <= 0)
        return [];
    return [...prompt.matchAll(/\bAt\s+(\d{2}):(\d{2})\.(\d{3})/giu)]
        .filter((match) => Number(match[2]) >= 60 || (Number(match[1]) * 60 + Number(match[2]) + Number(match[3]) / 1000) > durationSeconds + 0.001)
        .map((match) => match[0]);
}
function startsWithOfficialInstruction(prompt, mode) {
    if (mode === "T2VA")
        return prompt.startsWith("integrated_multimodal_description:");
    if (mode === "I2VA")
        return prompt.startsWith("For the target video, at 0.00 seconds");
    if (mode === "FL2VA" || mode === "L2VA") {
        return prompt.startsWith("How the reference pictures align with the target video");
    }
    return true;
}
function sentenceCount(section) {
    const content = section.trim();
    if (!content || /^N\/A$/iu.test(content))
        return 0;
    return Math.max(1, content.match(/[.!?](?=\s|$)/gu)?.length ?? 1);
}
function promptPrefixesForLora(lora) {
    return [...new Set((lora.promptPrefixes ?? videoLoraDefinition(lora.id)?.promptPrefixes ?? [])
            .map((prefix) => prefix.trim())
            .filter(Boolean))];
}
export function checkH3Prompt(promptText, options = {}) {
    const prompt = promptText.trim();
    const mode = options.mode ?? inferH3PromptMode(options.hasImageReference ?? true, Boolean(options.hasEndImage));
    const items = [];
