import { H3_TURBO_LORA_FILENAME, H3_TURBO_LORA_ID, isH3Ref2vTurboEnabled, isH3TurboLoraId, videoLoraCompatibleWithModel, videoLoraFilename, videoPromptForLoras } from "./video-loras.js";
...
function applyVideoLoraStack(workflow, task, locale = "zh-CN") {
    ...
    if (!isMiniMaxH3Model(task.modelId) || selected.length === 0)
        return;
    const sageNodes = Object.values(workflow).filter((node) => node.class_type === "PathchSageAttentionKJ" && Array.isArray(node.inputs?.model));
...
function applyMiniMaxH3Ref2vTurboSampling(workflow, task) {
    ...
    if (!Array.isArray(currentModel) || typeof currentModel[0] !== "string")
        return;
    if (consumers.some((node) => JSON.stringify(node.inputs?.model) !== JSON.stringify(currentModel)))
        return;
