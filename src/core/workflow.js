import { H3_AFTER_MIDNIGHT_LORA_ID, H3_TURBO_LORA_FILENAME, H3_TURBO_LORA_ID, isH3Ref2vTurboEnabled, isH3PddLoraId, isH3SlaTurboLoraId, isH3TurboFourStepLoraId, isH3TurboV4LoraId, isH3TurboLoraId, videoLoraCompatibleWithModel, videoLoraFilename, videoPromptForLoras } from "./video-loras.js";
...
import { h3LatentSaveModeFor, h3LatentSaveModeSavesJointAv, h3LatentSaveModeSavesMotionContext } from "./h3-latent-save.js";
...
export function isMiniMaxH3ContinuumModel(modelId) {
    return modelCatalog.get(modelId)?.definition.variant === "continuum";
}
...
function workflowBasename(workflowPath) {
...
export function isKnownBundledH3WorkflowPath(workflowPath) {
    return knownBundledH3WorkflowFilenames.has(workflowBasename(workflowPath ?? ""));
}
export function isMiniMaxH3ContinuumV38Workflow(workflowPath) {
...
export function isMiniMaxH3ContinuumManagedWorkflow(workflowPath) {
...
export function h3ContinuumModeForSource(source) {
    if (source.h3ContinuumSequence && source.h3ContinuumSequence.acceptedChunks > 0)
        return "managed";
...
export function h3ContinuumWorkflowPathForInput(workflowPath) {
...
export function h3ContinuumManagedWorkflowPathForInput(workflowPath) {
    const basename = workflowBasename(workflowPath);
    if (basename !== H3_CONTINUUM_LEGACY_WORKFLOW_FILENAME &&
...
function applyVideoLoraStack(workflow, task, locale = "zh-CN") {
    ...
    if (!isMiniMaxH3Model(task.modelId) || selected.length === 0)
        return;
    const attentionNodes = Object.values(workflow).filter((node) => (node.class_type === "PathchSageAttentionKJ" ||
...
function applyMiniMaxH3Ref2vTurboSampling(workflow, task) {
    if (task.taskType === "generation" && task.videoLoras?.some((lora) => isH3PddLoraId(lora.id) && videoLoraCompatibleWithModel(lora, task.modelId)))
        return;
    const ref2vTurbo = isH3Ref2vTurboEnabled(task);
    const fl2vaFourStepTurbo = isMiniMaxH3Fl2vaModel(task.modelId) &&
        Boolean(task.videoLoras?.some((lora) => isH3TurboFourStepLoraId(lora.id) && videoLoraCompatibleWithModel(lora, task.modelId)));
    ...
    if (!ref2vTurbo && !fl2vaFourStepTurbo && !fl2vaV4Turbo && !fl2vaSlaTurbo && !afterMidnight)
        return;
    ...
    if (!ref2vTurbo && !fl2vaFourStepTurbo && !fl2vaV4Turbo && !fl2vaSlaTurbo)
        return;
...
function h3SamplingShiftIsReachable(workflow, consumer, shiftId) {
...
function samplingShiftInsertionPoint(workflow, consumer) {
    let current = h3ModelLink(consumer.inputs?.model);
    const visited = new Set();
    let deepestPostNodeId;
    let deepestPostUpstream;
    while (current) {
        const [nodeId] = current;
        if (visited.has(nodeId))
            return null;
...
function upsertMiniMaxH3SamplingShift(workflow, consumers, shift) {
    ...
    if (existing && consumers.every((consumer) => h3SamplingShiftIsReachable(workflow, consumer, existing[0]))) {
        existing[1].inputs = {
            ...(existing[1].inputs ?? {}),
    ...
    if (insertionPoints.some((point) => !point))
        return;
    const first = insertionPoints[0];
    if (insertionPoints.some((point) => point.postNodeId !== first.postNodeId || JSON.stringify(point.upstream) !== JSON.stringify(first.upstream)))
        return;
 ...
 */
function applyMiniMaxH3PddSampling(workflow, task) {
    if (task.taskType !== "generation")
        return;
    const pdd = task.videoLoras?.find((lora) => isH3PddLoraId(lora.id) && videoLoraCompatibleWithModel(lora, task.modelId));
    if (!pdd)
        return;
    const sampler = Object.values(workflow).find((node) => node.class_type === "KSamplerSelect");
    const schedulers = Object.values(workflow).filter((node) => node.class_type === "BasicScheduler");
...
export function normalizeH3Steps(value, modelId = "", videoLoras) {
    if (videoLoras?.some((lora) => isH3PddLoraId(lora.id) && videoLoraCompatibleWithModel(lora, modelId)))
        return 8;
...
function generationSafetyProfileForModel(modelId, workflowPath) {
    if (isMiniMaxH3Model(modelId)) {
        const capabilities = modelCatalog.get(modelId)?.definition.capabilities;
        return {
            label: isMiniMaxH3ContinuumModel(modelId)
...
export function workflowSupportsH3TurboSampling(source, options = {}) {
    ...
    if (nativeTurbo)
        return true;
    const fl2vaFourStepTurbo = options.videoLoras?.some((lora) => isH3TurboFourStepLoraId(lora.id) && videoLoraCompatibleWithModel(lora, options.modelId ?? "")) === true;
...
export function workflowSupportsH3PddSampling(source, options = {}) {
    if (!source || typeof source !== "object" || Array.isArray(source))
        return false;
    const nodes = Object.values(source).filter((node) => Boolean(node) && typeof node === "object" && !Array.isArray(node));
    const hasNode = (classType, predicate = () => true) => nodes.some((node) => {
        if (node.class_type !== classType)
            return false;
        const inputs = node.inputs;
        return Boolean(inputs) && typeof inputs === "object" && !Array.isArray(inputs) &&
...
/**
 * Continuum samples the overlap and the newly requested segment together.
 * H3's temporal grid is 5 + 17*n frames, so choose the nearest valid grid
 * point instead of rounding the visible segment independently.
 */
function continuumFrameCountForSeconds(durationSeconds, contextFrames = H3_CONTINUUM_CONTEXT_FRAMES) {
    const safeDuration = Number.isFinite(durationSeconds)
        ? Math.max(1, durationSeconds)
        : 1;
    const target = Math.max(5, Math.round(safeDuration * 24) + Math.max(0, Math.floor(contextFrames)));
    const lower = 5 + Math.max(0, Math.floor((target - 5) / 17)) * 17;
    const upper = lower + 17;
    return target - lower <= upper - target ? lower : upper;
}
export function continuumSampledFrameCountForSeconds(durationSeconds, contextFrames = H3_CONTINUUM_CONTEXT_FRAMES) {
...
export function continuumV38MaxDurationSeconds(maxGeneratedFrames = 362, maxDurationSeconds = 15) {
    ...
    for (let duration = upperBound; duration >= H3_CONTINUUM_V38_MIN_DURATION_SECONDS; duration -= 1) {
        if (continuumV38SampledFrameCountForSeconds(duration) <= frameBudget)
            return duration;
    }
    return H3_CONTINUUM_V38_MIN_DURATION_SECONDS;
}
export function continuumVisibleFrameCountForTask(task) {
    return Math.max(1, continuumSampledFrameCountForSeconds(task.duration) - H3_CONTINUUM_CONTEXT_FRAMES);
}
export function continuumMaxDurationSeconds(maxGeneratedFrames = 362, contextFrames = H3_CONTINUUM_CONTEXT_FRAMES, maxDurationSeconds = 15) {
    const frameBudget = Number.isFinite(maxGeneratedFrames)
        ? Math.max(1, Math.floor(maxGeneratedFrames))
        : 362;
    const upperBound = Number.isFinite(maxDurationSeconds)
    ...
    for (let duration = upperBound; duration >= 1; duration -= 1) {
        if (continuumSampledFrameCountForSeconds(duration, contextFrames) <= frameBudget) {
            return duration;
        }
    }
    return 1;
...
export function workflowSupportsH3ContinuumExtension(source) {
    if (!source || typeof source !== "object" || Array.isArray(source))
        return false;
    const serialized = JSON.stringify(source);
    const classTypes = new Set(Object.values(source).flatMap((node) => {
 ...
 */
export function workflowSupportsH3ContinuumManagedExtension(source) {
    if (!source || typeof source !== "object" || Array.isArray(source))
        return false;
    const serialized = JSON.stringify(source);
    const classTypes = new Set(Object.values(source).flatMap((node) => {
...
export function continuumFirstFrameTask(task) {
    ...
    if (!source)
        return task;
    if (source.resolution === 1080 || source.resolution === 1440)
        throw new Error("Continuum 首帧来源不支持原生高分辨率二次采样档。");
    return { ...task, ...source, resolution: source.resolution };
}
export function continuumTimelinePromptForTask(task, _chunkSeconds) {
...
export function activityTimeoutMinutesForTask(task, ltxExtensionTimeoutMinutes) {
    ...
    if (task.modelId === "seedvr2-native-int8")
        return 90;
    if (task.modelId === "dlss5-sr" || task.modelId === "dlss5-konohamaru")
        return 90;
...
export function extensionSafetyForTask(task, locale = "zh-CN") {
    ...
    if (isMiniMaxH3ContinuumModel(task.modelId)) {
        ...
        const maxDurationSeconds = continuumV38
            ? continuumV38MaxDurationSeconds(task.maxGeneratedFrames, generationSafety.maxDurationSeconds)
            : continuumMaxDurationSeconds(task.maxGeneratedFrames, contextFrames, generationSafety.maxDurationSeconds);
        ...
        if (task.sourceVideoDuration > 0 &&
            task.trimEndSeconds + 0.05 < task.sourceVideoDuration) {
            return result(false, message("continuumEndRequired"));
        }
        ...
        if (continuumV38 && task.duration < H3_CONTINUUM_V38_MIN_DURATION_SECONDS) {
            return result(false, message("continuumDurationMinimum", {
        ...
        if (sampledFrames > task.maxGeneratedFrames || task.duration > maxDurationSeconds) {
            return result(false, message("continuumBudget", {
                sampledFrames,
                maxGeneratedFrames: task.maxGeneratedFrames
            }));
        }
        return result(true, continuumV38
...
export function renderWorkflow(source, task, context = {}) {
    const h3LatentSaveMode = h3LatentSaveModeFor(task, task.taskType === "extension" && isMiniMaxH3R2vModel(task.modelId));
    ...
    const tokens = {
        PROMPT: videoPromptForLoras(continuumManaged
            ? continuumTimelinePromptForTask(task, task.h3ContinuumSequence?.chunkSeconds ?? task.duration)
            : task.prompt, task.videoLoras),
        NEGATIVE_PROMPT: "",
        SEED: task.seed,
        ...
        H3_AV_SOURCE_FRAME_INDEX: context.h3AvSourceFrameIndex ?? (continuumV38 && !continuumManaged
        ...
        FRAMES: context.frames ?? (task.taskType === "extension" && isMiniMaxH3ContinuumModel(task.modelId)
            ? continuumSampledFrames
    ...
    if (isMiniMaxH3Model(task.modelId)) {
        ...
        if (outputNodeTypes.size) {
            for (const [nodeId, node] of Object.entries(workflow)) {
                if (node.class_type && outputNodeTypes.has(node.class_type))
    ...
    if (isMiniMaxH3Model(task.modelId) && task.videoLoras?.some((lora) => isH3PddLoraId(lora.id) && videoLoraCompatibleWithModel(lora, task.modelId))) {
