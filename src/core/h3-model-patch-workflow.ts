import type { Draft, H3ExecutionPolicySnapshot, H3SpectrumModelAwareMode, UiLocale } from "../types.js";
import {
  h3AttentionOwnerFor,
  resolveH3ExecutionPolicy
} from "./h3-execution-policy.js";
...
export interface NormalizeMiniMaxH3ModelPatchChainOptions {
  ...
  sparseAttentionMode?: string;
  runtimeMode?: string;
  comfyCompilerMode?: string;
...
function requiredModelLink(value: unknown, locale: UiLocale): ModelLink {
  const result = modelLink(value);
  if (result) return result;
  throw new Error(message("h3PatchChainUnknown", {}, locale));
}

function attentionOwnerForNode(node: H3ModelPatchWorkflowNode): ReturnType<typeof h3AttentionOwnerFor> | null {
  if (node.class_type === "PathchSageAttentionKJ") return "sage";
  if (node.class_type === "H3SLAAttention") return "sla";
  if (node.class_type === "H3SparseAttention" || node.class_type === "H3SparseAttentionAdvanced" || node.class_type === "BlockSparseAttention") return "h3-sparse";
...
function inspectModelChain(
  ...
  finalInput: unknown,
  locale: UiLocale
): { attentionOwners: Array<{ nodeId: string; owner: ReturnType<typeof h3AttentionOwnerFor> }> } {
  let current = requiredModelLink(finalInput, locale);
  const visited = new Set<string>();
  const attentionOwners: Array<{ nodeId: string; owner: ReturnType<typeof h3AttentionOwnerFor> }> = [];
  while (true) {
    const [nodeId] = current;
    if (visited.has(nodeId)) throw new Error(message("h3PatchChainCycle", {}, locale));
    visited.add(nodeId);
    const node = workflow[nodeId];
    if (!node?.class_type) throw new Error(message("h3PatchChainUnknown", {}, locale));
    const owner = attentionOwnerForNode(node);
    if (owner) attentionOwners.push({ nodeId, owner });
    if (!knownModelPatchClasses.has(node.class_type)) return { attentionOwners };
    current = requiredModelLink(node.inputs?.model, locale);
  }
}

function existingNodeIds(workflow: H3ModelPatchWorkflow, classType: string, locale: UiLocale): string[] {
  const ids = Object.entries(workflow)
    .filter(([, node]) => node.class_type === classType)
    .map(([id]) => id);
  if (ids.length > 1) throw new Error(message("h3PatchDuplicate", { nodeType: classType }, locale));
  return ids;
}
...
function bypassManagedLink(
  ...
  if (!managedIds.has(link[0])) return link;
...
function replaceManagedReferences(workflow: H3ModelPatchWorkflow, managedIds: Set<string>, locale: UiLocale): void {
...
function spectrumInputs(model: ModelLink, modelAwareMode: H3SpectrumModelAwareMode): Record<string, unknown> {
  return {
    model,
    enabled: true,
    blend_weight: 0.5,
...
function sparseInputs(model: ModelLink, mode: "sol-attn" | "native-sla" | "vsa"): Record<string, unknown> {
...
function assertConsumers(
  workflow: H3ModelPatchWorkflow,
  locale: UiLocale,
  needsModelChain: boolean
): Array<[string, H3ModelPatchWorkflowNode]> {
  const consumers = Object.entries(workflow).filter(([, node]) =>
    consumerClasses.has(node.class_type ?? "") && Array.isArray(node.inputs?.model)
  );
  if (!consumers.length && needsModelChain) throw new Error(message("h3PatchConsumersMissing", {}, locale));
  if (needsModelChain && (
...
export function normalizeMiniMaxH3ModelPatchChain(
  ...
  if (!consumers.length) return h3ExecutionPolicySnapshotForPolicy(policy, spectrumEnabled, previewEnabled);
  ...
  if (owners.length > 1) throw new Error(message("h3AttentionConflict", {}, locale));
...
function isH3ChainPolicyActive(policy: ReturnType<typeof resolveH3ExecutionPolicy>): boolean {
  return policy.attentionMode !== "pytorch" || policy.sparseAttentionMode !== "off" || policy.spectrumEnabled || policy.previewEnabled;
}

function h3ExecutionPolicySnapshotForPolicy(
  policy: ReturnType<typeof resolveH3ExecutionPolicy>,
  spectrumEnabled: boolean,
  previewEnabled: boolean
): H3ExecutionPolicySnapshot {
  return {
