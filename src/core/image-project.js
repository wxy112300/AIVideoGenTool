...
function isCorruptedImagePrompt(value) {
...
function normalizeImageCrop(value) {
    ...
    if (typeof source.documentPath !== "string" || !source.documentPath.trim() ||
        typeof source.croppedPath !== "string" || !source.croppedPath.trim())
        return undefined;
    return {
        x,
        y,
...
function normalizeImageAssetVersion(value, fallbackCreatedAt) {
    if (!value || typeof value !== "object")
        return null;
    const source = value;
    ...
    return {
        ...
        format: imageOutputFormats.includes(source.format)
            ? source.format
...
function normalizeImageHistoryProject(value) {
    ...
    const orderedVersions = validVersionNumbers
        ...
        : [...versions]
            ...
            .reverse();
    const largestVersionNumber = orderedVersions.reduce((largest, version) => Math.max(largest, version.versionNumber), 0);
    ...
    return {
        ...
        ...(typeof source.coverVersionId === "string" && source.coverVersionId.trim()
            ...
            : {}),
        nextVersionNumber: Math.max(largestVersionNumber + 1, normalizedInteger(source.nextVersionNumber, largestVersionNumber + 1, 1)),
...
function normalizeImageReference(value, index) {
    ...
    const role = imageReferenceRoles.includes(source.role)
        ? source.role
        : undefined;
...
export function normalizeImageReferences(values) {
 ...
 */
export function renumberImageReferences(values) {
    return [...values]
...
export function normalizeImageHistory(value) {
...
export function normalizeImageEditDraft(value) {
    ...
    const storedPromptVersions = Array.isArray(source.promptVersions) && source.promptVersions.length
        ? source.promptVersions
            .filter((version) => version && typeof version.id === "string" && typeof version.text === "string")
...
export function nextImagePictureNumber(draft) {
    ...
    const usedNumbers = new Set(draft.pictures
...
export function imageEditDraftFromQueueTask(task, currentDraft) {
...
export function imageEditPicturesForVersion(version) {
...
function normalizedLineagePath(value) {
 ...
 */
export function findImageProjectLineage(projects, basePicture) {
    const hash = basePicture.contentHash?.trim().toLowerCase();
    const paths = new Set([basePicture.absolutePath, basePicture.originalPath]
...
export function expandImageSeeds(seed, outputCount, randomSeed = () => {
...
export function nextImageVersionNumber(project) {
    const largestExisting = project.versions.reduce((largest, version) => Math.max(largest, version.versionNumber), 0);
    return Math.max(1, project.nextVersionNumber, largestExisting + 1);
}
export function imageProjectCoverVersion(project) {
