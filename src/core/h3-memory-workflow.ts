...
import {
  normalizeMiniMaxH3ModelPatchChain as normalizeNativeMiniMaxH3ModelPatchChain,
  type H3ModelPatchWorkflow,
  type H3ModelPatchWorkflowNode,
  type NormalizeMiniMaxH3ModelPatchChainOptions as NativeNormalizeOptions
} from "./h3-model-patch-workflow.js";
import { workflowMessage } from "./runtime/workflow-messages.js";

export type { H3ModelPatchWorkflow, H3ModelPatchWorkflowNode } from "./h3-model-patch-workflow.js";
 ...
 * memory fields are deliberately ignored here and can never add a node back
 * into a rendered graph. New callers should import the native 0.35 patcher.
 */
...
function assertNoMixedAttentionOwners(
  workflow: H3ModelPatchWorkflow,
  locale: UiLocale
): void {
...
function consumersFor(workflow: H3ModelPatchWorkflow): Array<[string, H3ModelPatchWorkflowNode]> {
  return Object.entries(workflow).filter(([, node]) =>
    consumerClasses.has(node.class_type ?? "") && Array.isArray(node.inputs?.model)
  );
}

function appendPostSamplingWrappers(
  workflow: H3ModelPatchWorkflow,
...
export function normalizeMiniMaxH3ModelPatchChain(
  ...
  const nativeOptions: NativeNormalizeOptions = {
  ...
  normalizeNativeMiniMaxH3ModelPatchChain(workflow, nativeOptions);
}
