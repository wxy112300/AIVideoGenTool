...
export function h3CommunityPromptWriterContract(mode, preset = "official-storyboard") {
    const referenceRules = mode === "R2V"
        ? [
            ...
            preset === "detailed-cinematic"
                ? "R2V detail budget: for a simple reference-generation task, 350-500 grounded English words in detailed_description is only a starting point. In this detailed preset, go beyond it when each added sentence grounds a causal action, reaction, camera, visible change, continuity relation, or sound event; state static reference facts once instead of repeating them; never pad to reach a target."
    ...
    return [
        ...
        preset === "detailed-cinematic"
            ? "Detailed-expansion factual boundary: preserve every fact, user-written action, and action order from the user's brief and supplied references. You may add reasonable, scene-grounded operational details that make the requested result executable, including ordinary micro-actions, anticipation, gaze or expression, body mechanics, contact, weight or momentum, and motivated camera behavior; add focus or light changes, ambience, or synchronized sound only when caused by the action or explicitly requested. Never collapse multiple user actions into a generic summary. If output space is tight, remove repeated reference inventory and assistant-added detail before any user-required action, camera instruction, reaction, dialogue, or constraint. Do not introduce new characters, locations, important props, plot events, visible text, dialogue, music, or major appearance changes, and never override an explicit constraint."
...
export function h3SmallModelPromptContract(mode, preset = "official-storyboard") {
    ...
    const detailedExpansionRules = preset === "detailed-cinematic"
        ? [
            ...
            "Detailed expansion budget rule: there is no fixed 350-500 or 500-word target for this preset. Spend extra length on user-requested action, interactions, reactions, camera path, timing, continuity, dialogue, and causally synchronized sound; state reference appearance once and remove repeated inventories, generic adjectives, decorative atmosphere, and other filler before removing a necessary user action, camera instruction, reaction, dialogue, or constraint."
        ]
        : [];
    return [
        ...
        "Use a silent four-pass workflow: extract the user's concrete requirements; plan causal action phases across the supplied duration without an equal-time grid; render the required H3 fields; audit before answering.",
