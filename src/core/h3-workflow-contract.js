...
const RUNTIME_NODE_REQUIREMENTS = {
    ...
    LocalVideoStudioH3ContinuumSamplerV38: {
        inputs: [
            ...
            { name: "chunks", type: "ANY" },
            { name: "chunk_seconds", type: "ANY" },
            { name: "initial_state", type: "H3_CONTINUUM_STATE" }
        ]
    },
...
function addMissingClasses(errors, classes, available) {
    for (const classType of classes) {
        if (!available.has(classType))
            errors.push(`缺少节点 class_type=${classType}`);
    }
}
...
function validateFirstPass(nodes, errors) {
    ...
    if (!available.has("LocalVideoStudioH3SaveJointAV"))
        return;
...
function requireOutputReference(errors, nodes, classType, inputName, expectedClass, outputIndex) {
    const nodeId = nodeIdsForClass(nodes, classType)[0];
    const value = nodeId ? inputsFor(nodes.get(nodeId))[inputName] : undefined;
    const reference = nodeRef(value);
...
function requireOutputReferenceAtNodeId(errors, nodes, nodeId, classType, inputName, expectedClass, outputIndex) {
    const value = nodeId ? inputsFor(nodes.get(nodeId))[inputName] : undefined;
    const reference = nodeRef(value);
    if (!reference || !nodes.has(reference.nodeId)) {
        errors.push(`${classType}.${inputName} 必须引用已存在的 workflow 节点`);
...
function nodeIdReferencing(nodes, classType, inputName, sourceNodeId, outputIndex) {
    if (!sourceNodeId)
        return undefined;
...
function validateContinuum(nodes, errors) {
    const available = classTypes(nodes);
    ...
    requireOutputReference(errors, nodes, "LocalVideoStudioH3ArtifactToContinuumState", "joint_av", "LocalVideoStudioH3LoadJointAV", 0);
...
function validateContinuumV38(nodes, errors) {
    const available = classTypes(nodes);
    ...
    const bridgeId = nodeIdsForClass(nodes, "LocalVideoStudioH3ArtifactToContinuumState")[0];
    requireOutputReferenceAtNodeId(errors, nodes, bridgeId, "LocalVideoStudioH3ArtifactToContinuumState", "joint_av", "LocalVideoStudioH3LoadJointAV", 0);
    const samplerType = "LocalVideoStudioH3ContinuumSamplerV38";
    ...
    if (samplerId) {
        ...
        if (inputs.sequence_prompt !== "{{PROMPT}}") {
            errors.push(`${samplerType}.sequence_prompt 必须保留 PROMPT 占位符`);
        }
        if (inputs.chunks !== "{{H3_CONTINUUM_CHUNKS}}") {
            errors.push(`${samplerType}.chunks 必须保留 H3_CONTINUUM_CHUNKS 占位符`);
        }
        if (inputs.chunk_seconds !== "{{H3_CONTINUUM_CHUNK_SECONDS}}") {
            errors.push(`${samplerType}.chunk_seconds 必须保留 H3_CONTINUUM_CHUNK_SECONDS 占位符`);
        }
        if (inputs.width !== "{{WIDTH}}" || inputs.height !== "{{HEIGHT}}") {
            errors.push(`${samplerType}.width/height 必须保留 WIDTH/HEIGHT 占位符`);
        }
        ...
        requireOutputReferenceAtNodeId(errors, nodes, samplerId, samplerType, "initial_state", "LocalVideoStudioH3ArtifactToContinuumState", 0);
        if ("first_frame" in inputs || "reference_video_1" in inputs) {
            errors.push(`${samplerType} 使用 native state 时不能重复连接同源 First Image 或 Video Guide`);
        }
    }
    ...
    if (generatedDecodeId === undefined) {
        errors.push(`VAEDecode.samples 必须有一个节点引用 ${samplerType} 的 video latent output 0`);
    }
    if (audioDecodeId === undefined) {
        errors.push(`VAEDecodeAudio.samples 必须引用 ${samplerType} 的 audio latent output 1`);
    }
    if (finalizeId) {
    ...
    requireOutputReferenceAtNodeId(errors, nodes, diagnosticsId, "LocalVideoStudioH3ContinuumDiagnostics", "bridge_report", "LocalVideoStudioH3ArtifactToContinuumState", 1);
    requireOutputReferenceAtNodeId(errors, nodes, diagnosticsId, "LocalVideoStudioH3ContinuumDiagnostics", "status", samplerType, 3);
    requireOutputReferenceAtNodeId(errors, nodes, diagnosticsId, "LocalVideoStudioH3ContinuumDiagnostics", "assembly_plan", samplerType, 2);
    requireOutputReferenceAtNodeId(errors, nodes, diagnosticsId, "LocalVideoStudioH3ContinuumDiagnostics", "assembly_report", "H3ContinuumAssembleSeamV35", 2);
    if (diagnosticsId) {
    ...
    requireOutputReferenceAtNodeId(errors, nodes, concatId, "LTXVConcatAVLatent", "video_latent", samplerType, 0);
    requireOutputReferenceAtNodeId(errors, nodes, concatId, "LTXVConcatAVLatent", "audio_latent", samplerType, 1);
    const serializerId = nodeIdsForClass(nodes, "LocalVideoStudioH3SaveJointAV")[0];
...
function validateContinuumManagedV38(nodes, errors) {
    const available = classTypes(nodes);
    ...
    for (const legacyClass of [
        ...
        if (available.has(legacyClass))
            errors.push(`managed workflow 不得包含 legacy bridge/saver：${legacyClass}`);
    }
    ...
    if (samplerId) {
        ...
        if (inputs.chunk_seconds !== "{{H3_CONTINUUM_CHUNK_SECONDS}}")
            errors.push(`${samplerType}.chunk_seconds 必须保留 H3_CONTINUUM_CHUNK_SECONDS 占位符`);
        if (inputs.run_storage !== "Save + Auto Resume")
            errors.push(`${samplerType}.run_storage 必须固定为 Save + Auto Resume`);
        if (inputs.generation_mode !== "Review Each Chunk")
            errors.push(`${samplerType}.generation_mode 必须固定为 Review Each Chunk`);
        if (inputs.prompt_mode !== "Timeline")
            errors.push(`${samplerType}.prompt_mode 必须固定为 Timeline`);
        if (inputs.run_name !== "{{H3_CONTINUUM_RUN_NAME}}")
            errors.push(`${samplerType}.run_name 必须保留 managed run-name 占位符`);
        if (inputs.project_id !== "{{H3_CONTINUUM_PROJECT_ID}}")
            errors.push(`${samplerType}.project_id 必须保留 managed project-id 占位符`);
        if (inputs.review_action !== "{{H3_CONTINUUM_REVIEW_ACTION}}")
            errors.push(`${samplerType}.review_action 必须保留 review-action 占位符`);
        if ("initial_state" in inputs)
            errors.push(`${samplerType} managed workflow 不得包含 initial_state`);
        requireOutputReferenceAtNodeId(errors, nodes, samplerId, samplerType, "model", "PathchSageAttentionKJ", 0);
        requireOutputReferenceAtNodeId(errors, nodes, samplerId, samplerType, "clip", "CLIPLoader", 0);
        requireOutputReferenceAtNodeId(errors, nodes, samplerId, samplerType, "video_vae", "VAELoader", 0);
        requireOutputReferenceAtNodeId(errors, nodes, samplerId, samplerType, "sampler", "KSamplerSelect", 0);
    ...
    requireOutputReferenceAtNodeId(errors, nodes, finalizeId, "H3ContinuumAssembleSeamV35", "audio", "VAEDecodeAudio", 0);
    requireOutputReferenceAtNodeId(errors, nodes, finalizeId, "H3ContinuumAssembleSeamV35", "assembly_plan", samplerType, 2);
    if (!generatedDecodeId)
        errors.push(`VAEDecode.samples 必须引用 ${samplerType} 的 video latent output 0`);
    if (!audioDecodeId)
        errors.push(`VAEDecodeAudio.samples 必须引用 ${samplerType} 的 audio latent output 1`);
...
function validateSecondPass(nodes, errors) {
    ...
    if (upscalerClass && available.has("LTXVSeparateAVLatent")) {
        requireInputReference(errors, nodes, upscalerClass, upscalerClass === "MinimaxH3LatentUpscaler3D" ? "latent" : "samples", "LTXVSeparateAVLatent");
    }
...
export function h3ComfyAvWorkflowKind(source) {
    ...
    if (classes.has("H3ContinuumSamplerV38") &&
    ...
    if (classes.has("LocalVideoStudioH3LoadJointAV") ||
...
export function validateH3ComfyWorkflow(source) {
    ...
    if (!kind)
        return { valid: true, kind: null, errors: [] };
    const errors = [];
    if (kind === "first-pass-av")
        validateFirstPass(nodes, errors);
    else if (kind === "continuum-managed-extension")
        validateContinuumManagedV38(nodes, errors);
    else if (kind === "continuum-extension") {
        if (classTypes(nodes).has("H3ContinuumSamplerV38") ||
            classTypes(nodes).has("LocalVideoStudioH3ContinuumSamplerV38"))
            validateContinuumV38(nodes, errors);
        else
            validateContinuum(nodes, errors);
    }
    else
        validateSecondPass(nodes, errors);
...
function inputSpecFor(node, inputName) {
    ...
    for (const groupName of ["required", "optional", "hidden"]) {
        ...
        if (!isRecord(group))
            continue;
        if (inputName in group)
            return group[inputName];
    }
    return undefined;
}
...
function isStringComboSpec(spec) {
...
function runtimeTypeMatches(spec, expected) {
    ...
    if (expected === "STRING" && isStringComboSpec(spec))
        return true;
    return actual === expected;
}
function outputTypesFor(node) {
    if (!isRecord(node) || !Array.isArray(node.output))
        return [];
    return node.output.filter((value) => typeof value === "string")
        .map((value) => value.toUpperCase());
}
function runtimeNode(source, classType) {
    if (!isRecord(source))
        return undefined;
...
export function h3ComfyWorkflowRuntimeIssues(workflow, objectInfo) {
    ...
    if (!isRecord(objectInfo))
        return ["/object_info 响应无效，无法验证 H3 AV 节点 schema"];
    ...
    const continuumV38 = continuum && !continuumManaged && (workflowClassTypes.has("H3ContinuumSamplerV38") ||
...
function dynamicOptionKeys(spec) {
    ...
    if (!Array.isArray(options))
        return [];
    return options.flatMap((option) => isRecord(option) && typeof option.key === "string" ? [option.key] : []);
}
function runtimeInputIssue(issues, objectInfo, classType, inputName, expected) {
 ...
 */
export function h3ExecutionNodeRuntimeIssues(workflow, objectInfo) {
    const nodes = graphNodes(workflow);
    if (!isRecord(objectInfo))
        return ["/object_info 响应无效，无法验证 H3 执行节点 schema"];
    const issues = [];
    const objectInfoRecord = objectInfo;
    for (const [nodeId, node] of nodes) {
        const classType = node.class_type;
        if (classType !== "ModelAttentionBackend" && classType !== "BlockSparseAttention")
            continue;
        ...
        if (classType === "ModelAttentionBackend") {
            const attentionSpec = runtimeInputIssue(issues, objectInfoRecord, classType, "attention", "COMBO");
        ...
        const selectionSpec = runtimeInputIssue(issues, objectInfoRecord, classType, "selection", "ANY");
