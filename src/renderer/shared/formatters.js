...
export function interpolationMultiplier(value) {
    if (value === "rife2x")
        return 2;
    if (value === "rife4x")
        return 4;
...
export function frameRateSummary(fps, interpolation) {
...
export function formatElapsedDuration(seconds, t = createTranslator("zh-CN").t) {
    const rounded = Math.max(0, Math.round(seconds));
    const minutes = Math.floor(rounded / 60);
    return t(uiKeys.format.elapsed, { minutes, seconds: rounded % 60 });
}
export function formatUpscaleEstimateRange(minSeconds, maxSeconds, t = createTranslator("zh-CN").t) {
    const format = (seconds) => {
        const rounded = Math.max(1, Math.round(seconds));
        if (rounded < 60)
            return t(uiKeys.format.upscaleSeconds, { value: rounded });
        if (rounded < 3600)
            return t(uiKeys.format.upscaleMinutes, { value: Math.round(rounded / 60) });
...
export function formatFullHistoryTime(value) {
    const date = new Date(value);
    if (Number.isNaN(date.getTime()))
        return value;
...
export function historyRenderDuration(version, t = createTranslator("zh-CN").t) {
    if (!version.startedAt)
        return t(uiKeys.format.unknownDuration);
...
export function queueStageElapsedText(task, t = createTranslator("zh-CN").t) {
    if (!task.stageStartedAt)
        return t(uiKeys.format.stagePending);
...
export function queueEstimateText(seconds, t = createTranslator("zh-CN").t) {
...
export function elapsedText(startedAt, t = createTranslator("zh-CN").t) {
    if (!startedAt)
        return t(uiKeys.format.waitingTimer);
    const seconds = Math.max(0, Math.floor((Date.now() - Date.parse(startedAt)) / 1000));
    const minutes = Math.floor(seconds / 60);
    return t(uiKeys.format.running, { value: formatElapsedDuration(seconds, t) });
}
export function performanceCard(label, id, value, suffix, detail = "") {
