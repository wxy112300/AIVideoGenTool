...
export function renderUpscaleDialog(options) {
    ...
    const [targetWidth, outputHeight] = options.upscaleDimensions(version.width, version.height, dialog.targetHeight);
    ...
    const estimatedTime = options.formatUpscaleEstimateRange(estimate.secondsMin, estimate.secondsMax);
    ...
    const profiles = options.environment?.modelProfiles
    ...
    const outputFilename = options.createUpscaleFilename(version.outputFilename, dialog.targetHeight);
      ...
      <section class="upscale-dialog" role="dialog" aria-modal="true" aria-labelledby="upscale-title" tabindex="-1">
        ...
        <div class="upscale-dialog-body">
          ...
          <div><label>${t(uiKeys.upscale.targetResolution)}</label><div class="upscale-resolution">
            ${[720, 1080, 1440, 2160].map((height) => `<button class="${height === selectedTargetHeight ? "primary" : "secondary"}" data-upscale-height="${height}" ${height <= sourceShortEdge ? "disabled" : ""}>${height === 2160 ? "4K" : `${height}p`}</button>`).join("")}
          </div></div>
