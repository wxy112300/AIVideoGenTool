...
import { createAetherScaleUpscaleFilename, createDlss5UpscaleFilename, createKonohamaruUpscaleFilename, h3NativeUpscaleDimensions } from "../../core/upscale";
...
import { KONOHAMARU_DEFAULT_MODE, KONOHAMARU_DEFAULT_NR_STYLE, KONOHAMARU_MODEL_ID, KONOHAMARU_MODE_SPECS, KONOHAMARU_NR_STYLES, isKonohamaruFrameOutputFps, isKonohamaruMode, isKonohamaruNrStyle, konohamaruFrameFpsOptions, konohamaruOutputGeometry } from "../../core/konohamaru-dlss5";
...
function konohamaruUiStatus(environment, t) {
    if (!environment) {
        return {
            tone: "warning",
            available: true,
...
function sourceVideoSizeBytes(version) {
...
function estimateUpscaleDiskBytes(version, targetWidth, targetHeight) {
    ...
    return Math.max(sourceSizeBytes, Math.ceil(sourceSizeBytes * targetPixels / sourcePixels));
}
/**
 ...
 */
export function estimateKonohamaruDiskBytes(version, targetWidth, targetHeight, outputFps) {
    ...
    const duration = Number.isFinite(version.duration) && version.duration > 0
        ? version.duration
        : 0;
    if (!safeWidth || !safeHeight || !safeFps || !duration)
        return null;
    // Matches the upstream Auto H.264 target: pixels × FPS × 16 / 165888.
    const videoBitrateKbps = safeWidth * safeHeight * safeFps * 16 / 165888;
    const videoBytes = videoBitrateKbps * 1000 / 8 * duration;
    if (!Number.isFinite(videoBytes) || videoBytes <= 0)
        return null;
    const sourceSizeBytes = sourceVideoSizeBytes(version) ?? 0;
    const audioAndContainerAllowance = Math.min(16 * 1024 * 1024, Math.max(256 * 1024, sourceSizeBytes * 0.08));
...
export function renderUpscaleDialog(options) {
    ...
    const selectedKonohamaruStyle = requestedKonohamaruStyle;
    const selectedKonohamaruIntensity = typeof dialog.konohamaruNrIntensity === "number" &&
        ...
        : 1;
    const konohamaruFrameOptions = isKonohamaruSelected
        ? konohamaruFrameFpsOptions(version.fps)
        : ["off"];
    const requestedKonohamaruFrame = dialog.konohamaruFrameInterpolation;
    const selectedKonohamaruFrame = requestedKonohamaruFrame === "off"
        ? "off"
    ...
    const konohamaruOutputFrames = Math.max(1, Math.ceil(Math.max(0, version.duration) * konohamaruOutputFps));
    let konohamaruGeometry = null;
    let konohamaruGeometryError = "";
    if (isKonohamaruSelected) {
        try {
            konohamaruGeometry = konohamaruOutputGeometry(version.width, version.height, selectedKonohamaruMode);
        }
        catch (error) {
