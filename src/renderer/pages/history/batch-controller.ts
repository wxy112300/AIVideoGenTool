...
import {
  historyTagChipMarkup,
  mountHistoryTagEditor,
  openHistoryTagEditor
} from "./tags-controller";
import { renderIcons } from "../../shared/icons";

export interface HistoryBatchControllerOptions {
  getKind(): HistoryKind;
  ...
  clearSelection(): void;
  requestDelete(kind: HistoryKind, assetIds: string[]): void;
}
...
function visibleItems(context: RendererContext, options: HistoryBatchControllerOptions): HistoryItem[] {
...
function selectedItems(context: RendererContext, options: HistoryBatchControllerOptions): HistoryItem[] {
  const state = context.getState();
  if (!state) return [];
...
function batchTagChipOptions(context: RendererContext) {
  return {
...
function syncBatchTagsPanel(
  context: RendererContext,
  options: HistoryBatchControllerOptions
): void {
  const panel = context.root.querySelector<HTMLElement>("[data-history-batch-tags-root]");
  if (!panel) return;
  const items = selectedItems(context, options);
  const list = panel.querySelector<HTMLElement>("[data-history-tag-list]");
  if (list) {
  ...
  if (!items.length) panel.hidden = true;
}
...
function syncSelectionDom(
  ...
  context.root.querySelectorAll<HTMLElement>("[data-history]").forEach((card) => {
    ...
    const checkbox = card.querySelector<HTMLInputElement>("[data-history-batch-select]");
    if (checkbox) checkbox.checked = selected;
  });
  const count = context.root.querySelector<HTMLElement>("[data-history-batch-count]");
  if (count) count.textContent = context.t("history.batch.selected", {
    selected: selectedIds.length,
    total: visible.length
  });
  const selectAll = context.root.querySelector<HTMLButtonElement>("[data-history-batch-action=select-all]");
  if (selectAll) {
  ...
  context.root.querySelectorAll<HTMLButtonElement>("[data-history-batch-action=copy], [data-history-batch-action=tags], [data-history-batch-action=delete]").forEach((button) => {
    button.disabled = selectedIds.length === 0 || options.isBusy();
  });
  syncBatchTagsPanel(context, options);
}

export function mountHistoryBatchController(
  context: RendererContext,
  options: HistoryBatchControllerOptions
): RendererCleanup {
  ...
  let pendingCheckboxClick: { assetId: string; shiftKey: boolean } | null = null;

  const setMode = (enabled: boolean): void => {
    if (enabled === options.isBatchMode()) return;
  ...
  const selectSelection = (assetId: string, shiftKey = false): void => {
  ...
  const handleBatchAction = (action: string): void => {
    if (action === "exit") {
      setMode(false);
      return;
    }
    ...
    const selected = pruneHistoryBatchSelection(
      options.getSelectedIds(),
      visibleItems(context, options).map((item) => item.id)
    );
    if (!selected.length || options.isBusy()) return;
    if (action === "tags") {
      options.setTagsPanelOpen(true);
      const tagsRoot = root.querySelector<HTMLElement>("[data-history-batch-tags-root]");
      if (tagsRoot) {
  ...
  root.addEventListener("click", (event) => {
    if (!options.isBatchMode()) {
      const toggle = (event.target instanceof Element)
        ? event.target.closest<HTMLElement>("[data-history-batch-toggle]")
        : null;
      if (toggle) {
        stop(event);
        setMode(true);
    ...
    const toggle = target?.closest<HTMLElement>("[data-history-batch-toggle]");
    if (toggle) {
      stop(event);
      setMode(false);
      return;
    }
    const action = target?.closest<HTMLElement>("[data-history-batch-action]")?.dataset.historyBatchAction;
    if (action) {
      stop(event);
    ...
    const checkbox = target?.closest<HTMLInputElement>("[data-history-batch-select]");
    if (checkbox) {
    ...
    const checkboxWrap = target?.closest<HTMLElement>(".history-batch-checkbox-wrap");
    if (checkboxWrap) {
      const input = checkboxWrap.querySelector<HTMLInputElement>("[data-history-batch-select]");
      const assetId = input?.dataset.historyBatchSelect;
      pendingCheckboxClick = assetId ? { assetId, shiftKey: event.shiftKey } : null;
      event.stopImmediatePropagation();
      return;
    }
    const card = target?.closest<HTMLElement>("[data-open-history], [data-open-image-history]");
    if (!card || target?.closest(
      ".history-media-badges, [data-history-curation], .history-detail-curation, .history-card-more, .history-preview-progress, [data-image-media-retry], [data-image-media-locate], .history-batch-checkbox-wrap, button, input, select, textarea, a"
    )) return;
  ...
  root.addEventListener("change", (event) => {
    ...
    const target = event.target instanceof HTMLInputElement
      ? event.target.closest<HTMLInputElement>("[data-history-batch-select]")
      : null;
    if (!target) return;
    event.stopImmediatePropagation();
  ...
  root.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Enter") {
      stop(event);
      const assetId = card.dataset.openHistory ?? card.dataset.openImageHistory;
      if (assetId) selectSelection(assetId, event.shiftKey);
  ...
  root.addEventListener("keyup", (event) => {
    if (!options.isBatchMode() || !(event.key === " " || event.key === "Spacebar")) return;
    const target = event.target instanceof Element ? event.target : null;
    const card = target?.closest<HTMLElement>("[data-open-history], [data-open-image-history]");
    if (!card || target !== card) return;
    stop(event);
  ...
  const closeTagsPanel = (): void => {
    options.setTagsPanelOpen(false);
    const panel = root.querySelector<HTMLElement>("[data-history-batch-tags-root]");
    if (panel) panel.hidden = true;
  };
  tagsRoot?.querySelector<HTMLElement>("[data-history-batch-tags-close]")?.addEventListener("click", (event) => {
    stop(event);
    closeTagsPanel();
  }, { signal });
  const tagEditorCleanup = tagsRoot
    ? mountHistoryTagEditor(context, tagsRoot, {
        ...
        applyOperation: async (operation: HistoryBatchTagOperation) => {
          const items = selectedItems(context, options);
          const updates: HistoryMetadataBatchUpdate[] = items.map((item) => ({
            assetId: item.id,
            patch: {
          ...
          if (updates.length) options.setState(await context.application.updateHistoryMetadataBatch(updates));
        },
        onCommitted: () => {
          syncBatchTagsPanel(context, options);
          syncSelectionDom(context, options);
