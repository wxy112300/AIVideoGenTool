import { assignHistoryMasonryColumns, historyCardsByOrder, historyAlbumColumnCount, historyMasonryColumnCount } from "./helpers";
export function historyScrollAnchorIsValid(snapshot, historyKind, layout, filterSignature, assetExists) {
...
export function clampHistoryScrollPosition(scrollY, documentHeight, viewportHeight) {
...
export function createHistoryLayoutController(context, reportUserAction, getHistoryFilterSignature = () => "") {
    let layout = "masonry";
    let scrollPosition = 0;
    let historyScrollSnapshot = null;
    let renderedHistoryFilterSignature = getHistoryFilterSignature();
    let scrollRestorePending = false;
    let layoutAnchor = null;
    let layoutRestoreFrame = null;
    ...
    let titleMeasureFrame = null;
    let titleEvents = null;
    const captureLayoutAnchor = () => {
        ...
        if (heading) {
            ...
            if (heading.getBoundingClientRect().top > stickyTop + 1)
                return null;
        }
        const cards = [...document.querySelectorAll(".history-gallery-item")];
        if (!cards.length)
            return null;
    ...
    const restoreLayoutAnchor = () => {
        ...
        layoutRestoreFrame = window.requestAnimationFrame(() => {
            layoutRestoreFrame = null;
            const card = [...document.querySelectorAll(".history-gallery-item")]
    ...
    const restoreScrollPosition = () => {
        ...
        scrollRestoreFrame = window.requestAnimationFrame(() => {
            ...
            const position = clampHistoryScrollPosition(snapshot?.scrollY ?? scrollPosition, documentHeight, window.innerHeight);
            ...
            scrollRestoreFrame = window.requestAnimationFrame(() => {
                ...
                const target = snapshot?.assetId
                    ? [...document.querySelectorAll(".history-gallery-item")]
                        .find((item) => item.dataset.history === snapshot.assetId)
                    : undefined;
                if (!snapshot || !target || !historyScrollAnchorIsValid(snapshot, currentRoute.historyKind, layout, currentFilterSignature, Boolean(target))) {
                ...
                const correctAnchor = () => {
                    if (context.getRoute().page !== "history" || !target.isConnected)
                        return;
                ...
                if (typeof ResizeObserver !== "undefined") {
                    ...
                    const gallery = target.closest(".history-gallery") ?? target;
                    ...
                    const observer = new ResizeObserver(() => {
                        ...
                        correctionUsed = true;
                        observer.disconnect();
                        if (scrollRestoreObserver === observer)
                            scrollRestoreObserver = null;
                        scrollRestoreFrame = window.requestAnimationFrame(() => {
                            scrollRestoreFrame = null;
                            correctAnchor();
    ...
    const bindMasonry = () => {
        const gallery = document.querySelector(".history-gallery.masonry");
        if (!gallery)
            return;
    ...
    const bindAlbum = () => {
        const gallery = document.querySelector(".history-gallery.album");
        if (!gallery)
            return;
    ...
    const bindImageHistoryViewer = () => {
        const stagePanel = document.querySelector(".image-history-stage-panel");
        const versionRail = document.querySelector(".image-history-version-rail");
        const versionList = document.querySelector(".image-history-version-list");
        if (!stagePanel || !versionRail || !versionList)
            return;
        const update = () => {
    ...
    const bindTitleMarquees = () => {
        const titles = [...document.querySelectorAll(".history-card-title, .history-detail-title")];
        if (!titles.length)
            return;
        ...
        const measureTitle = (title) => {
            if (events.signal.aborted || !title.isConnected)
                return;
            const text = title.querySelector(".history-card-title-track > span");
    ...
    return {
        ...
        bindViewportControls: () => {
            ...
            const backTop = document.querySelector("#history-back-top");
            const update = (capturePosition = true) => {
                if (capturePosition && context.getRoute().page === "history" && !scrollRestorePending)
                    scrollPosition = window.scrollY;
        ...
        switchLayout: (nextLayout) => {
            ...
            document.querySelectorAll("[data-history-layout]").forEach((button) => {
