...
export function renderImageHistoryPage(
  ...
  return `
    ...
    <section class="history-gallery ${viewModel.historyLayout}">
      ${projects.length === 0
        ? `<div class="empty panel"><h2>${options.t(uiKeys.history.card.imageEmptyTitle)}</h2><p>${options.t(uiKeys.history.card.imageEmptyDescription)}</p></div>`
        : cards}
    </section>`;
}
  ...
  return `
    ...
    <section class="history-gallery ${viewModel.historyLayout}">
      ${viewModel.state.history.length === 0
        ? `<div class="empty panel"><h2>${options.t(uiKeys.history.card.videoEmptyTitle)}</h2><p>${options.t(uiKeys.history.card.videoEmptyDescription)}</p></div>`
        : cards}
    </section>`;
}
  ...
  return `
    <div class="history-detail-back">
      ...
      <div class="history-detail-tools">
        ...
        <span class="history-detail-position" aria-label="${options.t(uiKeys.history.page.position, { current: historyIndex + 1, total: viewModel.state.history.length })}">${options.t(uiKeys.history.page.position, { current: historyIndex + 1, total: viewModel.state.history.length })}</span>
    ...
    <section class="history-detail-hero">
      ...
      <aside class="history-detail-sidebar">
        <section class="panel history-summary">
          ...
          ${retiredModel ? "" : `<button class="secondary button-with-icon" data-edit-history="${asset.id}" aria-label="${options.t(uiKeys.history.page.adjustInCreate)}" title="${options.t(uiKeys.history.page.adjustInCreate)}">${options.icon("sliders-horizontal")}${options.t(uiKeys.history.page.adjustInCreate)}</button>`}
          ${videoFile?.absolutePath ? `${retiredModel ? "" : `<button class="secondary button-with-icon" data-continue-history="${asset.id}" data-source-version="${version.id}" aria-label="${options.t(uiKeys.history.page.continueCreation)}" title="${options.t(uiKeys.history.page.continueCreation)}">${options.icon("video")}${options.t(uiKeys.history.page.continueCreation)}</button>`}<button class="secondary button-with-icon" data-copy-file="${options.escapeHtml(videoFile.absolutePath)}" aria-label="${options.t(uiKeys.history.menu.copyFile)}" title="${options.t(uiKeys.history.menu.copyFile)}">${options.icon("copy")}${options.t(uiKeys.history.menu.copyFile)}</button><button class="secondary button-with-icon history-file-action" data-show-file="${options.escapeHtml(videoFile.absolutePath)}" aria-label="${options.t(uiKeys.history.menu.openFolder)}" title="${options.t(uiKeys.history.menu.openFolder)}">${options.icon("folder-open")}${options.t(uiKeys.history.page.locateFile)}</button>` : ""}
            <button class="secondary button-with-icon" data-open-upscale ${videoFile?.absolutePath && options.versionShortEdge(version) < 2160 ? "" : "disabled"}>${options.icon("maximize-2")}${options.versionShortEdge(version) >= 2160 ? options.t(uiKeys.history.page.current4k) : options.t(uiKeys.history.page.improveResolution)}</button>
            <button class="secondary danger history-delete-button button-with-icon" data-delete-history="${asset.id}">${options.icon("trash-2")}${options.t(uiKeys.history.page.deleteVideoRecord)}</button>
          </div>
        </section>
