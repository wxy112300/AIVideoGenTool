...
export function renderHistoryHeading(viewModel, options) {
...
export function renderImageLightboxMarkup(viewModel, options) {
...
function formatPerformancePercent(value, t) {
    return value == null ? t(uiKeys.history.detail.unavailable) : `${Math.round(value)}%`;
}
function formatPerformanceBytes(value, formatBytes, t) {
    return value == null ? t(uiKeys.history.detail.unavailable) : formatBytes(value);
}
export function renderPerformanceStatsMarkup(stats, options) {
    if (!stats) {
...
export function renderVideoInputSnapshotMarkup(asset, options) {
    ...
    if (asset.inputMode === "video" || asset.sourceVideoPath) {
        ...
        if (asset.sourceAssetId)
            items.push(`<dt>${options.t(uiKeys.history.detail.sourceProject)}</dt><dd><code>${options.escapeHtml(asset.sourceAssetId)}</code></dd>`);
        if (asset.sourceVersionId)
            items.push(`<dt>${options.t(uiKeys.history.detail.sourceVersion)}</dt><dd><code>${options.escapeHtml(asset.sourceVersionId)}</code></dd>`);
    }
    else {
        ...
        if (asset.startImagePath)
            items.push(`<dt>${options.t(uiKeys.history.detail.firstFrame)}</dt><dd><code>${options.escapeHtml(asset.startImagePath)}</code></dd>`);
        if (asset.endImagePath)
            items.push(`<dt>${options.t(uiKeys.history.detail.lastFrame)}</dt><dd><code>${options.escapeHtml(asset.endImagePath)}</code></dd>`);
...
export function renderImageReferenceSnapshotMarkup(version, options) {
    if (!version.references.length)
        return `<p class="history-empty-note">${options.t(uiKeys.history.detail.noInputSnapshot)}</p>`;
