...
export function mountHistoryTagsController(context, options) {
    ...
    const tagsRoot = root.querySelector("[data-history-tags-root]");
    if (!tagsRoot)
        return () => events.abort();
    ...
    const renderSuggestions = (query = "") => {
        ...
        const suggestions = availableTags().filter((tag) => !assigned.has(historyTagKey(tag)) &&
    ...
    const commitTags = (buildNext, closeAfter = false) => {
