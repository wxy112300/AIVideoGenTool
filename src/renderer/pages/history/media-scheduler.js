...
export function createHistoryMediaScheduler(concurrency) {
    ...
    let sequence = 0;
    let disposed = false;
    const nextTask = () => [...pending.values()].sort((left, right) => priorityOrder[left.priority] - priorityOrder[right.priority] || left.sequence - right.sequence).find((task) => !running.has(task.key));
    const pump = () => {
        if (disposed)
            return;
        while (running.size < limit) {
            ...
            Promise.resolve()
                ...
                .then((success) => {
                if (success === true && !controller.signal.aborted)
    ...
    return {
        enqueue: (key, task, priority = "prefetch") => {
            if (disposed || !key || completed.has(key))
                return;
