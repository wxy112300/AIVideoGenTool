...
export interface HistoryTimelineMarkerPosition {
  index: number;
  top: number;
}
...
export function formatHistoryTimelineDate(value: string, locale?: string): string {
...
function timelineDateKey(value: string): { key: string; validDate: boolean; timestampMs: number } {
  const date = new Date(value);
...
export function historyTimelineEntries(
  records: ReadonlyArray<HistoryTimelineRecord>,
  sort: HistorySort,
  locale?: string
): HistoryTimelineEntry[] {
  if (!isHistoryTimeSort(sort)) return [];
...
export function groupHistoryTimelineEntries(
  entries: ReadonlyArray<HistoryTimelineEntry>
): HistoryTimelineGroup[] {
  const groups = new Map<string, HistoryTimelineGroup>();
...
export function clampHistoryTimelineProgress(value: number): number {
  return Number.isFinite(value) ? Math.min(1, Math.max(0, value)) : 0;
}

export function historyTimelineProgress(scrollY: number, maxScrollY: number): number {
  if (!Number.isFinite(maxScrollY) || maxScrollY <= 0) return 0;
  return clampHistoryTimelineProgress(scrollY / maxScrollY);
}

export function historyTimelineScrollY(progress: number, maxScrollY: number): number {
...
export function closestHistoryTimelineMarker(
  markers: ReadonlyArray<HistoryTimelineMarkerPosition>,
  progress: number
): number {
...
export function historyTimelineMarkerLabel(labels: ReadonlyArray<string>): string {
  return labels[0] ?? "—";
}
