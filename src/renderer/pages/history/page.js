...
import { renderHistoryHeading, renderImageMediaStatus, renderImageLightboxMarkup, renderImageReferenceSnapshotMarkup, renderH3TokenCountMarkup, renderPerformanceStatsMarkup, renderVideoInputSnapshotMarkup, renderVideoLoraSnapshotMarkup } from "./fragments";
...
function renderHistoryCardCuration(rating, favorite, options) {
...
function renderHistoryRatingControl(assetId, rating, options) {
...
function historySortOptions(options, selected, includeDuration) {
    ...
    if (includeDuration) {
        values.push(["duration-desc", uiKeys.history.filter.sortDurationDesc], ["duration-asc", uiKeys.history.filter.sortDurationAsc]);
    }
    return values.map(([value, key]) => `<option value="${value}" ${selected === value ? "selected" : ""}>${options.t(key)}</option>`).join("");
}
function renderHistoryFilter(viewModel, options, totalCount, visibleCount, modelIds, tagNames) {
...
function historyComputeMode(version, options) {
...
function historyCodeValue(value, options) {
    return `<code>${options.escapeHtml(value)}</code>`;
}
function historyRequestedEffectiveValue(requested, effective, options) {
...
function historyBooleanValue(value, options) {
    if (value === undefined) {
        return options.escapeHtml(options.t(uiKeys.history.detail.legacyNotSaved));
    }
...
function renderH3ExecutionStrategyMarkup(version, asset, options) {
    const policy = version.h3ExecutionPolicy ?? asset.h3ExecutionPolicy;
    const rows = [];
    const addTranslatedRow = (key, value) => {
        if (value !== undefined) {
    ...
    if (attentionRequested !== undefined || attentionEffective !== undefined) {
        addTranslatedRow(uiKeys.history.page.attentionBackend, historyRequestedEffectiveValue(attentionRequested, attentionEffective, options));
    }
    ...
    if (sparseRequested !== undefined || policy?.sparseAttentionMode !== undefined) {
        addTranslatedRow(uiKeys.history.page.sparseAttention, historyRequestedEffectiveValue(sparseRequested, policy?.sparseAttentionMode, options));
    }
    ...
    if (runtimeRequested !== undefined || policy?.runtimeMode !== undefined) {
        addTranslatedRow(uiKeys.history.page.runtimePolicy, historyRequestedEffectiveValue(runtimeRequested, policy?.runtimeMode, options));
    }
    ...
    if (compilerRequested !== undefined || policy?.comfyCompilerMode !== undefined) {
        addTranslatedRow(uiKeys.history.page.comfyCompiler, historyRequestedEffectiveValue(compilerRequested, policy?.comfyCompilerMode, options));
    }
    ...
    if (spectrumRequested !== undefined || spectrumEffective !== undefined || spectrumModelAware !== undefined) {
        ...
        addTranslatedRow(uiKeys.history.page.spectrum, spectrumModelAware === undefined
            ? spectrumValue
...
export function renderHistoryDetailPage(viewModel, options) {
    ...
    const executionPrompt = videoPromptForLoras(asset.prompt, version.videoLoras ?? asset.videoLoras);
    ...
    const videoEditAction = (variant) => retiredModel ? "" : `<button class="${variant} button-with-icon" data-edit-history="${asset.id}" aria-label="${options.t(uiKeys.history.page.adjustInCreate)}" title="${options.t(uiKeys.history.page.adjustInCreate)}">${options.icon("sliders-horizontal")}${options.t(uiKeys.history.page.adjustInCreate)}</button>`;
    const videoContinueAction = (variant) => !retiredModel && videoFile?.absolutePath ? `<button class="${variant} button-with-icon" data-continue-history="${asset.id}" data-source-version="${version.id}" aria-label="${options.t(uiKeys.history.page.continueCreation)}" title="${options.t(uiKeys.history.page.continueCreation)}">${options.icon("video")}${options.t(uiKeys.history.page.continueCreation)}</button>` : "";
...
function renderImageHistoryVersionThumb(project, item, pinnedVersion, selectedVersionId, options) {
...
export function renderImageHistoryDetailPage(viewModel, options) {
    ...
    const imageStartVideoAction = (variant) => `<button class="${variant} button-with-icon" data-image-continue-video-project="${options.escapeHtml(project.id)}" data-image-continue-video-version="${options.escapeHtml(version.id)}">${options.icon("video")}${options.t(uiKeys.history.page.startVideo)}</button>`;
    const imageContinueEditAction = (variant) => `<button class="${variant} button-with-icon" data-image-continue-edit-project="${options.escapeHtml(project.id)}" data-image-continue-edit-version="${options.escapeHtml(version.id)}">${options.icon("wand-sparkles")}${options.t(uiKeys.history.page.continueEdit)}</button>`;
