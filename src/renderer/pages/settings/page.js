...
export function renderSettingsPage(viewModel, options) {
    ...
    const t = options.t;
    const s = (key, params) => settingsText(settings.uiLocale, key, params);
    const environmentScan = viewModel.environmentScan;
    const escape = (value) => options.escapeHtml(value == null ? "" : String(value));
    ...
    const environmentOverview = renderSettingsEnvironmentOverview({
        environmentScan,
        environmentItems: deriveEnvironmentOverviewItems(environmentScan, viewModel.comfyConnected),
    ...
    const renderProfileCard = (profile) => renderSettingsModelScanCard(profile, {
        ...sharedFragmentOptions,
    ...
    const promptProfiles = profiles
        ...
        .sort((left, right) => {
        const orderDifference = promptModelCapabilityOrderValue(right.id) - promptModelCapabilityOrderValue(left.id);
    ...
    const selectedAutoPromptSeed = options.h3AutoPromptSeeds.find((seed) => seed.id === settings.h3AutoPromptSeedId);
    ...
    const videoAvailable = videoProfiles.filter((profile) => profile.available && profile.integrated).length;
    ...
    const { nodeUpdatesAvailable, customNodeInstallFinalizing, customNodeInstallGloballyBlocked } = dependencyActionState;
    const promptRuntime = derivePromptRuntimeState(settings, environmentScan, promptProfiles, {
        isQwenVlPeftPromptModel: options.isQwenVlPeftPromptModel,
        isGemmaPromptModel: options.isGemmaPromptModel,
        isComfyMultimodalPromptModel: options.isComfyMultimodalPromptModel
    });
    ...
    const activePanel = viewModel.settingsTab === "system" ? systemPanel :
