...
export function mountSettingsLogsController(context, options) {
    ...
    const terminal = root.querySelector("#app-log-terminal");
    if (terminal) {
        ...
        terminal.addEventListener("scroll", (event) => {
            event.stopImmediatePropagation();
            options.setAppLogFollowTail(terminal.scrollHeight - terminal.scrollTop - terminal.clientHeight < 48);
        }, { signal });
    }
    return () => events.abort();
}
