...
export function createAppLogContextMenu(context, clearLogScreen) {
    let menu = null;
    let events = null;
    const close = () => {
        events?.abort();
    ...
    const open = (clientX, clientY) => {
        ...
        nextMenu.querySelector("button:not(:disabled)")?.focus();
        ...
        nextMenu.addEventListener("click", async (event) => {
            const button = event.target.closest("[data-app-log-action]");
        ...
        document.addEventListener("pointerdown", (event) => {
            if (!nextMenu.contains(event.target))
                close();
