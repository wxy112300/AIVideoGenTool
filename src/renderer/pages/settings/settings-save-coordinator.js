...
export class SettingsSaveCoordinator {
    ...
    async requestSave(nextSettings) {
        ...
        if (directoryComparisonKey(oldDirectory) !== directoryComparisonKey(newDirectory)) {
            this.dependencies.requestDirectoryMigration(previousSettings, nextSettings, oldDirectory, newDirectory);
            return "migration-required";
        }
        await this.save(nextSettings);
        return "saved";
    }
    async save(nextSettings, mode = "apply") {
        ...
        if (imageModelChanged && savedState.imageDraft.modelId === previousSettings.defaultImageModel) {
            ...
            const qualityProfile = capability.qualityProfiles.some((profile) => profile.id === savedState.imageDraft.qualityProfile)
        ...
        if (savedState.settings.ltxExtensionModelProfile !== previousProfile) {
            ...
            if (savedState.draft.modelId === "sulphur2") {
                const bundled = await this.dependencies.getBundledWorkflow("sulphur2", savedState.draft.inputMode);
                if (bundled) {
