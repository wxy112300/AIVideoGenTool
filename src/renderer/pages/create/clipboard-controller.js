...
import { h3ReferenceTag, imageFileIsSupported } from "./helpers";
export function mountCreateClipboardController(context, options) {
    const handler = async (event) => {
        const t = context.t;
        if (context.getRoute().page !== "create")
            return;
        ...
        if (state.draft.inputMode !== "image")
            return;
        ...
        const focusedPasteTarget = activeElement instanceof HTMLElement
            ? activeElement.closest("[data-paste-frame]")
            : null;
        if (isMiniMaxH3R2vModel(state.draft.modelId)) {
            const targetSlot = state.draft.h3ReferenceSlots.find((slot) => slot.mediaType === "image" && !slot.mediaPath);
            if (!targetSlot) {
                const { imageCount } = h3ReferenceSlotCounts(state.draft.h3ReferenceSlots);
                context.notify(imageCount >= 9
