  ...
  return `
    ${slots.length ? `<div class="h3-reference-grid">${slots.map((slot, index) => `
      <article class="h3-reference-slot" data-h3-slot="${escapeHtml(slot.id)}">
        ...
        <label>${t(uiKeys.create.fragments.referenceRole)}<select data-h3-slot-role="${escapeHtml(slot.id)}">${h3ReferenceSlotRoleOptions(slot.role, h3ReferenceRoleLabels)}</select></label>
