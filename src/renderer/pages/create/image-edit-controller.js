...
function remapPromptPictureReferences(text, pictureNumberMap, removedPictureNumbers) {
    return text.replace(pictureReferencePattern, (match, numberText) => {
        const oldNumber = Number(numberText);
        const nextNumber = pictureNumberMap.get(oldNumber);
        if (nextNumber !== undefined)
            return match.replace(numberText, String(nextNumber));
...
export function mountImageEditController(context, options) {
    ...
    const bindImageDropZone = (zone, replacePictureId) => {
    ...
    root.querySelectorAll("[data-remove-image-picture]").forEach((button) => {
        button.addEventListener("click", (event) => {
            ...
            const pictures = deletingBaseSlot
                ? draft.pictures.map((item) => item.id === pictureId
                    ? { ...item, absolutePath: "", width: 0, height: 0, role: "base", crop: undefined, markup: undefined, mask: undefined }
                    : item)
                : renumberImageReferences(remainingPictures);
            const pictureNumberMap = new Map();
            if (!deletingBaseSlot) {
                ...
                for (const item of remainingPictures) {
                    ...
                    if (nextPictureNumber !== undefined)
                        pictureNumberMap.set(item.pictureNumber, nextPictureNumber);
                }
            }
            const promptVersions = deletingBaseSlot
                ...
                : draft.promptVersions.map((version) => ({
                    ...version,
                    text: remapPromptPictureReferences(version.text, pictureNumberMap, new Set([picture.pictureNumber]))
                }));
            if (!deletingBaseSlot)
    ...
    root.querySelectorAll("[data-markup-image-picture]").forEach((button) => {
        button.addEventListener("click", (event) => {
            event.preventDefault();
            event.stopImmediatePropagation();
    ...
    insertSnippet?.addEventListener("click", (event) => {
        event.stopImmediatePropagation();
        if (!promptInput || !snippetSelect?.value)
            return;
    ...
    promptInput?.addEventListener("input", () => {
        ...
        if (!draft)
            return;
        options.invalidatePromptEditHistory();
        const updated = updateManualPromptVersion(draft.promptVersions, draft.activePromptVersion, promptInput.value, t(uiKeys.create.interaction.manualEdit));
    ...
    const focusPromptInput = () => {
        window.requestAnimationFrame(() => {
            const nextInput = root.querySelector("#image-edit-prompt-input");
            if (!nextInput)
                return;
            nextInput.focus();
    ...
    root.querySelector("#enhance-prompt")?.addEventListener("click", async (event) => {
        ...
        try {
            ...
            const text = await context.enhancePrompt({
                ...
                referenceContext: [
                    pictures.map((picture) => `Slot ${picture.pictureNumber} / Picture ${picture.pictureNumber} = ${options.imageReferenceRolePromptLabel(picture.role ?? "auto")}`).join("\n"),
    ...
    countInput?.addEventListener("input", () => {
        ...
        if (countValue)
            countValue.textContent = t(uiKeys.create.imageEdit.outputCountValue, { count: outputCount });
    }, { signal });
