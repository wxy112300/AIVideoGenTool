...
import { BUILTIN_VIDEO_LORAS, H3_TURBO_LORA_ID, profileProvidesVideoLora, videoLoraCompatibleWithDraft } from "../../../core/video-loras";
...
export function buildImageEditPageViewModel(options) {
    ...
    const imageResolutionOptions = imageCapability.sourceResolutionOnly ? imageResolutionOptionsFor(basePicture?.width ?? 0, basePicture?.height ?? 0).slice(0, 1) : imageResolutionOptionsFor(basePicture?.width ?? 0, basePicture?.height ?? 0);
...
export function buildVideoCreatePageViewModel(options) {
    ...
    const turboLoraProfile = environmentScan?.modelProfiles.find((profile) => profile.id === H3_TURBO_LORA_ID);
