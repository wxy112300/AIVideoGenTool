...
import { mountImageEditController } from "./image-edit-controller";
...
export function mountCreatePageController(options) {
    ...
    const applyVideoLoraStack = async (videoLoras) => {
        const state = getState();
        if (!state)
            return;
        ...
        const previousBundledPath = options.bundledWorkflows[options.bundledWorkflowKey(previousWorkflowModelId, state.draft.inputMode)]?.path;
    ...
    root.querySelector("#add-video-lora")?.addEventListener("click", async () => {
        ...
        if (!state)
            return;
        const id = root.querySelector("#video-lora-to-add")?.value ?? "";
        const lora = BUILTIN_VIDEO_LORAS.find((item) => item.id === id);
        if (!lora || state.draft.videoLoras.some((item) => item.id === id))
            return;
    ...
    root.querySelectorAll("[data-move-video-lora]").forEach((button) => {
        button.addEventListener("click", async () => {
            const state = getState();
    ...
    const updateLoraStrength = (id, rawValue) => {
        const state = getState();
        if (!state)
            return;
        ...
        root.querySelector(`[data-video-lora-strength="${CSS.escape(id)}"]`).value = String(strength);
        root.querySelector(`[data-video-lora-strength-number="${CSS.escape(id)}"]`).value = String(strength);
    };
    root.querySelectorAll("[data-video-lora-strength]").forEach((input) => {
    ...
    for (const id of ["model", "ratio", "resolution", "steps", "spectrum-mode", "fps", "frame-interpolation", "motion", "seed"]) {
        root.querySelector(`#${id}`)?.addEventListener("change", async (event) => {
            ...
            if (id === "model") {
                ...
                const bundledPromise = cachedBundled
                    ...
                    : options.context.application.getBundledWorkflow(value, previousDraft.inputMode)
                        .catch(() => null);
                const rememberBundledWorkflow = (bundled) => {
                ...
                if (cachedBundled)
                    rememberBundledWorkflow(cachedBundled);
                options.patchDraft({
                    modelId: value,
                    videoLoras: [],
                    ...
                    ...(isMiniMaxH3Model(value)
                        ? {
                            ...
                            resolution: 480,
                            duration: 5,
                            steps: isMiniMaxH3Q3GgufModel(value) ? 8 : 20,
                            fps: 24,
                            frameInterpolation: "off",
                            motion: "natural",
                            h3LatentSaveMode: h3LatentSaveModeFor(previousDraft, previousDraft.inputMode === "video" && oldWasR2V),
                            h3SaveJointAv: h3SaveJointAvForLatentSaveMode(h3LatentSaveModeFor(previousDraft, previousDraft.inputMode === "video" && oldWasR2V)),
                    ...
                    ...(isMiniMaxH3Model(previousDraft.modelId) && !isMiniMaxH3Model(value) && previousDraft.ratio === "21:9"
                        ? { ratio: "source" }
                        : {}),
                    ...(cachedBundled && !cachedBundled.supportsEndImage && !nextIsR2V ? { endImagePath: "", endImageWidth: 0, endImageHeight: 0 } : {}),
                ...
                if (selectionRevision !== modelSelectionRevision || !bundled)
                    return;
            ...
            const patch = id === "ratio" ? { ratio: value } :
    ...
    root.querySelector("#clear-seed")?.addEventListener("click", () => {
        options.patchDraft({ seed: null });
        options.context.requestRender();
    }, { signal });
    root.querySelector("#random-seed")?.addEventListener("click", () => {
        options.patchDraft({ seed: options.imageEdit.randomSeedValue() });
        options.context.requestRender();
    }, { signal });
    const range = root.querySelector("#duration");
    const number = root.querySelector("#duration-number");
    const updateDuration = (value) => {
        ...
        options.syncEnqueueUi();
        if (range)
            range.value = String(duration);
        if (number)
            number.value = String(duration);
        ...
        if (added)
            added.textContent = t(uiKeys.create.interaction.trimAdded, { value: duration.toFixed(1) });
        if (total)
            total.textContent = t(uiKeys.create.interaction.trimApproxTotal, { value: (kept + duration).toFixed(1) });
    };
    ...
    root.querySelector("#enqueue")?.addEventListener("click", async () => {
        const state = getState();
        if (!state || options.isEnqueueBusy())
            return;
