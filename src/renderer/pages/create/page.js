...
import { renderCreateModelOptions, renderH3ReferenceSlotsMarkup, renderImageEditPromptInstructionOptions } from "./fragments";
function renderPromptModeInfo(tip, icon, escapeHtml) {
...
function imageReferenceRoleOptions(options, picture) {
    return Object.entries(options.imageReferenceRoleLabels)
        .map(([value, label]) => `<option value="${value}" ${picture.role === value || (picture.pictureNumber === 1 && value === "base") ? "selected" : ""}>${label}</option>`)
        .join("");
}
export function renderImageEditPage(viewModel, options) {
    ...
    <div class="create-workspace image-edit-workspace">
      ...
      <section class="panel composer image-edit-composer">
        ...
        <section class="composer-control-group image-edit-output-group"><div class="composer-group-heading"><div><strong>${t(uiKeys.create.imageEdit.generationSettings)}</strong><span>${viewModel.outputCountVisible ? t(uiKeys.create.imageEdit.batchDescription) : escapeHtml(viewModel.promptlessDescription)}</span></div></div><div class="composer-control-grid image-edit-settings-grid">
          ...
          ${viewModel.outputCountVisible ? `<label class="settings-field range-field"><span class="range-heading"><span>${t(uiKeys.create.imageEdit.outputCount)}</span><strong id="image-edit-count-value">${t(uiKeys.create.imageEdit.outputCountValue, { count: viewModel.count })}</strong></span><input id="image-edit-count" type="range" min="1" max="10" step="1" value="${viewModel.count}"><span class="range-scale"><span>1</span><span>${t(uiKeys.create.imageEdit.sequentialGeneration)}</span><span>10</span></span></label>` : ""}
        </div></section>
...
export function renderCreatePage(viewModel, options) {
    const icon = options.icon;
    const escapeHtml = options.escapeHtml;
    const t = options.t;
    ...
    <div class="create-workspace ${viewModel.isR2V ? "r2v-workspace" : ""}">
      ...
      ${viewModel.extending && viewModel.isR2V && viewModel.r2vTotalCount > 1 ? `<section class="h3-motion-context-references">
        ...
        ${renderH3ReferenceSlotsMarkup(ensureMotionContextSourceSlot(viewModel.draft.h3ReferenceSlots, viewModel.draft.sourceVideoPath), { icon, escapeHtml, h3ReferenceRoleLabels: options.h3ReferenceRoleLabels, t, lockedFirstVideo: true })}
      </section>` : ""}
      </section>
      ...
      <div class="composer-settings">
        ...
        <label class="settings-field settings-ratio">${t(uiKeys.create.videoSettings.ratio)}
          <select id="ratio" ${viewModel.extending ? "disabled" : ""}>
            ${["source", "16:9", "9:16", "1:1", "4:3", "3:4"].map((ratio) => `<option value="${ratio}" ${viewModel.draft.ratio === ratio ? "selected" : ""}>${ratio === "source" ? viewModel.extending ? t(uiKeys.create.videoSettings.followInputVideo) : t(uiKeys.create.videoSettings.originalImageRatio) : ratio}</option>`).join("")}
          </select>
        </label>
        ...
        <label class="settings-field settings-fps">${t(uiKeys.create.videoSettings.targetFps)}
          <select id="fps" ${viewModel.isMiniMaxH3 ? "disabled" : ""}>
            ${(viewModel.isMiniMaxH3 ? [24] : [8, 12, 16, 24, 25, 30]).map((value) => `<option value="${value}" ${viewModel.draft.fps === value ? "selected" : ""}>${value} FPS</option>`).join("")}
          </select>
        </label>
