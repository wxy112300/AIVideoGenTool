  ...
  return `
    ...
    <div class="create-workspace image-edit-workspace">
      ...
      <section class="panel composer image-edit-composer">
        ...
        <div class="interpolation-summary settings-summary ${viewModel.enqueueBlockReason ? "unsafe" : ""}"><div><strong>${viewModel.enqueueBlockReason || (viewModel.promptless ? escapeHtml(viewModel.promptlessSummary) : t(uiKeys.create.imageEdit.summary, { count: viewModel.count, seedMode: t(viewModel.draft.seed == null ? uiKeys.runtime.random : uiKeys.runtime.same) }))}</strong><span>${viewModel.promptless ? escapeHtml(viewModel.promptlessResultDescription) : t(uiKeys.create.imageEdit.noUpscale, { capability: escapeHtml(viewModel.imageCapabilityName) })}</span></div><p>${escapeHtml(viewModel.imageProfileStatusText)}</p></div>
  ...
  return `
    ...
    <div class="create-workspace ${viewModel.isR2V ? "r2v-workspace" : ""}">
      ...
      <div class="submit-row composer-submit-row">
        <button class="ghost danger button-with-icon" id="clear-draft">${icon("trash-2")}${t(uiKeys.create.videoSettings.clearDraft)}</button>
