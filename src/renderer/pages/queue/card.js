...
import { h3LatentSaveModeFor } from "../../../core/h3-latent-save";
...
function compactWorkValue(value) {
    return Number.isInteger(value) ? String(value) : value.toFixed(2).replace(/\.?0+$/u, "");
}
export function queueWorkProgressText(task, t) {
    const work = task.workProgress;
    if (!work || work.value <= 0 || work.max <= 0)
        return "";
...
export function queueTaskHasRecoveryCheckpoint(task) {
    if (task.taskType === "generation")
        return Boolean(task.h3FirstPassCheckpoint);
    if (task.taskType === "upscale")
        return Boolean(task.seedVr2Checkpoint?.completed.length);
    return false;
}
...
export function renderQueueTaskCard(task, queuePosition, options) {
    ...
    const h3LatentSaveSummary = task.taskType !== "upscale" && task.taskType !== "image-generation" && isMiniMaxH3Model(task.modelId)
        ? (() => {
            const mode = h3LatentSaveModeFor(task, task.taskType === "extension" && isMiniMaxH3R2vModel(task.modelId));
