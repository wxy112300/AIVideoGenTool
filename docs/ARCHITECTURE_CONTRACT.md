...
- Settings information architecture separates `ComfyUI environment` ownership (selected installation, core/data directories, Python binding, service lifecycle, core compatibility/update and safe repair) from `nodes & dependencies` ownership (custom nodes and installable Python/runtime packages). The temporary `Performance & acceleration` surface continues to own the existing H3 Attention selection until the H3 Memory Optimization work is complete; its selector and persisted `h3AttentionMode` must not be duplicated or migrated during the environment-page extraction.
