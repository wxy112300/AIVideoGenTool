...
> 状态：已完成交互原型，正式 Electron 功能尚未实现  
> 制定日期：2026-08-10  
> 目标版本：建议作为 `0.3.0` 图片工作台里程碑  
> 交互基线：`prototypes/` 当前图片创建、图片历史和图片详情原型

当前边界：原型用于确认交互，不代表正式应用已经支持图片任务。正式 renderer 的历史页仍以视频数据为中心，图片/视频 Tab、图片项目聚合、图片队列执行和 Qwen 工作流均需按本文实施。
...
依赖：Phase 0；真实提交依赖 Phase 1，UI可使用 fixture 并行开发。
...
依赖：Phase 4；视频交接可与 Phase 4 后半并行。
...
```text
Phase 0: Agent A 独占契约
                 ↓
        ┌────────┼─────────┐
Phase 1 Agent B  │  Phase 2 Agent D（fixture UI）
                 │
                 └─ Agent C 先做IPC骨架
        └────────┼─────────┘
                 ↓
Phase 3: Agent C 主导，B协助工作流
                 ↓
Phase 4/5: Agent D 主导，A协助项目/版本纯函数
                 ↓
Phase 6/7: C与D按文件边界并行
                 ↓
Phase 8: 独立验收 Agent 做回归
```
