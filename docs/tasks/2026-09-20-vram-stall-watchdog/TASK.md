...
- Scope / Authority: 为本地、应用管理的 ComfyUI 队列任务增加可关闭、可选 5/10/15 分钟的显存压力卡死检测；触发后重启 ComfyUI，并按现有重试次数重跑同一不可变任务快照，耗尽后标记失败并继续队列。详细设计与验收以 [PLAN](PLAN.md) 为准。
...
- 已确认 / 决定：新增一个 `0 | 5 | 10 | 15` 分钟设置，`0` 为关闭且默认值为关闭。启用后 watchdog 恢复次数复用 `autoRetryCount`，不依赖通用 `autoRetryFailedTasks` 开关；通用开关仍只控制其他可恢复错误。UI 将该选择器表述为共同的“恢复重试次数”。
- 已确认 / 决定：只有本地、应用管理的 ComfyUI 可执行 watchdog 重启。远程 endpoint 显示不可用且不主动中止远端 prompt；现有连接/服务超时仍照常处理。
- 已完成：纯状态机与压缩时间 trace fixture、`0 | 5 | 10 | 15` 设置/迁移/UI、claim-time policy、单一任务资源采样、Comfy productive-progress、图片/视频/SeedVR2 阶段边界、`memory-pressure-stall` 精确恢复、应用管理本地运行时检查、有限重试与诊断摘要均已落地；默认关闭，未修改现有普通 VRAM watchdog 的 abort 语义。
- 下一步 / 限制：尚未取得真实近满正常 trace、共享显存/分页卡死 trace，也未做隔离 Electron + ComfyUI + GPU smoke；当前 Windows 采样使用 fail-open 的 Shared/Commit/Pages counter 批量读取，尚未加入直接 DXGI adapter LUID/Budget helper。真实样本前不宣称阈值跨机器验证。
...
- 实际命令、结果、对应文件状态：`npm.cmd run typecheck` 通过；`npm.cmd run test:all` 通过（unit 180 files / 1595 tests，integration 6 files / 81 tests）；`npm.cmd run verify` 通过（含 build 与 `verify:ux-ui-contrast`）。工作区保留用户原有 `docs/tasks/README.md` 修改及本任务文档，未生成需提交的 `dist` 变更。
