...
- 下一步：由接手负责人先查看是否已有较新 TASK/运行报告或其他执行 agent；有证据则复用。没有时，只安排一个有界 AetherScale 单图 smoke 工作包。
- 不要重复：同 hash VapourKit 全量下载/递归列举；用 Aether/Merserk DLL 改名修 HECer；因节点注册成功就宣布放大方案可用。
- 阻塞性质：HECer 是已记录的外部资产缺失；AetherScale 是真实验证尚未在本卡确认，可能受环境/运行窗口限制，具体原因待接手核验。不要把两个 provider 合成一个阻塞原因。
...
这些是后续执行计划，不是本轮已发出的运行指令。原计划的 3×、长批次、对照 benchmark 属于之后阶段；不能为了一个解锁判断一次跑完整矩阵。
A1 证据写入本任务的 `evidence/aether-smoke.md`：当前 ComfyUI `/object_info` 快照及 hash、固定 runtime/revision、实际 worker 日志中的 carrier/feature-18 运行结果、真实输入/输出文件定位与媒体检查。fixture、READY 或仅提交成功均不能替代这些证据。
出现 hash/schema 不匹配、原生崩溃、无法中断、只有普通 resize 时停止对应实验并保留证据。HECer/AetherScale 互不静默回退，现有 upscale 路径保持可用。
...
接手只需读本 Resume、计划对应 provider 的 WP7/停止条件、当前 runtime/catalog；无需重读千行计划全部内容。
本轮是文档/静态源码调查，未运行下载、安装、schema 请求、测试或真实输出；不提升产品 ready 状态。
