...
类型：Research / writing guide
状态：候选写作参考；不自动改变 Prompt Pack 或当前默认策略。
整理范围：2026-08-18 前后的公开资料，冲突时以官方资料和 Prompt Pack 设计为准。
...
- 普通 5 秒非 R2V 提示词可先参考约 250-500 个英文单词，再按对白、动作节点和镜头复杂度自然增加；这不是上限。
...
本地 Qwen3.5 2B/4B 使用精简的 H3 小模型契约，不再把完整官方规则和所有参考解释重复塞进上下文。它应当按四步静默处理：

1. **提取**：列出用户明确说出的主体、物体、尺度、材质、衣着或暴露程度、动作、姿态、运镜、对白和画面文字。这里的词是必须保留的内容，不是可以随意润色掉的草稿。
2. **规划**：按照实际有效时长安排动作起因、准备、发展、反应、镜头运动和片尾状态。每一段都必须带来新的可见、可听或行为变化。
3. **成稿**：只填当前 H3 模式需要的字段；参考图只提供紧凑的身份、构图和连续性锚点，主要篇幅给动作和运镜。
4. **审计**：逐项检查用户要求是否都还在，是否误加了用户没有要求的角色、道具、剧情或静态图片清单，并删除不承担功能的句子。

扩写器不应默认加入 `masterpiece`、`ultra HD`、`4K`、`cinematic`、`photorealistic` 等质量口号。只有当用户明确要求某种风格，或该描述确实改变目标画面时，才保留它。普通 H3 生成的每句话至少要完成一项工作：保留用户要求、推进动作、控制镜头、同步声音或保护连续性；否则删掉。
...
- 生成类 R2V 的 `detailed_description` 对简单 5 秒任务通常从约 350-500 个英文单词开始；对白密集、多镜头、参考关系复杂或时长更长时可以超过这个范围
...
2. R2V `detailed_description` 已给出 350-500 词的典型起始范围，但不是硬上限，检查器也不应强制词数。
