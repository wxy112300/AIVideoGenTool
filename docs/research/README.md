...
这里保存有来源、日期、版本和验证范围的研究/实验记录。它们解释“为什么这样判断”，不自动成为产品契约或当前任务状态。
...
- Static validated 只表示源码、schema、manifest 或文档对照完成；Runtime validated 必须有实际运行和输出证据。
- 研究中的建议、旧版本号和机器路径不应复制进当前契约；采用后要把稳定边界写入对应契约，并保留原始 evidence。
- 同一主题只有一个当前 TASK 负责状态和下一步；研究文件不承担任务台账。
