...
- Motion Context uses the `v0.3.1` compatibility line for ComfyUI 0.32/0.33. The bundled API workflow keeps `context_length=22` and uses the upstream-aligned `audio_context_length=24`; after a node update, saved ComfyUI canvas nodes must be deleted and re-added. Duplicate or renamed Motion Context copies are a warning because both patches can compete at runtime.
