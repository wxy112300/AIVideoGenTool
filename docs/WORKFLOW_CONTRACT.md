...
- Queue tasks retain every execution-affecting option. Settings changes may refresh the H3 acceleration subset on waiting tasks; once execution starts, the task's policy remains fixed.
...
- H3 prompt enhancement uses one shared control plan across Qwen3.6/Qwen3.8, H3 Prompt Writer, and the MiniMax H3 Prompt Rewriter. User text, annotations, exact dialogue/visible text, LoRA knowledge, mode/reference roles, duration, preset policy, and conditional camera/scale controls keep that priority order. Internal contracts may describe validation rules, but the final H3 prompt uses concise positive execution language instead of accumulating generic failure-token lists.
- H3 defaults to one continuous `[Shot 1]` in every generation mode, including R2VA. Multiple views, shots, cuts, montage, or scene changes are enabled only by an explicit user request; a bare `[Shot 1]` is a format marker and is not the mechanism enforcing continuity. The ten preset choices and all quick-insert entries remain individually available and do not stack into hidden preset groups.
- Reference semantics are mode-specific. FL2VA sends both ordered endpoint images to every capable visual enhancer and binds each entity across frames by user labels and stable visible traits rather than screen position or apparent size. R2VA keeps sources, reusable Subjects, clothing ownership, actions, and transfer roles distinct. Extend treats the extracted boundary as the exact last-visible source state and develops the requested continuation from it. The MiniMax H3 Prompt Rewriter supports Base/FL2VA visual rewriting but not R2VA; unsupported R2VA requests fail with guidance to Qwen3.6/Qwen3.8 or H3 Prompt Writer.
- Execution-time LoRA trigger words remain centralized and deduplicated outside the prompt enhancer. A trigger already present anywhere as a complete token in the user prompt stays in place and is not injected again. For missing triggers in H3 prompts with an official first-frame or endpoint-alignment declaration, that reference declaration stays at the absolute start and the trigger is inserted immediately after it; otherwise the missing trigger is inserted at the prompt start.
- Scale words bind to a named or visibly distinguishable subject and preserve relative size ordering against full-size scene anchors. Enhancement carries only facts supplied by the user or reliably visible in references; unknown age, gender, measurement, identity, and appearance are omitted. Human action expansion favors reachable poses, continuous weight transfer, stable limb ownership, and specific plausible hand contact, especially during close passes and occlusion.
...
Legacy H3 generation/extension tasks with no AV output policy and an effective `all` save intent are upgraded to `h3AvOutputPolicy=shared` in the atomic queue claim, before graph submission. The claim persists the shared policy, `h3LatentSaveMode=all`, and `h3SaveJointAv=true`; it preserves prompts, seeds, workflow/source inputs, sampling parameters, and drafts. Legacy missing save fields retain their existing save-all default. Only the next eligible waiting task is changed. Disabled or partial save modes, policy errors/unknown policies, first-pass checkpoints, managed sequences, and independent upscale tasks retain their original contract. This does not retroactively create AV for completed outputs or change consumer routing; deploy the updated application and bundled H3 AV node together.
...
- Application-generated Spectrum H3 graphs keep adaptive forecasting enabled but disable `offline_smoothing_replay` by default. The replay path retains a multi-GB feature archive and reloads the H3 model for a second pass; on the ComfyUI 0.35 + RTX 4090 dynamic-VRAM profile, the measured smoke stalled during that reload under host/shared-GPU memory pressure. Manually authored graphs may still opt into replay, and their two-pass progress must be reported as two passes rather than treating the first pass as complete.
...
- H3 Memory Optimization is withdrawn from product execution. The Create control, Queue metadata, dependency card, scanner entry, installer route and workflow injection are absent; old draft/queue/history fields are read only and normalize to the withdrawn state. Rendered workflows remove managed `H3MemoryOptimization` and `H3AIMDOResidencyLimiter` wrappers and never add them back. Do not delete user-owned history, media, models or external `custom_nodes` files.
- H3 execution uses one per-task policy for each run: dense attention (`sage`, `sage-triton`, `pytorch`, or `comfy-kitchen`), sparse strategy, native runtime mode, and compiler mode follow saved Settings while the task is waiting, then remain fixed once execution starts. Spectrum and preview keep their own task choices. ComfyUI 0.35 native `ModelAttentionBackend` / `BlockSparseAttention` are schema-checked at claim time; old fields cannot override the current waiting-task policy. VSA remains fail-closed until a FastH3 weight/kernel pairing is actually validated.
- `H3-Optimizations` is not an active Settings observation item and must not participate in installation, scanning or readiness. Reintroduction requires a new explicit product decision plus real provider evidence, host RAM/shared-GPU measurements and long-frame validation; a completed output with `qkv_provider=standard_h3_qkv` or `memory=baseline` is not evidence that Memory was active.
...
- LightX2V Turbo may stack with Spectrum only on Spectrum `v0.2.6+`. The H3 8-step path retains the native ComfyUI ER-SDE contract; the current official v1.2 768p 4-step path uses Euler + Beta with video shift 6 and audio shift 3. H3 Motion Context extension still disables Spectrum.
- Spectrum `model_aware_mode` is available only on `v0.2.7+`, remains opt-in/default-off, and must be serialized into the immutable task snapshot. The current recommended Spectrum release is `v0.2.27`; do not make that recommendation a hard minimum. Omit the node input entirely when mode is `off` so older supported Spectrum workflows remain compatible. `v0.2.11` fixes native ER-SDE forecast-state cleanup, `v0.2.14` protects KJNodes preview callbacks during replay, `v0.2.15` adds optional H3 Continuum metadata interoperability, `v0.2.16` adds optional Untwisting RoPE visual-reference patch contracts plus isolated, bounded post-run research cleanup, `v0.2.17` completes native masked H3 forecasting and keeps learned-latent sampler-2 refinement's actual-prefix policy independent from sampler 1, `v0.2.18–v0.2.20` add optional fail-closed RefDelta Solver v0.2.0+ API-v1 interoperability, nested custom-node discovery, and tracked-step provenance fixes, `v0.2.21` adds ComfyUI 0.34+ PDD H3 FinalLayer compatibility, `v0.2.22` adds native SEEDS-2/SEEDS-3 and SA-Solver support with sampler-specific fail-closed state handling, `v0.2.23` completes active SA-Solver PECE and RefDelta multi-backend interoperability while making `balanced` the active-PECE default, `v0.2.24` removes two unnecessary actual-evaluation barriers in validated few-step/progressive paths and makes the configured RES `tail_actual_steps` authoritative, `v0.2.25` adds the narrow ComfyUI DynamicVRAM/Comfy Compiler malloc-graph compatibility boundary, `v0.2.26` adds provider-generic numerical-attention history/receipt qualification with actual-only fail-closed behavior, and `v0.2.27` adds streamed forecast-head projection, CUDA target-lifetime cleanup, and reviewed Core BlockSparseAttention Mixed-Grid forecast recovery to reduce forecast-head VRAM pressure without promising a whole-workflow speedup. The current built-in H3 workflows still use RES multistep or ER-SDE, so this recommendation does not switch sampler families. RefDelta, SEEDS, and SA-Solver are not hard dependencies; existing Euler, RES multistep, ER-SDE, ordinary SA-Solver, Continuum, and ordinary workflow parameters remain supported. On older cores without `mask_row_values`, masked forecasting must fail closed to one native H3 transformer evaluation. None of Continuum, Diff-Aid, Untwisting RoPE, or RefDelta is a hard dependency. Keep the existing no-cache/EasyCache exclusion and fail-closed behavior for unknown contracts.
...
- Motion Context recommends the current `v0.6.2` release on ComfyUI 0.34.0+; `v0.3.1` remains the compatible fallback line for ComfyUI 0.32/0.33. The v0.6 line adds optional manual-canvas Chain looping, a `segments` count, and numbered-slot clearing; v0.6.2 also normalizes blank `segments` values from older canvas workflows. The bundled API workflow keeps `context_length=22` and uses the upstream-aligned `audio_context_length=24`. Load 0 still represents the first clip with no latent file, positive indices load an exact saved slot, and Chain remains a manual-canvas sequencing node; the application's constructed API workflow uses the four base nodes and explicit positive Slot 1, so it does not require Chain. Upstream's delete-and-re-add guidance applies only to manually saved ComfyUI canvas workflows whose widget positions predate the current node schema; the application's constructed API workflow does not require that migration. Duplicate or renamed Motion Context copies are a warning because both patches can compete at runtime. Its continuation UI reserves Slot 1 for the source video and accepts up to 9 image references plus 2 additional video references (3 videos total, 12 media slots overall).
