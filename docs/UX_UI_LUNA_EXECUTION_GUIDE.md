...
- P03/L10 shared surface、P03/L11 text/separator、P03/L12 action/focus 与 P03/L13 status/badge 迁移已实现并通过 current-renderer capture、全矩阵 diagnose、900×800 输入焦点 smoke、四状态 selector matrix、disabled submit canary、20 组对比度检查与 `npm.cmd run verify`；L13 为 zero-visual-diff，版本仍为 `0.30.0`；不得重复派发 L10–L13，下一 package 从 L14 开始；
