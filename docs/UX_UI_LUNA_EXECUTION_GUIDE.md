...
- P04/L16–L17 已完成：L16 声明 Page/Section/Object/Body/Label/Meta/Technical 七级 type roles；L17 按当前 renderer 最终 cascade 校正 page 基线并迁移共享 `h1/h2/h3` 与 History gallery object heading，保留详情/弹窗/任务卡/album 紧凑特例。focused token tests `9/9`、typecheck、build、三 locale 各 `136/136` screenshots、代表视口 diagnose 与中英文输入焦点 smoke 通过；版本仍为 `0.30.2`，下一 package 为 L18；
