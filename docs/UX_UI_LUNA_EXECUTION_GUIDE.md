...
- P20/L65–L66 已完成：`docs/UX_UI_P20_QA_REPORT.md` 与 `docs/UX_UI_P20_SCREENSHOT_MANIFEST.json` 记录 zh-CN 全量 136 张、en-US/zh-TW 各 36 张、Settings 五状态 20 张和 125%/150% 各 18 张 current-renderer evidence；150% 检查暴露的 Create 摘要页面级横溢出已通过 `07-create-composer.css` 窄屏 owner 规则修复，并用 focused token test 防回归。版本为 `0.40.1`，`npm.cmd run verify`（88 files / 656 tests、production build、20 组对比度检查）与 `verify:markup-visual` 退出码 0；G18 仍由用户/集成 owner 负责真实 ComfyUI、完整键盘、系统辅助功能、关闭生命周期和最终发布结论。
