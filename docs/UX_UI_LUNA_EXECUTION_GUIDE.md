...
- P17/L52–L55 已完成：Settings tab 补齐 `tablist`/`tab`/`tabpanel`、`aria-selected`/`aria-controls`、roving `tabindex` 与 Arrow/Home/End；`<=900px` 使用单行横向 compact category strip；rescan 移入本机环境 action group；页头保留 dirty/discard/save，并为保存/扫描提供局部 status/`aria-busy`。现有保存、扫描、安装队列、IPC、持久化与字段 selector 未变。900×800/760×800 keyboard smoke、1440×900/1280×800/900×800/760×800 diagnose、20 张 Settings 状态截图与 `npm.cmd run verify`（87 files / 652 tests）通过；P17 已 `verified/integrated`，下一 Phase 为 P18。P17/P18 实现继续以当前 renderer 为源，不得回放旧 prototype。
