...
- P05/L22–L24 已完成：L22 将 72px topbar 接入 `--ux-topbar-height`，将 page sticky offset 对齐为 72px，并在 760px 以下 topbar normal-flow 断点将 offset 归零；L23 将 Create/Queue heading 接入共享 sticky offset，Create 的层级降到 topbar 之下，Queue 在窄屏显式归零；L24 将 History heading、History detail 返回条和 Settings heading 接入同一 token，并保留窄屏 top:0 规则。P05 相关 focused token tests `16/16`、`npm.cmd run verify`（80 files / 618 tests、production build、20 组对比度检查）通过；四个主页面与 Details 的上下滚动、close-dialog 和断点 smoke 仍待执行，下一集成 gate 为 G05；
