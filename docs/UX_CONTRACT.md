...
- H3 performance controls live in Settings under `Performance & acceleration`: dense attention, sparse strategy, native runtime mode and memory compiler are separate persisted selectors. There is no H3 Memory control and no `H3-Optimizations` card. The ComfyUI environment page may show core/package evidence but must not create a second H3 strategy selector.
- H3 selectors are task-level execution inputs for compatible H3 workflows. Saving dense attention, sparse strategy, native runtime, or compiler settings refreshes waiting H3 tasks; a running task remains unchanged, and a failed/cancelled task refreshes when the user resets it for another attempt. Spectrum, Turbo/SLA, live preview and the final video VAE keep their own compatibility/readiness states. A page reorganization must not silently migrate or delete these settings.
