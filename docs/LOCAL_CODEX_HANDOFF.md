...
- ComfyUI 连接测试、参考图上传、任务提交、WebSocket 真实进度、history 轮询兜底与 interrupt。
- 工作流递归占位符注入。
- API workflow 结构、`{{PROMPT}}` 和 `{{INPUT_IMAGE}}` 校验。
- 应用重启时把遗留 `running` 任务恢复为 `waiting`。
- 队列移动、复制、失败重试和同模型优化。
- 成功任务写入历史快照，解析常见输出文件并支持 Explorer 定位。
- PowerShell 环境检查和初始化脚本。

## 还没有完成或仅有框架

1. **模型工作流**：仓库不应假设节点编号。需要在本地 ComfyUI 分别导出 Sulphur 2、Wan 2.2 5B、HunyuanVideo 1.5 的 API JSON。
2. **真实进度实测**：已监听 WebSocket `progress` 和 `execution_error`，但需要用当前 ComfyUI 版本验证消息字段，并决定是否显示当前节点名称。
3. **安全取消**：已经调用 `/interrupt`，但部分视频是否可播放取决于工作流是否持续保存帧。推荐设计为生成帧临时目录 + FFmpeg 原子编码。
4. **输出解析实测**：已解析 `images/gifs/videos/audio/files` 中的 `filename/subfolder/type`。仍需用各视频节点真实 history 响应补 fixture。
...
7. **队列操作**：按钮排序、同模型优化、复制和重试已完成；拖动排序和失败任务编辑尚未实现。
8. **历史交互**：基础详情页和 Explorer 定位已完成；真实播放、悬停预览、版本组和复制文件到剪贴板尚未实现。
