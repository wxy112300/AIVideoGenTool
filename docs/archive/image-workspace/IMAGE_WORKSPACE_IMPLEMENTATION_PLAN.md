...
类型：Archive / mixed historical implementation plan
归档日期：2026-09-07
...
> 制定日期：2026-08-10  
> 版本基线：当前应用为 `0.9.0`；图片放大仍作为后续能力单独验收
> 交互基线：当前 `src/renderer/` 图片创建、图片历史和图片详情实现及 renderer evidence；`prototypes/` 仅为历史参考

当前边界：正式 renderer 已支持图片创建模式、图片批次队列、独立图片历史 Tab、图片项目详情、Qwen 2511 图片编辑和两种图片模型适配；FLUX.2 Klein 4B 已完成真实 GPU 验收并提供快速/高质量档位，独立 AI 图片放大仍需按本文实施。
...
- Picture 顺序必须稳定。删除中间 Picture 后不得静默把其他引用改指向不同素材；已有 Picture 编号保持不变，新建槽位优先回收最低的空缺编号。
...
3. **P2：Phase 3，图片批次执行和产物安全**
  - 在 Qwen 单图闭环之后，完成多 run 顺序执行、重启恢复、取消保留、真实尺寸读取、PNG 输出和失败保护。
...
当前 P2 还必须接入独立视频/图片输出目录：入队快照保存最终目录，workflow 使用相对 output 子目录；当前首发固定 PNG，目录迁移仍遵守复制后校验、最后清理源文件的顺序。
...
- 重启不会重复已完成 run。
- PNG 文件能预览、复制并再次作为 Picture 输入。
- 输出验证失败不会删除 ComfyUI 原始输出。

依赖：Phase 0、1；renderer 卡片可与本 Phase 后半段并行，但真实图片提交必须先通过本 Phase 的产物验收。

## Phase 4：图片历史 Tab、项目聚合与删除

当前状态：正式 renderer 已完成视频/图片 Tab、图片项目聚合、auto/pinned 封面、图片版本缩略图、独立图片列表缩略图缓存、右键菜单、项目删除和单版本删除；待真实图片运行数据继续验收。
...
当前状态：正式 renderer 已完成版本轨道、大图查看、Prompt/来源/生成信息、像素复制、文件复制、Explorer 定位、封面操作、继续编辑、视频交接和删除确认；图片放大弹窗与放大队列仍属于 Phase 6。
