...
> 类型：Archive / historical research
> 状态：已归档；候选能力已拆分到当前 H3 TASK、长视频 TASK 或 research，不再作为实施计划入口。
> 归档日期：2026-09-07
...
| Phase 5：H3 Studio Image / Keyframe / Audio Drive / Chain | 🟡 已确认范围，待分阶段实施 | H3 Studio Image 与 Chain/latent Extend 已提升为高价值能力；仍按独立 Catalog/Adapter/Queue/History 接入，不复制参考项目。 Native Masked AV 长视频的专项接入边界见 [H3_NATIVE_MASKED_AV_LONG_VIDEO_PLAN.md](../h3-long-video/H3_NATIVE_MASKED_AV_LONG_VIDEO_PLAN.md)。 |
