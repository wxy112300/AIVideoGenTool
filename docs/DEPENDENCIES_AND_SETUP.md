...
- 本轮 H3 升级以 ComfyUI `0.35.0` 为推荐基线：原生 `ModelAttentionBackend`、`BlockSparseAttention`、H3 PDD head bank 和 `MiniMaxH3SigmaShift` 均由核心提供，不再依赖已撤回的 H3-Optimizations/AIMDO 产品节点。0.35 环境的验证基线包括 `comfy-kitchen==0.2.33`、`comfy-aimdo==0.5.3`、匹配的 SageAttention/Triton 与 PyTorch 2.10 cu130 trio；这些是所选 ComfyUI Python 的运行时证据，不是应用内置权重。
- 设置中的 H3 policy 在任务等待时跟随保存后的 dense attention（Sage、Sage-Triton、PyTorch、Comfy Kitchen）、sparse attention（Auto、关闭、Sol-Attention、原生 SLA、未验证即拒绝的 VSA）、`compatibility/native` runtime 和 `auto/disabled` Comfy compiler；任务开始后冻结本次执行。执行时按实时 `/object_info` 核对 0.35 节点的精确输入 schema；显存、async offload、compiler 和最终视频 VAE 不会跨工作流或跨正在执行的任务泄漏。
...
- H3 原生音视频采样的最低 ComfyUI 版本为 `v0.31.0`，当前推荐 `v0.35.0`；推荐版本是更新提示，不是离线入队的硬性阻挡。当前支持的 Turbo 权重包括官方 v1.2 768p 4-step、v1.0 8-step 和 Ref2V 4-step，均放入所选 ComfyUI 的 `models/loras`，不要把它们当成独立基础模型。旧 ckpt850 EMA、v0.1 FL2VA、旧 v1.0 768p 和 PinkFluffyBunny 已从新任务目录移除；历史只保留名称快照，编辑历史时不会带入创作页；AfterMidnight v1.2 仅用于 Ref2VA，不可移植到 FL2VA。
