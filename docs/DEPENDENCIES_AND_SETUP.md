...
- H3 原生音视频采样的最低 ComfyUI 版本为 `v0.31.0`，当前推荐 `v0.33.1`；推荐版本是更新提示，不是离线入队的硬性阻挡。当前支持的 Turbo 权重包括官方 v1.2 768p 4-step、v1.0 8-step 和 Ref2V 4-step，均放入所选 ComfyUI 的 `models/loras`，不要把它们当成独立基础模型。旧 ckpt850 EMA、v0.1 FL2VA、旧 v1.0 768p 和 PinkFluffyBunny 已从新任务目录移除；历史只保留名称快照，编辑历史时不会带入创作页；AfterMidnight v1.2 仅用于 Ref2VA，不可移植到 FL2VA。
